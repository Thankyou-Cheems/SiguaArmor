export type HitScenePackFormatVersion = "hit-scene-record/v1";
...
export interface HitScenePackSection {
  byteOffset: number;
  byteLength: number;
  elementCount: number;
...
export interface HitScenePackSource {
...
export interface HitScenePackCounts {
  vertices: number;
  triangles: number;
...
export interface HitScenePackHeader {
  formatVersion: HitScenePackFormatVersion;
  vehicleId: string;
  officialNameZh: string;
  source: HitScenePackSource;
  coordinateSystem: HitSceneCoordinateSystem;
  publishStatus: HitScenePublishStatus;
  counts: HitScenePackCounts;
  sections: {
    positions: HitScenePackSection;
    indices: HitScenePackSection;
    triangleComponentIndex: HitScenePackSection;
    triangleSurfaceProfileIndex: HitScenePackSection;
    faceNormals: HitScenePackSection;
    bvhRoots: HitScenePackSection[];
    bvhIndirectBuffer: HitScenePackSection;
  };
  bvh: {
...
export interface HitScenePackDescriptor {
  formatVersion: HitScenePackFormatVersion;
  vehicleId: string;
  packPath: string;
  sha256: string;
...
export interface HitScenePackManifest {
  schemaVersion: "1.0.0" | "2.0.0";
  formatVersion: HitScenePackFormatVersion;
  accessPolicy: {
    publishStatus: HitScenePublishStatus;
  };
  packs: HitScenePackDescriptor[];
}
...
export interface ParsedHitScenePack {
