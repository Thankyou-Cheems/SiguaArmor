...
export function normalizeViewerNavigationState(value = {}) {
  ...
  return {
    view,
    protection,
    ...(value.collisionQuery === "simple" ? { collisionQuery: "simple" } : {}),
...
export function parseCatalogLocation(input, index, options = {}) {
  ...
  const viewer = normalizeViewerNavigationState({
    ...
    collisionQuery: url.searchParams.get("collision"),
    attacker:
    ...
    // Engagement distance belongs to the current viewer session. Refreshing or
    // opening a shared URL intentionally starts from the default distance.
    distance: 0,
    yaw:
...
export function buildCatalogUrl(state, index, options = {}) {
  ...
  if (selectedCard) {
    ...
    if (viewer.collisionQuery === "simple") params.set("collision", "simple");
