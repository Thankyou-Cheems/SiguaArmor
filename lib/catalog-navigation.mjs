...
function compactAttacker(value, index) {
  const recordIndex = index.records.findIndex((record) => record.promoEntryId === value);
  return recordIndex >= 0 ? recordIndex.toString(36) : null;
}

function expandCompactAttacker(value, index) {
...
export function normalizeViewerNavigationState(value = {}) {
  ...
  const distance = roundNavigationNumber(
    clamp(Number.isFinite(Number(value.distance)) ? Number(value.distance) : 0, 0, 3000),
  );
...
export function parseCatalogLocation(input, index, options = {}) {
  ...
  const compactAttackerId = expandCompactAttacker(url.searchParams.get("a"), index);
  ...
  const viewer = normalizeViewerNavigationState({
    ...
    attacker: compactAttackerId || url.searchParams.get("attacker") || "",
...
export function buildCatalogUrl(state, index, options = {}) {
  ...
  if (selectedCard) {
    ...
    if (viewer.attacker) {
      const attackerIndex = compactAttacker(viewer.attacker, index);
      if (attackerIndex === null) params.set("attacker", viewer.attacker);
      else params.set("a", attackerIndex);
    }
