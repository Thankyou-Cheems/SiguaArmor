...
export type VehicleProjectilePlaybackResolution = {
    ...
    state: "unsupported";
    reason: "catalog-invalid" | "mount-binding-missing" | "mount-binding-ambiguous" | "station-binding-missing" | "station-binding-ambiguous" | "station-mismatch" | "launch-origin-missing" | "launch-route-unsupported" | "launch-frame-unresolved" | "launch-anchor-missing" | "launch-anchor-ambiguous" | "projectile-profile-missing" | "movement-mode-unresolved" | "guidance-live-input-required";
    detail: string;
};
...
const stripObjectPath = (value: unknown) => typeof value === "string" ? value.replace(/^uobject:/u, "") : "";
...
function componentSpaceLaunchShot(shot: LaunchOriginProfile["shots"][number]): LaunchOriginProfile["shots"][number] | null {
...
function hasGuidanceValue(value: Record<string, unknown> | null) {
    return Boolean(value && Object.values(value).some((entry) => entry !== null && entry !== undefined));
}
function guidedProjectile(profile: ProjectileProfile) {
    const guided = profile.guided ?? {};
    return (finite(guided.aimMaxDistanceCm) > 0 ||
...
function guidedPlaybackSupported(projectileProfile: ProjectileProfile, movementModes: ProjectileMovementMode[]) {
    ...
    return guidanceLossBehaviours(projectileProfile.guided).every((entry) => {
        ...
        if (finite(value?.TimeBeforeDetonationAfterGuidanceLoss) !== 0)
            return false;
        if (behaviour === "ContinueLastKnownTarget")
            return true;
        if (behaviour !== "ChangeMovementMode")
            return false;
...
function stationWeaponLaunchAnchor(station: CompiledVehicleStationGraph["stations"][number]): Extract<VehicleProjectilePlaybackBinding["launchAnchor"], {
    kind: "station-weapon-attachment";
}> | null {
    ...
    if (attachment?.state !== "derived-seat-pawn-component" ||
        ...
        !["observed", "derived", "derived-with-fallback"].includes(frame?.state ?? "unresolved") ||
        !value ||
        ![
...
export function compileVehicleProjectilePlaybackBinding({ catalog, stationGraph, stationId, visualPlacements = [], weapon, }: {
    ...
    const variantId = weaponAssignmentId.startsWith(`${equipmentBindingId}:`)
        ...
        : "";
    let mountBindings = catalog.vehicleMountBindings.filter((binding) => binding.equipmentBindingId === equipmentBindingId &&
        binding.cardId === weapon.sourceCardId &&
        binding.rawName === weapon.sourceRawName);
    if (mountBindings.length > 1 && variantId) {
        const variantBindings = mountBindings.filter((binding) => binding.weaponVariantIds.includes(variantId));
        if (variantBindings.length > 0)
            mountBindings = variantBindings;
    }
    ...
    const launchOrigin = catalog.launchOriginProfiles.find((candidate) => candidate.id === assignment.launchOriginProfileRef);
    if (!launchOrigin) {
        return unsupported("launch-origin-missing", "武器缺少 WeaponMesh1P 发射点档案");
    }
    const invalidCommonLaunchRoute = launchOrigin.anchorRole !== "weapon-actor-root" ||
        launchOrigin.componentRole !== "WeaponMesh1P";
    const singleBarrelRoute = launchOrigin.kind === "weapon-mesh1p-socket" &&
        launchOrigin.shotSelection === "single-barrel-socket" &&
        launchOrigin.shots.length === 1;
    const runtimeMultipodRoute = launchOrigin.kind === "multipod-socket-forward-offset" &&
    ...
    if (invalidCommonLaunchRoute || (!singleBarrelRoute && !runtimeMultipodRoute)) {
        return unsupported("launch-route-unsupported", "发射点不是已闭合的单炮口或原生运行时多筒 WeaponMesh1P 路线");
    }
    const projectileProfile = catalog.projectileProfiles.find((candidate) => candidate.id === assignment.projectileProfileRef);
    ...
    if (movementModes.some((mode) => mode === null)) {
        return unsupported("movement-mode-unresolved", "弹丸引用了未收录的运行时 movement mode");
    }
    const resolvedMovementModes = movementModes.filter((mode): mode is ProjectileMovementMode => mode !== null);
    const isGuided = hasGuidanceValue(assignment.guidanceController) ||
    ...
    if (!isGuided && resolvedMovementModes.length > 1) {
        return unsupported("movement-mode-unresolved", "非制导弹丸存在多个未选择的运行时 movement mode");
    }
    if (isGuided && (!hasGuidanceValue(assignment.guidanceController) ||
        !guidedPlaybackSupported(projectileProfile, resolvedMovementModes))) {
        return unsupported("guidance-live-input-required", "制导档案缺少可验证的 controller、loss behaviour 或 movement mode");
    }
    ...
    if (launchShots.some((shot) => shot === null)) {
        return unsupported("launch-route-unsupported", "组件锚点缺少可验证的插槽组件空间变换");
    }
    return {
        state: "ready",
        binding: {
            ...
            launchSelection: runtimeMultipodRoute
                ...
                : { kind: "single-barrel-socket" },
            launchShots: launchShots.filter((shot): shot is LaunchOriginProfile["shots"][number] => shot !== null),
...
export function selectVehicleProjectileLaunchShot(binding: VehicleProjectilePlaybackBinding, state: VehicleProjectileMagazineState) {
    if (!Number.isInteger(state.shotsFiredInMagazine) ||
    ...
    if (!shot)
        throw new Error("当前发射筒索引没有源锁定插槽");
    return {
        shotIndex,
        shot,
...
export function localProjectileAccelerationToWorld(local: Record<string, unknown>, direction: ProjectileVector3): ProjectileVector3 {
    ...
    return add(add(scale(forward, finite(local.X)), scale(right, finite(local.Y))), scale(up, finite(local.Z)));
}
export function buildVehicleProjectileSimulationInput(binding: VehicleProjectilePlaybackBinding, launch: {
    ...
    const base = {
        ...
        gravityZCmPerSecondSquared: binding.physics.worldGravityZCentimetresPerSecondSquared,
        gravityScale: finite(movement.ProjectileGravityScale, 1),
        constantAccelerationWorld: localProjectileAccelerationToWorld((movement.ConstantAcceleration ?? {}) as Record<string, unknown>, direction),
        constantAccelerationTimeoutSeconds: finite(movement.ConstantAccelerationTimeout),
        ...
        bounceAngleAffectsFriction: movement.bBounceAngleAffectsFriction !== false,
        minFrictionFraction: finite(movement.MinFrictionFraction),
        bounceVelocityStopSimulatingThresholdCmPerSecond: finite(movement.BounceVelocityStopSimulatingThreshold, 5),
        ...
        movementModeCurves: new Map(binding.curveAssets.map((curve) => [curve.assetPath, curve])),
    };
...
export function presentationProjectileSpreadSample(weaponAssignmentId: string, shotSequence: number) {
    const first = xorshift(hashSeed(`${weaponAssignmentId}:${shotSequence}`));
    const second = xorshift(first);
    return {
        azimuthUnit: first / 4294967296,
        cosineUnit: second / 4294967296,
    };
}
export function sampleProjectileTrajectory(samples: NativeProjectileTrajectorySample[], timeSeconds: number) {
    if (samples.length === 0)
        return null;
    if (timeSeconds <= samples[0]!.timeSeconds)
        return samples[0]!;
    if (timeSeconds >= samples.at(-1)!.timeSeconds)
        return samples.at(-1)!;
    let low = 0;
    ...
    const interpolate = (start: ProjectileVector3, end: ProjectileVector3): ProjectileVector3 => ({
    ...
    return {
        ...
        velocityCmPerSecond: interpolate(left.velocityCmPerSecond, right.velocityCmPerSecond),
...
export function loadWikiNativeProjectileAlgorithm(pathname: string) {
    ...
    projectileAlgorithmRequest = import(source)
        ...
        if (typeof candidate.simulateNonGuidedProjectile !== "function" ||
