import { projectCrewSeatBinding, type RuntimeCrewSeatBinding, type RuntimeCrewSeatStation, } from "./vehicle-crew-seat-runtime.ts";
...
export interface StationGraphStation {
    ...
    occupantMotion: {
        state: "derived-static-component-ancestry" | "derived-blueprint-vehicle-component-route";
    ...
    occupantAttachment: {
        state: "derived-seat-pawn-component-ancestry" | "observed-blueprint-vehicle-component-socket" | "derived-hidden-missing-socket-fallback";
        parent: {
        ...
        referenceFrame: StationGraphFrame;
        spatialMeaning: "runtime-soldier-attachment" | "hidden-runtime-fallback-no-rendered-body";
        source: string;
...
function frameClosed(frame: StationGraphFrame) {
    ...
    if (frame?.state === "unresolved")
        return value === null;
    return Boolean(["observed", "derived", "derived-with-fallback"].includes(frame?.state) &&
        value &&
...
export function compileVehicleStationGraph(record: VehicleStationGraphRecord | null, pointer: VehicleStationGraphPointer | null, owner: VehicleStationGraphOwner, placements: VehicleStationGraphVisualPlacement[]): CompiledVehicleStationGraph | null {
    if (!record || !pointer)
        return null;
    if (record.schemaVersion !== "sigua-vehicle-station-graph/v1" ||
    ...
    const binding = record.visualBindings.find((candidate) => (owner.catalogBindingRef === undefined ||
    ...
    const placementById = new Map(placements.map((placement) => [placement.stableOccurrenceId, placement]));
    ...
    for (const station of record.stations) {
        if (station.id !== `${record.sourceVehicleRef}:station:${station.catalogSeatIndex}` ||
        ...
        if (![
            ...
            station.occupantMotion.channels.some((channel) => channel !== "yaw" && channel !== "pitch") ||
        ...
        if (![
            ...
            !["vehicle-component", "station-component"].includes(station.occupantAttachment.parent?.kind) ||
    ...
    for (const station of record.stations) {
        ...
        if (new Set(inheritedChannels).size !== inheritedChannels.length ||
    ...
    const visitParent = (stationId: string) => {
        ...
        if (parent.kind === "station")
            visitParent(parent.stationId);
        visitState.set(stationId, "visited");
    };
    for (const stationId of stationById.keys())
        visitParent(stationId);
    const stationSeatIds = new Set<string>();
    for (const seat of record.seats) {
        const station = seat.stationId ? stationById.get(seat.stationId) : null;
        if ((seat.stationKind === null) !== (seat.stationId === null) ||
            (seat.stationId &&
                (!station ||
        ...
        if (seat.stationId)
            stationSeatIds.add(seat.stationId);
    }
    ...
    for (const occurrence of binding.occurrences) {
        ...
        if (occurrenceIds.has(occurrence.stableOccurrenceId) ||
            ...
            throw new Error(`SiguaWiki station graph occurrence ${occurrence.stableOccurrenceId} drifted`);
        }
        occurrenceIds.add(occurrence.stableOccurrenceId);
    }
    const visualStations: RuntimeStationGraphVisualStation[] = record.stations.map((station) => {
        const closure = binding.stationClosures.find(({ stationId }) => stationId === station.id);
        if (!closure) {
            throw new Error(`SiguaWiki station ${station.id} visual closure is missing`);
        }
        const owned = binding.occurrences.filter(({ ownerStationId }) => ownerStationId === station.id);
        const yawMembers = owned.map(visualMember);
        const pitchMembers = owned.filter(({ motionChannels }) => motionChannels.includes("pitch")).map(visualMember);
        const preferredYawAnchor = owned.find(({ source }) => source === "v10.5.3-seat-pawn-component-descendant") ?? owned[0] ?? null;
        return {
            ...
            movementState: station.closure.motion === "closed" ? "observed" : "unresolved",
            motion: {
                state: station.closure.motion === "closed" ? "derived" : "unresolved",
                ...
                sourceFunction: station.motion.yaw.sourceFunction as "runtime" | "runtime",
                ...
                ? {
                    positionSemantics: {
                        ...
                        seatPawnAttachment: {
                            ...
                            parentCatalogSeatIndex: stationById.get(station.mount.parent.stationId)!
                                .catalogSeatIndex,
                        },
                    },
                }
    ...
    projectCrewSeatBinding(crewSeat, {
