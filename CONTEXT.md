...
| Owner | Keeps |
| --- | --- |
| SiguaArmor | Product code, product tests, browser-ready presentation assets, build/release/deploy tooling |
...
bootstrap pinned by `public-assets.json`; it is intentionally ignored by Git. This URL is
for low-frequency developer/CI restoration, not production hotlinking. The live site
serves the same content-addressed files from the project-owned origin and CDN. Routine
updates upload only bytes absent from the live baseline through `release:delta`; they do
not create monthly full GitHub archives.
