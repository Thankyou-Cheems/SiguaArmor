...
Protection analysis prepares the model world matrices and hit normal matrix once inside each synchronous sampling batch, after its cancellation and generation checks. `prepareRuntimeHitRayQuery` owns that preparation and the complete ordered hit conversion. The prepared function never survives an animation-frame callback: the next batch prepares again, while hover/click/shot queries prepare independently. Sampling positions, all-hit traversal, surface identities, signed incidence and damage rules are unchanged. Display meshes are never substituted for hit geometry.

This applies invariant reuse inspired by Wald et al., [Interactive Rendering with Coherent Ray Tracing](https://doi.org/10.1111/1467-8659.00508), Eurographics 2001; it does not implement the paper's SIMD packet traversal. Production-page A/B probes compare complete ordered hits and protection-grid hashes against the preceding collector. Research owns those measurements. Selective BVH refit remains an experiment: full-pose page measurements did not justify adding its node-membership cache to the runtime, which retains full refit.

`createProtectionFrameBudget` adjusts scheduling without changing precision or sample order. Each calculation starts at 512 rays; measured sampling throughput can grow the next cap by at most 2x, up to 8,192, or lower it immediately. Every visited cell checks a 2–6 ms sampling deadline. The controller targets 8 ms including recent reconstruction/paint overhead; one indivisible ray, GC or a level transition can exceed that target. A single initial visit guarantees progress. The 32,768-visit ceiling also bounds scanning cached super-grid cells when the clock is coarse. Cancellation and generation checks precede every batch.

One-off level/grid setup is excluded from recurring-overhead feedback. Its cost cannot be recovered by shrinking every later batch. Monitor refresh rate is not treated as CPU capacity: aggressively shrinking work to a fraction of a high-refresh interval caused a feedback collapse in throttled page tests. A paused tab grants no additional work, and a new calculation resets the controller. The time guard, rather than the maximum count, controls expensive-hit workloads. Compare completion time, callback duration, interaction cancellation and final grid identity; a lower frame count alone is insufficient.
