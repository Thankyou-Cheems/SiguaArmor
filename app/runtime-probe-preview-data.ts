...
import { loadWikiRuntimeVisual, loadOptionalWikiVehicleGunnerSight, loadWikiVehicleDriverView, loadWikiVehicleRuntimeSource, loadWikiVehicleStationGraph, } from "../lib/wiki-source";
...
export interface RuntimeVisualAttachmentMember {
    ...
    componentClassPath: string;
    sourceMeshPath: string;
    source: "v10.5.3-seat-pawn-component-descendant" | "exact-station-equipment";
...
interface RuntimeStationMotion {
    ...
    driverMode: "split-yaw-pitch-components" | "combined-updated-component" | "paired-rotating-components";
    coordinateSystem: {
        space: "vehicle-local";
...
export interface RuntimeVisualAttachmentStation {
    ...
    closureMode: "visual-occurrence-membership" | "view-component-rotation";
...
const visualComponentSuppressionRules = (visualSelectionPolicyJson.globalSuppressions ?? []) as RuntimeVisualComponentSuppressionRule[];
const synchronizedWeaponPolicy = visualSelectionPolicyJson.synchronizedWeaponPolicy as RuntimeVisualSynchronizedWeaponPolicy;
const previewCache = new Map<string, Promise<RuntimeVehiclePreview>>();
function applySynchronizedWeaponPolicy(cardId: string, rawName: string, placements: RuntimeVisualPlacement[]) {
    if (synchronizedWeaponPolicy.schemaVersion !==
    ...
    const groups = synchronizedWeaponPolicy.groups.filter((group) => group.bindings.some((binding) => binding.cardId === cardId && binding.rawName === rawName));
    let filteredPlacements = placements;
    const selectedActors: string[] = [];
    let filteredOccurrences = 0;
    for (const group of groups) {
        const candidates = filteredPlacements.filter((placement) => placement.name === "WeaponMesh3P" &&
        ...
        const active = candidates.filter((placement) => placement.actor === group.activeActor);
        const suppressedIds = new Set(candidates
        ...
        filteredPlacements = filteredPlacements.filter((placement) => !suppressedIds.has(placement.stableOccurrenceId));
    }
    return { placements: filteredPlacements, selectedActors, filteredOccurrences };
}
function applyVisualSelection(cardId: string, rawName: string, sourcePlacements: RuntimeVisualPlacement[]) {
    const globallySuppressedIds = new Set(sourcePlacements
        .filter((placement) => visualComponentSuppressionRules.some((rule) => rule.componentNames.includes(placement.name) &&
        ...
        .map((placement) => placement.stableOccurrenceId));
    const globallyFiltered = sourcePlacements.filter((placement) => !globallySuppressedIds.has(placement.stableOccurrenceId));
    const synchronized = applySynchronizedWeaponPolicy(cardId, rawName, globallyFiltered);
    const rule = visualSelectionRules.find((candidate) => candidate.cardId === cardId && candidate.rawName === rawName);
    if (!rule) {
        return {
    ...
    const targets = synchronized.placements.filter((placement) => placement.name === rule.target.componentName &&
        rule.target.actorIncludes.some((needle) => placement.actor.includes(needle)));
    const managed = synchronized.placements.filter((placement) => managedNames.has(placement.name) &&
    ...
    return {
        placements: synchronized.placements.filter((placement) => !managedIds.has(placement.stableOccurrenceId) ||
        ...
        selection: {
            ...
            filteredOccurrences: managed.filter(({ stableOccurrenceId }) => stableOccurrenceId !== selectedId).length +
...
function toRuntimePreview(cardId: string, rawName: string, artifactRef: string, runtimeVariant: WikiVehicleRuntimeVariant, descriptor: WikiRuntimeVisual, stationGraphRecord: VehicleStationGraphRecord | null, gunnerSightRecord: VehicleGunnerSightRecord | null, driverViewRecord: VehicleDriverViewRecord): RuntimeVehiclePreview {
    ...
    const { placements, selection } = applyVisualSelection(cardId, rawName, descriptor.placements);
    const skeletalPlacements = placements.filter((placement) => placement.componentClassPath.includes("SkeletalMeshComponent"));
    const stationGraph = compileVehicleStationGraph(stationGraphRecord, runtimeVariant.stationGraph, {
        rawName,
    ...
    const visualAttachment = stationGraph.visualAttachment as RuntimeVisualAttachmentBinding;
    const crewSeat = stationGraph.crewSeat;
    const gunnerSight = compileVehicleGunnerSight(gunnerSightRecord, stationGraph);
    ...
    return {
        ...
        visual: {
            ...
            runtimeBonePoseJointCount: skeletalPlacements.reduce((sum, placement) => sum + (placement.runtimeBonePoseJointCount ?? 0), 0),
            runtimeBonePoseReferenceEquivalentOccurrenceCount: skeletalPlacements.filter(({ runtimeBonePoseReferenceEquivalent }) => runtimeBonePoseReferenceEquivalent === true).length,
            placements,
            selection,
        },
        hit,
...
export async function runtimePreviewForCatalogBinding(cardId: string, rawName: string, expectedRuntimeVehicleId: string | null, expectedVisualArtifactRef: string | null, siteEdition: SiteEdition = "international") {
    ...
    const request = Promise.all([
        ...
        const runtimeVariant = runtimeSource.variants.find((variant) => variant.rawName === rawName);
        ...
        if (descriptor.edition !== siteEdition ||
        ...
        const stationGraphRecord = runtimeVariant.stationGraph
            ? await loadWikiVehicleStationGraph(runtimeVariant.stationGraph.recordUrl) as VehicleStationGraphRecord
            : null;
        if (stationGraphRecord &&
        ...
        const [gunnerSightRecord, driverViewRecord] = stationGraphRecord
            ? await Promise.all([
                loadOptionalWikiVehicleGunnerSight(stationGraphRecord.sourceVehicleRef) as Promise<VehicleGunnerSightRecord | null>,
                loadWikiVehicleDriverView(stationGraphRecord.sourceVehicleRef) as Promise<VehicleDriverViewRecord>,
            ])
            : [null, null];
        if (!driverViewRecord) {
...
export async function runtimePreviewForVariant(cardId: string, rawName: string, siteEdition: SiteEdition = "international") {
    const runtimeSource = await loadWikiVehicleRuntimeSource(cardId) as WikiVehicleRuntimeSource;
    const runtimeVariant = runtimeSource.variants.find((variant) => variant.rawName === rawName);
    if (!runtimeVariant)
        return null;
    const visualArtifactRef = runtimeVariant.visualArtifacts[siteEdition]?.id ?? null;
    if (!visualArtifactRef)
        return null;
    return runtimePreviewForCatalogBinding(cardId, rawName, runtimeVariant.runtimeVehicleRef, visualArtifactRef, siteEdition);
}
export function loadRuntimePreviewForVariant(cardId: string, rawName: string, siteEdition: SiteEdition = "international") {
