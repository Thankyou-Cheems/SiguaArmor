...
import { ARMOR_THICKNESS_LEGEND_STOPS, ARMOR_THICKNESS_LEGEND_TICKS, RELATIVE_ARMOR_THICKNESS_STOPS, } from "../lib/armor-thickness-ramp";
import { ANALYSIS_VISUAL_DEPTH_BIAS_FACTOR, ANALYSIS_VISUAL_DEPTH_BIAS_UNITS, analysisVisualStableSurfaceReasons, isStableAnalysisVisualSurfacePlacement, type AnalysisVisualSurfaceEvidence, } from "../lib/analysis-visual-surface-policy";
...
import { RUNTIME_GROUND_SCALE_LABEL_INTERVAL_M, RUNTIME_GROUND_SCALE_TICK_INTERVAL_M, runtimeGroundScaleLengthM, } from "../lib/runtime-ground-scale";
import { shotResultRendersDirectTrace } from "../lib/shot-visual-policy";
import { RUNTIME_VIEWER_CAMERA_VIEWS, RUNTIME_VIEWER_INFANTRY_DISTANCES_M, SQUAD_INFANTRY_DEFAULT_HORIZONTAL_FOV_DEG, runtimeViewerCameraPose, runtimeViewerInfantryCameraPosition, verticalFovForHorizontalFov, type RuntimeViewerCameraViewId, } from "../lib/runtime-viewer-camera-presets";
import { weaponPenetrationKindForDamageTypePath, type WeaponPenetrationKind, } from "../lib/weapon-penetration-kind";
import { weaponNameZh } from "../lib/weapon-display-name";
import { vehicleDamageTypeIconColor, vehicleDamageTypeIconColorNumber, vehicleDamageTypeIconKindForPath, vehicleDamageTypeIconLabel, vehicleDamageTypeIconShortLabel, type VehicleDamageTypeIconKind, } from "../lib/vehicle-damage-type-icons";
import { playerHitComponentLabel } from "../lib/runtime-component-labels";
import { componentOnlyDamageSurfaceInfo, noPenetrationSurfaceInfo, spacedArmorSurfaceInfo, } from "../lib/hit-scene-render-batches";
import { clearHitSceneThreeModelDamageHighlight, createHitSceneThreeModel, setHitSceneThreeModelArmorThicknessScale, setHitSceneThreeModelComponentPoses, setHitSceneThreeModelDamageHighlight, setHitSceneThreeModelHoveredProfile, setHitSceneThreeModelMode, setHitSceneThreeModelSpecialArmorVisible, type HitSceneArmorThicknessRange, type HitSceneThreeModel, } from "../lib/hit-scene-three-renderer";
...
import type { RuntimeCrewSeatStation, RuntimeCrewSeatView, } from "../lib/vehicle-crew-seat-runtime";
...
import { OPERATION_VIEW_STANDARD_ASPECT_RATIO, OPERATION_VIEW_STANDARD_HORIZONTAL_FOV_DEGREES, createOperationViewPoseCommitScheduler, operationViewEquipmentRefs, operationViewKeyAction, operationViewHorizontalFovForMagnification, operationViewMotionStep, operationViewScenePresentation, type OperationViewMotionState, } from "../lib/operation-view-control";
...
import { TurretPreviewControls, type TurretOrientationIndicator, type TurretPreviewIndicatorKind, type TurretPreviewStation, } from "./TurretLimitsDisplay";
import { paintVehicleDamageTypeIconCanvas, } from "./VehicleDamageTypeIcon";
import { type RuntimeAttackSource, type RuntimeAttackSourceWeapon, } from "./runtime-probe-weapon-labels";
import { createRuntimeAttackSourceLibrary, createRuntimeStationEquipmentResolver, resolveRuntimeAttackSourceIndexEntry, type RuntimeAttackSourceLibrary, type RuntimeAttackSourcePresentation, type RuntimeStationEquipmentResolver, type WikiWeaponRuntimeIndexDocument, type WikiWeaponRuntimeSourceDocument, } from "./runtime-wiki-attack-source";
import { RUNTIME_PROTECTION_MAP_BLOCK_SIZE, RUNTIME_PROTECTION_MAP_MAX_PRECISION, RUNTIME_PROTECTION_MAP_MIN_PRECISION, RUNTIME_PROTECTION_MAP_STANDARD_MAX_PRECISION, RUNTIME_PROTECTION_MAP_SUPER_PRECISION, RUNTIME_PROTECTION_MAP_UI_UPDATE_INTERVAL_MS, clampRuntimeProtectionMapPrecision, classifyRuntimeProtectionShot, reconstructRuntimeProtectionMapBlock, runtimeProtectionMapCumulativeSampleCount, runtimeProtectionMapFrameHasBudget, runtimeProtectionMapGridSize, runtimeProtectionMapLevelOffsets, runtimeProtectionMapSuperGridSize, type RuntimeProtectionMapCell, type RuntimeProtectionMapPrecision, type RuntimeProtectionMapStandardPrecision, } from "../lib/runtime-protection-map";
import { decodeSharedShotPaths, encodeSharedShotPaths, } from "../lib/viewer-shot-share.mjs";
import { decodeViewerCameraState, encodeViewerCameraState, } from "../lib/viewer-camera-share.mjs";
import { decodeViewerTurretState, encodeViewerTurretState, } from "../lib/viewer-turret-share.mjs";
...
const FORCED_RUNTIME_RENDER_QUALITY_TIER = process.env.NEXT_PUBLIC_SIGUA_RENDER_QUALITY === "compatibility" ||
...
function createReferenceSoldierOutlineProxy(onTextureReady: () => void) {
    ...
    const texture = new THREE.TextureLoader().load("/images/reference-soldier-outline.webp", () => {
...
function runtimeGroundReferenceClearanceM(vehicleLengthM: number, vehicleWidthM: number) {
    return THREE.MathUtils.clamp(Math.max(vehicleLengthM, vehicleWidthM) * 0.2, RUNTIME_GROUND_REFERENCE_MIN_CLEARANCE_M, RUNTIME_GROUND_REFERENCE_MAX_CLEARANCE_M);
}
type SearchableSelectEffectRole = "penetration" | "direct-damage" | "radial-damage";
...
function selectorWeaponEffects(directFireRoute: boolean, ballistics: EditorNativeBallistics) {
    const effects: SearchableSelectEffect[] = [];
    if (directFireRoute) {
        const penetrationKind = weaponPenetrationKindForDamageTypePath(ballistics.damageTypePath);
        const penetrationMm = ballistics.penetrationAtRangeMm ?? 0;
        const penetrationLabel = penetrationKind === "shaped-charge"
            ? "破甲"
            : "穿深";
        const directDamageTypeKind = vehicleDamageTypeIconKindForPath(ballistics.damageTypePath) ??
            "generic";
        const directDamage = ballistics.impactDamageAtRange ?? 0;
        effects.push({
            id: "penetration",
            role: "penetration",
            damageTypeKind: penetrationKind === "shaped-charge" ? "heat" : "kinetic",
            penetrationKind,
            value: penetrationMm,
            eventIndex: null,
            title: `${penetrationLabel}能力：${metricText(penetrationMm)} 毫米`,
        });
        effects.push({
            id: "direct-damage",
            role: "direct-damage",
            ...
            value: directDamage,
            eventIndex: null,
            title: `${vehicleDamageTypeIconLabel(directDamageTypeKind)}：${metricText(directDamage)} 点直击伤害`,
        });
    }
    ballistics.explosiveLayers.forEach((layer, eventIndex) => {
        const damageTypeKind = vehicleDamageTypeIconKindForPath(layer.damageTypePath) ?? "generic";
        effects.push({
            ...
            eventIndex,
            title: `${vehicleDamageTypeIconLabel(damageTypeKind)}：${metricText(layer.baseDamage)} 点径向基础伤害（事件 ${eventIndex + 1}）`,
        });
    });
    return effects;
}
const ARMOR_THICKNESS_LEGEND_GRADIENT = `linear-gradient(90deg, ${ARMOR_THICKNESS_LEGEND_STOPS.map((stop) => {
    const [red, green, blue] = stop.rgb.map((channel) => Math.round(channel * 255));
    return `rgb(${red}, ${green}, ${blue}) ${stop.normalizedPosition * 100}%`;
}).join(", ")})`;
const RELATIVE_ARMOR_THICKNESS_LEGEND_GRADIENT = `linear-gradient(90deg, ${RELATIVE_ARMOR_THICKNESS_STOPS.map((stop) => {
    const [red, green, blue] = stop.rgb.map((channel) => Math.round(channel * 255));
    return `rgb(${red}, ${green}, ${blue}) ${stop.normalizedPosition * 100}%`;
}).join(", ")})`;
...
const SHOT_EXPLOSION_LAYER_DELAY_MS = RADIAL_DAMAGE_VISUAL_TIMING_MS.layerDelay;
const SHOT_EXPLOSION_EXPANSION_DURATION_MS = RADIAL_DAMAGE_VISUAL_TIMING_MS.expansion;
const SHOT_EXPLOSION_FADE_DURATION_MS = RADIAL_DAMAGE_VISUAL_TIMING_MS.fade;
const SHOT_EXPLOSION_DURATION_MS = SHOT_EXPLOSION_EXPANSION_DURATION_MS + SHOT_EXPLOSION_FADE_DURATION_MS;
...
function RuntimeViewerCameraControls({ activeView, infantryDistanceM, disabled, onView, onInfantryDistance, onFree, }: {
    ...
    return (<div className="viewer-camera-presets" aria-label="载具相机快捷预览">
      <div className="viewer-camera-presets__row">
        <span><b>五向视角</b><small>点击切换</small></span>
        <div role="group" aria-label="切换载具五向视角，不含底部视角">
          {RUNTIME_VIEWER_CAMERA_VIEWS.map((view) => (<button type="button" key={view.id} data-active={activeView === view.id} aria-pressed={activeView === view.id} disabled={disabled} title={view.kind === "soldier-ground"
                ? `${view.label}视图 · 标准步兵站姿眼高`
                : `${view.label}视图`} onClick={() => onView(view.id)}>
      ...
      <div className="viewer-camera-presets__row">
        ...
        <div role="group" aria-label="按普通步兵透视预览载具距离">
          {RUNTIME_VIEWER_INFANTRY_DISTANCES_M.map((distanceM) => (<button type="button" key={distanceM} data-active={infantryDistanceM === distanceM} aria-pressed={infantryDistanceM === distanceM} disabled={disabled} title={`以普通步兵视野在 ${distanceM} 米观察载具`} onClick={() => onInfantryDistance(distanceM)}>{distanceM}<small>m</small></button>))}
          <button type="button" data-active={activeView === null && infantryDistanceM === null} aria-pressed={activeView === null && infantryDistanceM === null} disabled={disabled} title="退出固定方向与距离，保留当前相机进入自由旋转视角" onClick={onFree}>自由</button>
        </div>
      </div>
    </div>);
}
...
function createCrewViewpointMarker(): CrewViewpointMarker {
    ...
    const lens = context.createRadialGradient(centerX - 9, centerY - 11, 3, centerX, centerY, 36);
...
function referenceTurretFromStationControl(station: RuntimeVisualAttachmentStation | null): ReferenceTurret | null {
    const control = station?.control;
    if (control?.state !== "observed" ||
        control.source !==
            "paired-sq-rotating-movement-components-native-layout" ||
        control.sourceFunction !== "runtime" ||
...
function runtimeTurretParentStation(station: RuntimeTurretPreviewStation, stations: RuntimeTurretPreviewStation[]) {
    if (station.parentCatalogSeatIndex !== null) {
        return stations.find((candidate) => candidate.seat.index === station.parentCatalogSeatIndex) ?? null;
    }
    ...
    return stations
        ...
        .sort((left, right) => (left.assembly?.yawPlacementIds.length ?? Number.MAX_SAFE_INTEGER) -
        (right.assembly?.yawPlacementIds.length ?? Number.MAX_SAFE_INTEGER))[0] ?? null;
}
function runtimeTurretStationDepth(station: RuntimeTurretPreviewStation, stations: RuntimeTurretPreviewStation[], visiting = new Set<string>()): number {
...
function runtimeTurretWorldYaw(station: RuntimeTurretPreviewStation, stations: RuntimeTurretPreviewStation[], poseStates: Readonly<Record<string, RuntimeTurretPoseState>>, visiting = new Set<string>()): number {
    ...
    return normalizeTurretYaw(runtimeTurretWorldYaw(parent, stations, poseStates, new Set(visiting).add(station.id)) + ownYaw);
}
function orderedRuntimeTurretStations(stations: RuntimeTurretPreviewStation[]) {
...
type RuntimePointerOutline = "damage" | "component-damage-no-vehicle" | "blocked-absolute" | "blocked-effective" | "penetrated-no-damage" | "unknown";
...
function runtimeWeaponSourceIdentity(source: RuntimeAttackSource): RuntimeWeaponSourceIdentity {
    const vehicleSource = source.sourceCategory === "vehicle";
    const supportSource = source.sourceCategory === "emplaced" ||
...
function runtimeWeaponSourceGroupRank(option: RuntimeWeaponSourceOption) {
    if (option.category === "vehicle")
        return 2;
    if (option.category === "emplaced" ||
        option.category === "commander-support") {
        return 1;
    }
    return 0;
}
function runtimeWeaponSourceOptions(options: readonly RuntimeWeaponOption[]) {
    ...
    return [...sourceOptions.values()].sort((left, right) => {
        const groupRankDifference = runtimeWeaponSourceGroupRank(left) -
...
function runtimeWeaponSourceOptionForWeapon(sourceOptions: readonly RuntimeWeaponSourceOption[], option: RuntimeWeaponOption | undefined) {
    if (!option)
        return null;
    return sourceOptions.find((sourceOption) => sourceOption.id === option.source.id) ?? null;
}
function RuntimeWeaponEffectLegend({ effects, }: {
    effects: readonly SearchableSelectEffect[];
}) {
    const penetrationEffects = effects.filter(({ role }) => role === "penetration");
    const directDamageEffects = effects.filter(({ role }) => role === "direct-damage");
    const radialEffects = effects.filter(({ role }) => role === "radial-damage");
    const renderEffect = (effect: SearchableSelectEffect) => {
        const effectLabel = effect.role === "penetration"
            ...
            : effect.role === "radial-damage"
                ...
                : null;
        return (<span className="infantry-weapon-effect-chip" data-damage-type-kind={effect.damageTypeKind} data-effect-role={effect.role} data-event-index={effect.eventIndex} data-zero={effect.value <= 0} title={effect.title} aria-label={effect.title} key={effect.id}>
        {effectLabel ? (<span className="infantry-weapon-effect-chip__label">
            {effectLabel}
          </span>) : null}
        <b>{metricText(effect.value)}</b>
      </span>);
    };
    return (<span className="infantry-weapon-effect-legend" data-density="compact" data-term="effects">
      <span className="infantry-weapon-effect-legend__column infantry-weapon-effect-legend__column--penetration" data-empty={penetrationEffects.length === 0}>
        {penetrationEffects.map(renderEffect)}
      </span>
      <span className="infantry-weapon-effect-legend__column infantry-weapon-effect-legend__column--direct" data-empty={directDamageEffects.length === 0}>
        {directDamageEffects.map(renderEffect)}
      </span>
      <span className="infantry-weapon-effect-legend__column infantry-weapon-effect-legend__column--radial" data-empty={radialEffects.length === 0}>
        {radialEffects.map(renderEffect)}
      </span>
    </span>);
}
function RuntimeWeaponSourceSelector({ value, options, onChange, onRequestGlobalLibrary, globalLibraryState, onOpenChange, }: {
    value: string;
    ...
    const matchedOptions = options.filter((option) => normalizedQuery
        ? normalizeWeaponQuery(`${option.group} ${option.label}`).includes(normalizedQuery)
        : true);
    ...
    const openSelector = () => {
        ...
        if (globalLibraryState === "idle" ||
            globalLibraryState === "error") {
            onRequestGlobalLibrary();
        }
        changeOpen(true);
    };
    return (<div className="viewer-search-select infantry-weapon-source-select" data-open={open} ref={rootRef}>
      <button className="viewer-search-select__trigger" type="button" aria-label="选择伤害来源" aria-haspopup="listbox" aria-expanded={open} onClick={openSelector}>
        <span className="viewer-search-select__value" title={selected?.label}>
          {selected?.menuLabel ?? "选择伤害来源"}
        </span>
        <small className="infantry-weapon-source-select__count">
          {selected?.count ?? 0} 弹种
        </small>
        <span className="viewer-search-select__chevron" aria-hidden="true">
          ⌄
        </span>
      </button>
      {open ? (<div className="viewer-search-select__menu">
          <div className="viewer-search-select__search">
            <input ref={searchRef} type="search" value={query} placeholder="搜索伤害来源" aria-label="搜索伤害来源" onChange={(event) => setQuery(event.currentTarget.value)} onKeyDown={(event) => {
                if (event.key !== "Enter")
                    return;
            ...
            {query ? (<button className="viewer-search-select__clear" type="button" aria-label="清除伤害来源搜索关键词" onClick={() => {
                    setQuery("");
          ...
          <div className="viewer-search-select__options" role="listbox" aria-label="伤害来源列表">
            {groupedOptions.map(({ group, options: groupOptions }) => (<div className="viewer-search-select__group" role="group" aria-label={group} key={group}>
                  <strong>{group}</strong>
                  {groupOptions.map((option) => (<button type="button" role="option" aria-selected={option.id === value} data-selected={option.id === value} key={option.id} onClick={() => choose(option.id)}>
                      <span>{option.menuLabel}</span>
                      <small>{option.count} 弹种</small>
                    </button>))}
                </div>))}
            {groupedOptions.length === 0 ? (<span className="viewer-search-select__empty">
                没有匹配项
              </span>) : null}
          </div>
        </div>) : null}
...
function RuntimeWeaponSelector({ value, options, targetDistanceM, onChange, onRequestGlobalLibrary, globalLibraryState, onOpenChange, onSourceOpenChange, }: {
    value: string;
    ...
    const sourceOptions = useMemo(() => runtimeWeaponSourceOptions(options), [options]);
    const selected = options.find((option) => option.value === value) ?? options[0];
    const selectedWeaponSource = runtimeWeaponSourceOptionForWeapon(sourceOptions, selected);
    const selectedWeaponSourceId = selectedWeaponSource?.id ?? "";
    const [selectedSourceId, setSelectedSourceId] = useState(selectedWeaponSourceId);
    const selectedSource = sourceOptions.find((option) => option.id === selectedSourceId) ?? null;
    const sourceFilteredOptions = useMemo(() => selectedSource
        ? options.filter((option) => option.source.id === selectedSource.id)
        : options, [options, selectedSource]);
    const normalizedQuery = normalizeWeaponQuery(query);
    const showAllSources = !selectedSource || Boolean(normalizedQuery);
    const matchedOptions = (normalizedQuery ? options : sourceFilteredOptions).filter((option) => normalizedQuery
        ? normalizeWeaponQuery(`${option.group} ${option.weaponLabel} ${option.label} ${option.provenanceLabels.join(" ")} ${option.searchText}`).includes(normalizedQuery)
        : true);
    const hiddenCount = Math.max(0, matchedOptions.length - WEAPON_SELECT_VISIBLE_LIMIT);
    const visibleOptions = matchedOptions.slice(0, WEAPON_SELECT_VISIBLE_LIMIT);
    const groupedOptions = showAllSources
        ? [...new Set(visibleOptions.map(({ familyId }) => familyId))]
            .map((familyId) => ({
            familyId,
            group: visibleOptions.find((option) => option.familyId === familyId)
                ?.group ?? familyId,
            options: visibleOptions.filter((option) => option.familyId === familyId),
        }))
    ...
    const chooseSource = (nextSourceId: string) => {
        ...
        if (!nextSourceId)
            return;
        const nextSource = sourceOptions.find((option) => option.id === nextSourceId);
        if (!nextSource)
            return;
        const currentMatchesSource = selected.source.id === nextSource.id;
        if (currentMatchesSource)
            return;
        const firstMatch = options.find((option) => option.source.id === nextSource.id);
        if (firstMatch)
            onChange(firstMatch.value);
    };
    const choose = (option: RuntimeWeaponOption) => {
        const nextSource = runtimeWeaponSourceOptionForWeapon(sourceOptions, option);
    ...
    const metrics = (option: RuntimeWeaponOption) => {
        ...
        const effectsLabel = effects.length > 0
            ...
            : "无已解析伤害或穿透数据";
        return (<span className="infantry-weapon-select__metrics" aria-label={effectsLabel}>
    ...
    return (<>
      <div className="viewer-attacker-control">
        ...
        <RuntimeWeaponSourceSelector value={selectedSourceId} options={sourceOptions} onChange={chooseSource} onRequestGlobalLibrary={onRequestGlobalLibrary} globalLibraryState={globalLibraryState} onOpenChange={onSourceOpenChange}/>
      </div>
      <div className="viewer-weapon-control">
        ...
        <div className="viewer-search-select infantry-weapon-select" data-open={open} data-source-filtered={!showAllSources} ref={rootRef}>
          <button className="viewer-search-select__trigger" type="button" aria-label="选择武器或弹种" aria-haspopup="listbox" aria-expanded={open} onClick={openSelector}>
          ...
          {open ? (<div className="viewer-search-select__menu">
              <div className="viewer-search-select__search">
                <input ref={searchRef} type="search" value={query} placeholder="搜索全部武器或弹种" aria-label="搜索全部武器或弹种" onChange={(event) => setQuery(event.currentTarget.value)} onKeyDown={(event) => {
                if (event.key !== "Enter" ||
                ...
                {query ? (<button className="viewer-search-select__clear" type="button" aria-label="清除武器搜索关键词" onClick={() => {
                    setQuery("");
                    searchRef.current?.focus();
                }}>
                    ×
              ...
              <div className="infantry-weapon-select__columns" aria-label="武器列表列标题">
                <span>武器 / 弹种</span>
              ...
              <div className="infantry-weapon-select__option-scroll">
                <div className="viewer-search-select__options" role="listbox" aria-label="武器与弹种列表">
                  {groupedOptions.length > 0 ? (<>
                      {groupedOptions.map(({ familyId, group, options: groupOptions }) => (<div className="viewer-search-select__group" role="group" aria-label={group} key={familyId}>
                            {showAllSources ? <strong>{group}</strong> : null}
                            {groupOptions.map((option) => (<button type="button" role="option" aria-selected={option.value === value} data-selected={option.value === value} key={option.value} onClick={() => choose(option)}>
                                <span>
                                  {showAllSources
                            ? option.label
                            : option.weaponLabel}
                      ...
                      {hiddenCount > 0 ? (<span className="viewer-search-select__empty">
...
type ShotPathMarkerKind = "spaced-armor" | "no-penetration" | "engine" | "ammo-rack" | "component-only-damage" | "gun-collision" | "other" | "penetrated" | "blocked";
...
function resolveShotPathMarkerStyle(kind: ShotPathMarkerKind): ShotPathMarkerStyle {
    ...
    const read = (property: string, fallback: string) => styles.getPropertyValue(`--hit-marker-${kind}-${property}`).trim() || fallback;
    ...
    return {
        ...
        dashed: read("border-style", SHOT_PATH_MARKER_STYLE_FALLBACK.dashed ? "dashed" : "solid") === "dashed",
    };
}
function opaqueShotPathMarkerColor(color: string) {
    const eightDigitHex = color.match(/^#([0-9a-f]{6})[0-9a-f]{2}$/iu);
    if (eightDigitHex)
        return `#${eightDigitHex[1]}`;
    const fourDigitHex = color.match(/^#([0-9a-f]{3})[0-9a-f]$/iu);
    if (fourDigitHex)
        return `#${fourDigitHex[1]}`;
    const rgba = color.match(/^rgba\(\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*[^)]+\)$/u);
    return rgba ? `rgb(${rgba[1]}, ${rgba[2]}, ${rgba[3]})` : color;
}
function editorPathMarkerKind(hitHeader: ParsedRuntimeHitScene["header"] | null, component: ParsedRuntimeHitScene["header"]["components"][number] | undefined, profile: ParsedRuntimeHitScene["header"]["surfaceProfiles"][number] | undefined, penetrated: boolean | null): ShotPathMarkerKind {
    ...
    if (component?.semanticKind === "gun-collision")
        return "gun-collision";
    if (component?.semanticKind === "engine")
        return "engine";
    if (component?.semanticKind === "ammo-rack")
        return "ammo-rack";
    if (hitHeader &&
        component &&
        profile &&
    ...
    if (component?.semanticKind === "other")
        return "other";
...
function distanceTicks(maxDistanceM: number) {
    ...
    const ticks = [0];
    for (let value = step; value < maxDistanceM; value += step)
        ticks.push(value);
...
function paintProtectionMap(canvas: HTMLCanvasElement, cells: Uint8Array, width: number, height: number) {
    ...
    cells.forEach((code, index) => {
        ...
        if (code === 1)
            image.data.set([67, 255, 120, 107], offset);
        else if (code === 2) {
...
function protectionMapSampleCount(mask: Uint8Array) {
    let count = 0;
    for (const sampled of mask)
        count += sampled === 0 ? 0 : 1;
    return count;
}
function seedSuperProtectionMap(standard: ProtectionMapGridCache, superGrid: ProtectionMapGridCache) {
    for (let row = 0; row < superGrid.height; row += 1) {
        const sourceRow = Math.min(standard.height - 1, Math.floor((row / superGrid.height) * standard.height));
        for (let column = 0; column < superGrid.width; column += 1) {
            const sourceColumn = Math.min(standard.width - 1, Math.floor((column / superGrid.width) * standard.width));
...
function runtimeProtectionMapSuperSampleOrder(width: number, height: number) {
    ...
    let index = 0;
    for (let level = RUNTIME_PROTECTION_MAP_MIN_PRECISION; level <= RUNTIME_PROTECTION_MAP_STANDARD_MAX_PRECISION; level += 1) {
        for (const [columnOffset, rowOffset] of runtimeProtectionMapLevelOffsets(level as RuntimeProtectionMapStandardPrecision)) {
            for (let row = rowOffset; row < height; row += RUNTIME_PROTECTION_MAP_BLOCK_SIZE) {
                for (let column = columnOffset; column < width; column += RUNTIME_PROTECTION_MAP_BLOCK_SIZE) {
...
interface ShotExplosionLayerVisual {
    root: THREE.Group;
    pressureSurface: THREE.Mesh<THREE.SphereGeometry, ShotExplosionPressureSurfaceMaterial>;
    groundArea: THREE.Mesh<THREE.CircleGeometry, THREE.MeshBasicMaterial>;
    exactRadiusRing: THREE.LineLoop<THREE.BufferGeometry, THREE.LineBasicMaterial>;
...
interface ExplosionPlacementPreview {
    ...
    exactRadiusRings: THREE.LineLoop<THREE.BufferGeometry, THREE.LineBasicMaterial>[];
...
interface ShotVisualRuntime {
    ...
    continuationArrow: THREE.LineSegments<THREE.BufferGeometry, THREE.LineDashedMaterial>;
...
function settledShotExplosionDamageHighlight(record: RuntimeShotRecord) {
    const settledLayers = record.visual.explosionLayers.filter((layer) => layer.configured && layer.settledComponentIndices.length > 0);
    if (settledLayers.length === 0)
        return null;
    return {
        componentIndices: [...new Set(settledLayers.flatMap((layer) => layer.settledComponentIndices))],
        colorHex: settledLayers[0].pressureSurface.material.uniforms.uColor.value.getHex(),
...
function createRuntimeGroundScaleNumberLabel(valueM: number, labelHeightM: number, rotationZRad: number, includeUnit: boolean) {
    ...
    const label = new THREE.Mesh(new THREE.PlaneGeometry(labelHeightM * (labelCanvas.width / labelCanvas.height), labelHeightM), new THREE.MeshBasicMaterial({
        map: labelTexture,
        transparent: true,
        opacity: 0.96,
        depthTest: false,
...
function createRuntimeGroundScaleAxis(lengthM: number, axisName: "length" | "width", tickSide: 1 | -1) {
    ...
    const tickCount = Math.round(lengthM / RUNTIME_GROUND_SCALE_TICK_INTERVAL_M);
    const baseline = new THREE.Mesh(new THREE.BoxGeometry(lengthM, thicknessM * 0.55, thicknessM), new THREE.MeshBasicMaterial({
        color: 0xd3b979,
        transparent: true,
        opacity: 0.7,
        depthTest: false,
    ...
    for (let tickIndex = 0; tickIndex <= tickCount; tickIndex += 1) {
        ...
        const isMetreMark = valueM % RUNTIME_GROUND_SCALE_LABEL_INTERVAL_M === 0;
        const tickDepthM = isMetreMark ? majorTickDepthM : minorTickDepthM;
        const tick = new THREE.Mesh(new THREE.BoxGeometry(thicknessM * (isMetreMark ? 1.05 : 0.72), thicknessM * 0.75, tickDepthM), new THREE.MeshBasicMaterial({
        ...
        if (!isMetreMark)
            continue;
        const label = createRuntimeGroundScaleNumberLabel(valueM, labelHeightM, axisName === "length" ? -Math.PI / 2 : 0, tickIndex === 0 || tickIndex === tickCount);
        label.position.set(valueM, thicknessM * 1.35, tickSide * (majorTickDepthM + labelHeightM * 0.62));
        group.add(label);
    }
    return group;
}
function createRuntimeGroundScale(lengthM: number, widthM: number) {
    const group = new THREE.Group();
    ...
    group.add(widthAxis);
    const origin = new THREE.Mesh(new THREE.RingGeometry(0.026, 0.038, 20), new THREE.MeshBasicMaterial({
        color: 0xb9c9cf,
        side: THREE.DoubleSide,
        transparent: true,
        opacity: 0.5,
...
function shotTerminalDistanceFromFirstHitM(result: EditorNativeShotResult) {
    ...
    return editorNativeTraceTerminalDistanceM({
        ...
        stoppedDistanceFromFirstHitM: stoppedLayer?.distanceFromFirstHitM ?? null,
    });
}
...
function createAnalysisVisualMaterial(stableSurface = false, presentation: AnalysisVisualPresentation = "analysis") {
...
function replaceAnalysisVisualMaterial(mesh: THREE.Mesh, stableSurface: boolean, disposePrevious = false, presentation: AnalysisVisualPresentation = "analysis") {
    const previousMaterials = Array.isArray(mesh.material)
        ? mesh.material
        : [mesh.material];
    mesh.material = Array.isArray(mesh.material)
        ? previousMaterials.map(() => createAnalysisVisualMaterial(stableSurface, presentation))
        : createAnalysisVisualMaterial(stableSurface, presentation);
    if (disposePrevious) {
        previousMaterials.forEach((material) => material.dispose());
    }
}
function setAnalysisVisualPresentation(group: THREE.Group, presentation: AnalysisVisualPresentation) {
    if (group.userData.analysisVisualPresentation === presentation)
        return;
    group.traverse((object) => {
        ...
        replaceAnalysisVisualMaterial(object, object.userData.analysisVisualStableSurface === true, true, presentation);
    });
...
function analysisVisualGeometryScore(source: THREE.Object3D) {
    ...
    return Math.max(size.x * size.y * size.z, extent ** 3 * 1e-9);
}
...
function defaultAttackWeaponOptionIndex(source: RuntimeAttackSource) {
    const armorPiercing = source.weapons.findIndex(({ gunName, runtimeAssetPath }) => /(?:_AP(?:_|$)|APFSDS|Armor-Piercing|Sabot)/iu.test(`${gunName} ${runtimeAssetPath ?? ""}`));
    return armorPiercing >= 0 ? armorPiercing : 0;
}
function runtimeAttackSourceMatchesId(source: RuntimeAttackSource, id: string) {
...
function parseWeaponSelectionValue(value: string) {
    ...
    const optionIndex = Number(value.slice(separatorIndex + WEAPON_SELECTION_SEPARATOR.length));
...
function effectiveDamageEventsByKind(result: EditorNativeShotResult, damageKind: EditorNativeDamageEvent["damageKind"]) {
    return result.damage.filter((damage) => damage.damageKind === damageKind
        && editorNativeEffectiveDamageAmount(damage) > 0);
}
function groupDamageEventsByVisibleLayer(layers: readonly EditorNativeShotResult["layers"][number][], events: readonly EditorNativeDamageEvent[]) {
...
function summarizeDamageOutcomes(events: readonly EditorNativeDamageEvent[]): DamageOutcomeSummary[] {
    const summaries = new Map<string, DamageOutcomeSummary & {
        poolDamage: number;
    }>();
    events.forEach((event) => {
        ...
        if (existing) {
            ...
            existing.effect = editorDamageCardEffect(existing.poolKind, existing.poolDamage, existing.maxHealth);
            return;
        }
        summaries.set(key, {
            key,
...
function DamageSettlementListItems({ events, animationKey, penetrationKind, }: {
    ...
    return summarizeEditorDamageSettlements(events).map((settlement) => {
        ...
        if (hasForwardedDamage) {
            return (<div key={`${animationKey}:${settlement.key}:forwarded`} className="viewer-causal-spine__settlement viewer-causal-spine__settlement--forwarded" data-damage-kind={settlement.damageKind} data-damage-type-kind={settlementColorKind} data-damage-forwarded="true" style={{
          ...
          <section className="viewer-causal-spine__forwarding-targets" aria-label={settlement.targets.map((target) => `${editorPoolLabel(target.poolKind)}受到 ${metricText(target.effectiveDamage)} 伤害`).join("，随后")}>
            {settlement.targets.map((target, index) => (<span key={`${target.poolId}:${target.forwarded ? "forwarded" : "direct"}`}>
        ...
        return (<div key={`${animationKey}:${settlement.key}`} className="viewer-causal-spine__settlement" data-damage-kind={settlement.damageKind} data-damage-type-kind={settlementColorKind} data-damage-forwarded={hasForwardedDamage ? "true" : undefined} style={{
                "--spine-accent": vehicleDamageTypeIconColor(settlementColorKind),
            } as CSSProperties}>
        <strong>{settlement.damageKind === "radial" ? `${typeLabel}爆炸结算` : `${typeLabel}直击结算`}</strong>
        <span aria-label={`${settlement.damageKind === "radial" ? "爆炸" : "直击"}伤害 ${metricText(settlement.incomingDamage)}，伤害类型系数 ${damageModifierText(settlement.damageTypeModifier)}${settlement.damageKind === "radial" ? `，爆炸系数 ${damageModifierText(settlement.routeMultiplier)}，派发 ${settlement.dispatchCount} 次` : ""}，合计生效 ${metricText(settlement.effectiveDamage)}`}>
          {metricText(settlement.incomingDamage)} × {damageModifierText(settlement.damageTypeModifier)}
...
function HitDpsFold({ resultLabel, secondaryLabel, state, children, }: {
    resultLabel: string;
    secondaryLabel?: string | null;
    state: "loading" | "ready" | "unavailable";
    children: ReactNode;
}) {
    return (<details className="viewer-hit-dps-fold" data-state={state}>
...
function hitDpsTimingFacts(target: WeaponHitDpsTarget, simulation: WeaponDpsSimulation, weapon: WeaponDpsWeapon | null): HitDpsTimingFact[] {
    ...
    if (simulation.reloads > 0 &&
        ...
        facts.push({ label: "装填 / 再发", value: `${reload.toFixed(2)} / ${interval.toFixed(2)} s` }, { label: "计时", value: "同时" });
    }
...
function HitDpsFacts({ facts }: {
    facts: readonly HitDpsTimingFact[];
}) {
    return (<dl className="viewer-hit-dps-timing__facts" data-count={facts.length}>
      {facts.map((fact) => (<div key={`${fact.label}:${fact.value}`}>
          <dt>{fact.label}</dt>
          <dd>{fact.value}</dd>
        </div>))}
    </dl>);
...
function hitDpsEstimateTimeLabel(estimate: WeaponHitDpsEstimate) {
    ...
    if (candidate.result.ammoExhausted)
        return "弹药耗尽";
...
function HitDpsTimingCard({ estimates, targets, weapon, factsState, factsUnavailableReason, clickedSemanticKind, }: {
    ...
    const directOneShotTarget = singleShotWeaponHitTarget(targets, clickedSemanticKind);
    if (directOneShotTarget && factsState === "unavailable") {
        ...
        const resultLabel = directOneShotTarget.poolKind === "hull"
            ? "单发摧毁"
            : `单发打坏${poolLabel}`;
        return (<HitDpsFold resultLabel={resultLabel} state="ready">
        <HitDpsFacts facts={[
    ...
    if (factsState === "loading") {
        return (<HitDpsFold resultLabel="计算中" state="loading">
        <p className="viewer-hit-dps-timing__reason">正在载入当前武器的射速、换弹与过热事实…</p>
      </HitDpsFold>);
    }
    if (factsState === "unavailable") {
        return (<HitDpsFold resultLabel="暂无DPS数据" state="unavailable">
    ...
    const primaryIsOneShot = primarySimulation.shots === 1 && primarySimulation.killTimeSeconds === 0;
    ...
    const marginalMathGain = !primaryEstimate.optimization.practical.meaningful &&
    ...
    const timelineEstimate = primarySimulation.damageCurve.length > 1
        ? primaryEstimate
        : secondaryEstimates.find((estimate) => hitDpsEstimateCandidate(estimate).result.damageCurve.length > 1) ?? primaryEstimate;
    ...
    if (primarySimulation.thermalState === "unavailable") {
        return (<HitDpsFold resultLabel={primaryResultLabel} secondaryLabel={secondarySummary || null} state="ready">
        <section className="viewer-hit-dps-timing viewer-hit-dps-timing--damage-only" data-state="ready" data-thermal-state="unavailable">
          <HitDpsFacts facts={[
                ...hitDpsTimingFacts(primaryEstimate, primarySimulation, weapon),
                ...secondaryFacts,
            ]}/>
          {timelineSimulation.damageCurve.length > 1 ? (<WeaponRhythmTimeline simulation={timelineSimulation} targetHealth={timelineEstimate.maxHealth} targetLabel={timelineTargetLabel} compact/>) : null}
        </section>
      </HitDpsFold>);
    }
    return (<HitDpsFold resultLabel={primaryResultLabel} secondaryLabel={secondarySummary || null} state="ready">
      <section className="viewer-hit-dps-timing" data-state="ready" aria-label="当前点击位置的自动击毁时间">
        ...
        <dl>
          ...
          {primarySimulation.burnDamage > 0 ? (<div><dt>正常自燃</dt><dd>{metricText(primarySimulation.burnDamage)}</dd></div>) : null}
        </dl>
        </div>
        <p className="viewer-hit-dps-timing__rhythm">
        <b>实战节奏</b>
        <span>{primaryPlanLabel}</span>
        {marginalMathGain === null ? null : (<small>数学最优只快 {marginalMathGain.toFixed(2)} s，不值得精确卡点</small>)}
        </p>
        {secondaryEstimates.length === 0 ? null : (<ul className="viewer-hit-dps-timing__secondary" aria-label="同次命中的其他目标">
        ...
        <WeaponRhythmTimeline simulation={timelineSimulation} targetHealth={timelineEstimate.maxHealth} targetLabel={timelineTargetLabel} compact/>
      </section>
    </HitDpsFold>);
}
function paintShotPathMarker(marker: ShotPathMarkerVisual, number: number, kind: ShotPathMarkerKind) {
...
function createShotPathMarker(number: number, renderOrder: number): ShotPathMarkerVisual {
    ...
    const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.064, 16, 12), new THREE.MeshStandardMaterial({
...
function shotTraceMarkerKind(result: EditorNativeShotResult, hitScene: ParsedRuntimeHitScene | null): ShotPathMarkerKind {
    const effectiveDamage = result.damage.filter((damage) => editorNativeEffectiveDamageAmount(damage) > 0);
    if (effectiveDamage.some((damage) => damage.poolKind === "ammo-rack"))
        return "ammo-rack";
    if (effectiveDamage.some((damage) => damage.poolKind === "engine"))
        return "engine";
    if (effectiveDamage.some(isEditorNativeComponentOnlyDamageEvent)) {
        return "component-only-damage";
    }
    if (effectiveDamage.some(isEditorNativeVehicleDamageEvent))
        return "penetrated";
    if (result.stoppedAtLayer !== null) {
        const layer = result.layers[result.stoppedAtLayer] ?? result.layers.at(-1);
        if (layer) {
            return editorPathMarkerKind(hitScene?.header ?? null, hitScene?.header.components[layer.componentIndex], hitScene?.header.surfaceProfiles[layer.surfaceProfileIndex], layer.penetrated);
        }
        return "blocked";
    }
...
function paintShotExplosionDamageTypeIcon(visual: ShotExplosionLayerVisual, kind: VehicleDamageTypeIconKind, color: number) {
    ...
    context.quadraticCurveTo(canvas.width - 4, canvas.height - 4, canvas.width - 18, canvas.height - 4);
...
function paintShotExplosionOriginLabel(visual: ShotExplosionLayerVisual, componentLabel: string, originOffsetM: number) {
    ...
    context.textBaseline = "middle";
    context.fillText(`爆心${Math.abs(originOffsetM) > 0.0001 ? ` · 法线偏移 ${(originOffsetM * 100).toFixed(0)} cm` : ""}`, 14, 28);
    context.fillStyle = "#f4f7f5";
    context.font = "600 18px system-ui, sans-serif";
    context.fillText(`命中组件：${componentLabel}`, 14, 62);
    visual.originLabelTexture.needsUpdate = true;
}
function paintShotExplosionGroundHeight(visual: ShotExplosionLayerVisual, heightM: number) {
    const label = `${heightM.toFixed(1)} m`;
    if (visual.groundHeightLabel.userData.heightLabel === label)
        return;
...
function updateShotExplosionDamageTypeIconPosition(visual: ShotExplosionLayerVisual, camera: THREE.Camera) {
    if (!visual.configured || !visual.root.visible)
        return;
    const radiusM = visual.outerRadiusM;
    if (radiusM <= 0.0001)
        return;
    const rootWorldPosition = visual.root.getWorldPosition(new THREE.Vector3());
    const rootWorldQuaternion = visual.root.getWorldQuaternion(new THREE.Quaternion());
    ...
    visual.pressureSurface.material.uniforms.uCameraDirectionLocal.value.copy(localCameraDirection);
    ...
    const iconDirection = ringCameraDirection.negate().applyAxisAngle(new THREE.Vector3(0, 1, 0), visual.iconAngleOffsetRad);
    ...
    const iconWidthM = Math.max(0.42, cameraWorldPosition.distanceTo(iconWorldPosition) * 0.036);
    visual.damageTypeIcon.scale.set(iconWidthM, iconWidthM * 0.7, 1);
    const handleScale = Math.max(0.9, Math.min(2.6, cameraWorldPosition.distanceTo(rootWorldPosition) * 0.018));
...
function createShotExplosionLayerVisual(traceIndex: number, layerIndex: number): ShotExplosionLayerVisual {
    ...
    const pressureSurface = new THREE.Mesh(new THREE.SphereGeometry(1, 48, 32), createShotExplosionPressureSurfaceMaterial());
    ...
    const groundArea = new THREE.Mesh(new THREE.CircleGeometry(1, 128), new THREE.MeshBasicMaterial({
        color: 0xffa12b,
        side: THREE.DoubleSide,
        transparent: true,
        opacity: 0,
    ...
    const exactRadiusRing = new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints(exactRadiusRingPoints), new THREE.LineBasicMaterial({
        color: 0xffd166,
        transparent: true,
        opacity: 0,
        depthTest: true,
    ...
    const originTether = new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(),
        new THREE.Vector3(),
    ...
    const impactAnchor = new THREE.Mesh(new THREE.SphereGeometry(0.075, 14, 10), new THREE.MeshBasicMaterial({
        color: 0xfff0c2,
        transparent: true,
        opacity: 0,
        depthTest: false,
    ...
    const groundHeightLabelTexture = new THREE.CanvasTexture(groundHeightLabelCanvas);
    ...
    const groundHeightLabel = new THREE.Sprite(new THREE.SpriteMaterial({
    ...
    const originLabel = new THREE.Sprite(new THREE.SpriteMaterial({
    ...
    const damageTypeIconTexture = new THREE.CanvasTexture(damageTypeIconCanvas);
    ...
    const damageTypeIcon = new THREE.Sprite(new THREE.SpriteMaterial({
    ...
    const dragHitArea = new THREE.Mesh(new THREE.SphereGeometry(0.46, 12, 8), new THREE.MeshBasicMaterial({
        transparent: true,
        opacity: 0.01,
        depthTest: false,
        depthWrite: false,
    ...
    paintShotExplosionDamageTypeIcon(visual, "generic", shotExplosionColor("generic"));
    paintShotExplosionOriginLabel(visual, "车辆部件", 0);
    paintShotExplosionGroundHeight(visual, 0);
    return visual;
}
function configureShotExplosionLayerVisual(visual: ShotExplosionLayerVisual, { origin, normal, color, damageTypeIconKind, outerRadiusM, innerRadiusM, delayMs, layerId, damageTypePath, settledComponentIndices, originComponentLabel, originOffsetM, }: {
    ...
    paintShotExplosionOriginLabel(visual, originComponentLabel, originOffsetM);
}
...
function settleShotExplosionLayerVisual(visual: ShotExplosionLayerVisual, selected: boolean) {
    if (!visual.configured || !selected) {
        visual.root.visible = false;
        return;
    }
    visual.root.visible = true;
    visual.pressureSurface.scale.setScalar(Math.max(0.001, visual.outerRadiusM));
    visual.groundArea.scale.setScalar(Math.max(0.001, visual.outerRadiusM));
    visual.groundArea.material.opacity = visual.showOriginLabel ? 0.055 : 0.026;
    visual.exactRadiusRing.scale.setScalar(Math.max(0.001, visual.outerRadiusM));
    visual.pressureSurface.material.uniforms.uOpacity.value = 0;
    visual.exactRadiusRing.material.opacity = 0.72;
    const offsetVisible = visual.showOriginLabel && Math.abs(visual.originOffsetM) > 0.025;
...
function setShotExplosionLayerAnimationFrame(visual: ShotExplosionLayerVisual, elapsedMs: number, selected: boolean) {
    ...
    const expansionProgress = THREE.MathUtils.clamp(localElapsedMs / SHOT_EXPLOSION_EXPANSION_DURATION_MS, 0, 1);
    const fadeProgress = THREE.MathUtils.clamp((localElapsedMs - SHOT_EXPLOSION_EXPANSION_DURATION_MS) / SHOT_EXPLOSION_FADE_DURATION_MS, 0, 1);
    const expansion = easeOutCubic(expansionProgress);
    const surfaceVisibility = 1 - THREE.MathUtils.smoothstep(fadeProgress, 0, 1);
    const ringReveal = THREE.MathUtils.smoothstep(fadeProgress, 0.08, 0.82);
    visual.root.visible = true;
    const currentRadiusM = Math.max(0.001, visual.outerRadiusM * expansion);
    ...
    const iconReveal = THREE.MathUtils.smoothstep(expansionProgress, 0.08, 0.28);
    const offsetVisible = visual.showOriginLabel && Math.abs(visual.originOffsetM) > 0.025;
    visual.originTether.visible = offsetVisible;
    ...
    visual.dragHandle.visible = visual.showOriginLabel;
    if (visual.settledComponentIndices.length === 0)
        return 0;
    const highlightRise = THREE.MathUtils.smoothstep(expansionProgress, 0.02, 0.28);
    const settledHighlightBlend = THREE.MathUtils.lerp(1, SHOT_EXPLOSION_SETTLED_HIGHLIGHT_STRENGTH, THREE.MathUtils.smoothstep(fadeProgress, 0.08, 1));
    return highlightRise * settledHighlightBlend;
}
function addExplosionOriginGlyph(group: THREE.Group, renderOrder: number, color = 0xffd67f) {
    const core = new THREE.Mesh(new THREE.SphereGeometry(0.09, 16, 10), new THREE.MeshBasicMaterial({
        color: 0xfff3cf,
        transparent: true,
        opacity: 0.96,
        depthTest: false,
    ...
    group.add(core);
    const halo = new THREE.Mesh(new THREE.SphereGeometry(0.18, 18, 12), new THREE.MeshBasicMaterial({
        color,
        side: THREE.DoubleSide,
        transparent: true,
        opacity: 0.16,
...
function createExplosionPlacementPreview(): ExplosionPlacementPreview {
    ...
    const areaDiscs = Array.from({ length: MAX_SHOT_EXPLOSION_LAYERS }, (_, layerIndex) => {
    ...
    const exactRadiusRings = Array.from({ length: MAX_SHOT_EXPLOSION_LAYERS }, (_, layerIndex) => {
        const ring = new THREE.LineLoop(new THREE.BufferGeometry().setFromPoints(circlePoints), new THREE.LineBasicMaterial({
            color: layerColors[layerIndex],
            transparent: true,
    ...
    const { core: originCore, halo: originHalo } = addExplosionOriginGlyph(originMarker, root.renderOrder + 10);
...
function setCylinderBetween(mesh: THREE.Mesh<THREE.CylinderGeometry, THREE.MeshBasicMaterial>, start: THREE.Vector3, end: THREE.Vector3, radiusM: number) {
...
function setShotTraceAnimationProgress(shotVisual: ShotVisualRuntime, solidProgress: number, continuationProgress: number) {
    const layout = shotVisual.animationLayout;
    if (!layout)
        return;
    const clampedSolid = THREE.MathUtils.clamp(solidProgress, 0, 1);
    const clampedContinuation = THREE.MathUtils.clamp(continuationProgress, 0, 1);
    const animatedTraceEnd = layout.traceStart.clone().lerp(layout.traceEnd, clampedSolid);
    setCylinderBetween(shotVisual.trace, layout.traceStart, animatedTraceEnd, 0.012);
    setCylinderBetween(shotVisual.traceOutline, layout.traceStart, animatedTraceEnd, 0.024);
    ...
    if (hasContinuation && clampedContinuation > 0) {
        const animatedContinuationEnd = layout.continuationStart.clone().lerp(layout.continuationEnd, clampedContinuation);
...
function createShotVisual(traceIndex: number): ShotVisualRuntime {
    ...
    const trace = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 1, 8, 1, false), new THREE.MeshBasicMaterial({
    ...
    group.add(trace);
    const traceOutline = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 1, 8, 1, false), new THREE.MeshBasicMaterial({
    ...
    const continuationTrace = new THREE.Line(new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(),
    ...
    const continuationArrow = new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints([
    ...
    const entryMarker = new THREE.Mesh(new THREE.SphereGeometry(0.045, 12, 8), new THREE.MeshBasicMaterial({
        color: initialStyle.stroke,
        transparent: true,
        depthTest: false,
        depthWrite: false,
    ...
    const terminalArrowHead = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.12, 12), terminalMarkerMaterial);
    ...
    const terminalArrowShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.12, 8), terminalMarkerMaterial);
    ...
    const layerMarkers = Array.from({ length: MAX_VISIBLE_LAYERS }, (_, index) => createShotPathMarker(index + 1, 23 + traceIndex * 12 + index));
    layerMarkers.forEach((marker) => group.add(marker.sphere));
    const explosionLayers = Array.from({ length: MAX_SHOT_EXPLOSION_LAYERS }, (_, layerIndex) => createShotExplosionLayerVisual(traceIndex, layerIndex));
...
function referenceVehicleWeapons(referenceData: ReferenceData, equipmentResolver: RuntimeStationEquipmentResolver | null) {
    if (!equipmentResolver)
        return [];
    return referenceData.weaponBindingIds.map((bindingId) => {
        const binding = equipmentResolver(bindingId);
        if (!binding) {
            throw new Error(`Vehicle reference data points to missing weapon binding ${bindingId}`);
        }
        return binding.equipment;
    });
}
function turretStationEquipmentLabel(referenceData: ReferenceData, seat: ReferenceSeat, equipmentResolver: RuntimeStationEquipmentResolver | null) {
    const labels = [...new Set(referenceVehicleWeapons(referenceData, equipmentResolver)
            .filter((weapon) => weapon.turretName === seat.turretName)
            .map((weapon) => weapon.displayName)
            .filter(Boolean))];
    if (labels.length === 0)
        return seat.turretName ?? "炮塔";
    if (labels.length === 1)
        return labels[0];
...
export function RuntimeVehicleViewer({ preview, showChrome = true, mode: requestedMode = "exterior", displayName = preview.variantRawName, attackSourcePresentation, referenceData, onModeChange, onClose, navigationState, onNavigationStateChange, onExteriorStreamingChange, attackLibraryOverride, duelTarget = false, allowGlobalAttackSources = true, onDuelHitChange, displayOverrides, shotTraceLimit = MAX_SHOT_TRACES, }: {
    ...
    onExteriorStreamingChange?: (state: {
        loaded: number;
        total: number;
    } | null) => void;
    ...
    const maxShotTraces = Math.max(1, Math.min(MAX_SHOT_TRACES, Math.trunc(shotTraceLimit)));
    ...
    const resetViewRef = useRef<((options?: {
        preserveShotVisual?: boolean;
    }) => void) | null>(null);
    const applyCameraViewPresetRef = useRef<((viewId: RuntimeViewerCameraViewId) => void) | null>(null);
    const applyInfantryDistancePreviewRef = useRef<((distanceM: number) => void) | null>(null);
    const enterFreeCameraViewRef = useRef<(() => void) | null>(null);
    const enterCrewViewpointRef = useRef<((stationId: string) => boolean) | null>(null);
    ...
    const operationEquipmentRefsRef = useRef<readonly string[]>([]);
    const operationInventorySlotsRef = useRef<readonly OperationInventorySlot[]>([]);
    const [firingPresentation, setFiringPresentation] = useState<VehicleFiringPresentation | null>(null);
    const selectOperationEquipmentRef = useRef<(equipmentRef: string) => void>(() => undefined);
    const vehicleProjectileShotSequenceRef = useRef(0);
    const vehicleProjectileSourceShotCountsRef = useRef(new Map<string, number>());
    const vehicleProjectileMagazineStateRef = useRef<{
        weaponAssignmentId: string | null;
        shotsFiredInMagazine: number;
    }>({ weaponAssignmentId: null, shotsFiredInMagazine: 0 });
    const vehicleWeaponOperationStatesRef = useRef(new Map<string, VehicleWeaponOperationState>());
    const activeCrewViewZoomIndexRef = useRef(0);
    const activeCrewViewStationIdRef = useRef<string | null>(null);
    const flushOperationPoseCommitRef = useRef<(() => void) | null>(null);
    ...
    const applyDriverMaskVisibilityRef = useRef<((visible: boolean) => void) | null>(null);
    ...
    const applyCameraNavigationRef = useRef<((state: ViewerNavigationState | undefined) => void) | null>(null);
    ...
    const setShotExplosionOriginRef = useRef<(shotId: number, originM: [
        number,
        number,
        number
    ] | null) => void>(() => undefined);
    const saveExplosionOriginRef = useRef<(originM: [
        number,
        number,
        number
    ], rayOrigin: THREE.Vector3, rayDirection: THREE.Vector3) => RuntimeShotRecord | null>(() => null);
    ...
    const pendingSharedShotsRef = useRef(decodeSharedShotPaths(navigationState?.shots ?? ""));
    ...
    const protectionPrecisionRef = useRef<RuntimeProtectionMapPrecision>(RUNTIME_PROTECTION_MAP_MIN_PRECISION);
    const [equipmentResolver, setEquipmentResolver] = useState<RuntimeStationEquipmentResolver | null>(null);
    const scheduleProtectionMapRef = useRef<((options?: ProtectionMapScheduleOptions) => void) | null>(null);
    ...
    const driverMaskAvailable = driverView.mask.state === "observed-source-viewport-geometry";
    const crewOccupantPlan = useMemo(() => buildCrewOccupantPresentationPlan(preview.crewSeat), [preview.crewSeat]);
    const crewOccupantCounts = useMemo(() => ({
        total: crewOccupantPlan.length,
        rendered: crewOccupantPlan.filter(({ renderKind }) => renderKind !== "protected-nonspatial").length,
    ...
    const nonSpatialCrewOccupants = useMemo(() => crewOccupantPlan.filter(({ renderKind }) => renderKind === "protected-nonspatial"), [crewOccupantPlan]);
    ...
    const vehicleMeshSkeletalPoseEvidence = vehicleMeshRuntimePosePlacement
        ? runtimeSkeletalPoseEvidence({
            observedSampleCount: vehicleMeshRuntimePosePlacement.runtimeBonePoseNormalTimeSampleCount ??
                1,
            referenceEquivalent: vehicleMeshRuntimePosePlacement.runtimeBonePoseReferenceEquivalent ===
                true,
        })
        : null;
    const vehicleMeshObservedSuspensionPose = vehicleMeshRuntimePosePlacement
        ? runtimePlanarSuspensionPoseForVisualOccurrence(preview.suspension.records, preview.generatedClass, vehicleMeshRuntimePosePlacement.stableOccurrenceId)
        : null;
    const vehiclePlanarSuspensionCoverage = runtimePlanarSuspensionCoverageForGeneratedClass(preview.suspension.coverage, preview.generatedClass);
    ...
    const runtimeTurretStations = useMemo<RuntimeTurretPreviewStation[]>(() => {
        if (!referenceData || !visual)
            return [];
        const stationInputs = referenceData.seats.flatMap((seat) => {
            if (!seat.turretName)
                return [];
            const crewSeat = preview.crewSeat?.seats.find((candidate) => candidate.catalogSeatIndex === seat.index &&
            ...
            const visualAttachment = preview.visualAttachment?.stations.find((station) => station.catalogSeatIndex === seat.index &&
                station.turretName === seat.turretName) ?? null;
            const turret = seat.turret ??
                referenceTurretFromStationControl(visualAttachment);
            const view = crewSeat.views.find((candidate) => candidate.source === "seat-pawn-get-camera-component") ?? crewSeat.views.find((candidate) => candidate.viewId.endsWith("-default")) ?? crewSeat.views[0] ?? null;
            return turret
                ? [{ seat, turret, visualAttachment, crewSeat, view }]
                : [];
        });
        const primaryInput = stationInputs.find(({ seat }) => seat.role === "gunner" && seat.stationKind === "weapon-station") ?? stationInputs.find(({ seat }) => seat.role === "gunner") ??
            stationInputs[0];
        const dedupedPlacements = dedupeIdenticalVisualPlacements(visual.placements).placements;
        const allTurretNames = [
            ...new Set(stationInputs.map(({ seat }) => seat.turretName!)),
        ];
        const fallbackSpecs = new Map(allTurretNames.map((turretName) => [
            turretName,
        ...
        const stations = stationInputs.map((input) => {
            ...
            const indicatorKind: TurretPreviewIndicatorKind = input === primaryInput
                ? "main-turret"
                : seat.stationKind === "remote-weapon-station"
                    ...
                    : seat.role === "machine-gunner" ||
                        ...
                        ? "machine-gun"
                        : "weapon-station";
            const stationWeaponNames = referenceVehicleWeapons(referenceData, equipmentResolver)
                .filter((weapon) => weapon.turretName === seat.turretName)
            ...
            const assembly = resolveRuntimeTurretAssembly({
                ...
                absorbsSiblingStations: input === primaryInput && seat.role === "gunner",
                fallbackYawAnchorComponentName: fallbackSpecs.get(seat.turretName!)?.yawAnchorComponentName,
                fallbackYawAnchorActorName: fallbackSpecs.get(seat.turretName!)?.yawAnchorActorName,
                fallbackPitchUsesYawAnchor: fallbackSpecs.get(seat.turretName!)?.pitchUsesYawAnchor,
                fallbackHitActorClassNames: fallbackSpecs.get(seat.turretName!)?.hitActorClassNames,
            ...
            return {
                ...
                equipmentLabel: turretStationEquipmentLabel(referenceData, seat, equipmentResolver),
                turret,
                indicatorKind,
        ...
        const nestedAssemblies = carryNestedRuntimeTurretAssemblies(stations.map((station) => station.assembly), stations.map((station) => {
            if (station.parentCatalogSeatIndex === null)
                return null;
            const parentIndex = stations.findIndex((candidate) => candidate.seat.index === station.parentCatalogSeatIndex);
            if (parentIndex < 0) {
                throw new Error(`${preview.variantRawName} station F${station.seat.index} parent F${station.parentCatalogSeatIndex} is missing`);
            }
            return {
                parentIndex,
    ...
    const defaultTurretStation = preferredCrewViewStation(runtimeTurretStations) ??
    ...
    const clampedTurretYaw = activeTurretStation
        ? clampTurretYaw(activeTurretStation.turret, activeTurretPose.yawDegrees)
        : 0;
    ...
    const updateTurretStationPose = useCallback((station: RuntimeTurretPreviewStation, yawDegrees: number, pitchDegrees: number, options: {
        ...
        const nextPoseStates = {
            ...
            [station.id]: {
                ...
                pitchDegrees: clampTurretPitch(station.turret, clampedYaw, pitchDegrees),
            },
        };
    ...
    const commitTurretNavigation = useCallback((stationId: string, poseStates = turretPoseStatesRef.current) => {
        ...
        if (!current || !onNavigationStateChangeRef.current)
            return;
        const activeStationIndex = Math.max(0, runtimeTurretStations.findIndex((station) => station.id === stationId));
    ...
    useEffect(() => {
        if (!onExteriorStreamingChange)
            return;
        onExteriorStreamingChange(mode === "exterior" && viewerState.kind === "loading"
    ...
    const [attackLibrary, setAttackLibrary] = useState<RuntimeAttackSourceLibrary | null>(null);
    const [vehicleOperationLibrary, setVehicleOperationLibrary] = useState<RuntimeAttackSourceLibrary | null>(null);
    const globalAttackLibraryRequestRef = useRef<Promise<RuntimeAttackSourceLibrary> | null>(null);
    const [globalAttackLibraryState, setGlobalAttackLibraryState] = useState<"idle" | "loading" | "ready" | "error">("idle");
    ...
    const [weaponDpsFactsState, setWeaponDpsFactsState] = useState<"idle" | "loading" | "ready" | "unavailable">("idle");
    ...
    const [activeCameraView, setActiveCameraView] = useState<RuntimeViewerCameraViewId | null>(null);
    const [activeCrewViewStationId, setActiveCrewViewStationId] = useState<string | null>(null);
    const [activeCrewViewZoomIndex, setActiveCrewViewZoomIndex] = useState(0);
    const [activeOperationEquipmentRef, setActiveOperationEquipmentRef] = useState("");
    ...
    const publishVehicleWeaponOperationState = useCallback((equipmentRef: string, operationState: VehicleWeaponOperationState) => {
        ...
        vehicleWeaponOperationStore.publish(equipmentRef, operationState);
    ...
    const [crewOccupantDisplayEnabled, setCrewOccupantDisplayEnabled] = useState(false);
    const [crewHitProxyDisplayEnabled, setCrewHitProxyDisplayEnabled] = useState(false);
    ...
    }, [preview.visualVehicleId, vehicleWeaponOperationStore]);
    const driverViewActive = activeCrewViewStationId === DRIVER_VIEWPOINT_ID;
    const controlTargetStation = runtimeTurretStations.find(({ id }) => id === controlTargetId) ?? null;
    ...
    const controlTargetPitch = controlTargetStation
        ? clampTurretPitch(controlTargetStation.turret, controlTargetYaw, controlTargetPose.pitchDegrees)
        : 0;
    const controlTargetLabel = controlTargetId === DRIVER_CONTROL_TARGET_ID
        ? "驾驶员 · F1"
        : controlTargetStation?.label ?? (RUNTIME_VIEWER_CAMERA_VIEWS.find(({ id }) => id === activeCameraView)
            ?.label ?? "自由相机");
    const controlTargetCount = 2 + runtimeTurretStations.length;
    const controlTargetStationIndex = runtimeTurretStations.findIndex(({ id }) => id === controlTargetId);
    const controlTargetIndex = controlTargetId === CAMERA_CONTROL_TARGET_ID
        ...
        : controlTargetId === DRIVER_CONTROL_TARGET_ID
            ...
            : controlTargetStationIndex >= 0
                ...
                : 0;
    useEffect(() => {
        if (controlTargetId !== CAMERA_CONTROL_TARGET_ID &&
            controlTargetId !== DRIVER_CONTROL_TARGET_ID &&
    ...
    const activeGunnerSightStation = activeTurretStation?.crewSeat.stationId
        ? gunnerSight?.stations.find((station) => station.stationId === activeTurretStation.crewSeat.stationId) ?? null
        : null;
    const gunnerSightPresentationAvailable = Boolean((activeGunnerSightStation?.state === "observed-static-presentation" ||
        activeGunnerSightStation?.state === "observed-dynamic-presentation") &&
        (activeGunnerSightStation.defaultZoomStages.some(({ projectionRef }) => projectionRef !== null) ||
            activeGunnerSightStation.weaponModes.some(({ zoomStages }) => zoomStages.some(({ projectionRef }) => projectionRef !== null)) ||
            activeGunnerSightStation.layers.some(({ role, projectionRef }) => (role === "viewport-screen" || role === "reticle") &&
                projectionRef !== null) ||
            activeGunnerSightStation.textLayers.length > 0 ||
            activeGunnerSightStation.dynamicBindings.length > 0));
    const gunnerSightOverlayVisible = Boolean(gunnerSightPresentationAvailable &&
    ...
    useEffect(() => {
        if (activeCrewViewStationId === null)
            return;
        const station = runtimeTurretStations.find(({ id }) => id === activeCrewViewStationId) ?? null;
        ...
        const operationPoseCommitScheduler = createOperationViewPoseCommitScheduler();
        ...
        const applyOperationMovement = (elapsedSeconds: number) => {
            if (!station)
                return null;
            const step = operationViewMotionStep([...heldOperationKeys], elapsedSeconds, inputDynamics, operationMotionState);
            ...
            if (step.yawDelta === 0 && step.pitchDelta === 0)
                return step;
            const current = turretPoseStatesRef.current[station.id] ?? {
                yawDegrees: 0,
                pitchDegrees: 0,
            };
            updateTurretStationPose(station, current.yawDegrees + step.yawDelta, current.pitchDegrees + step.pitchDelta, { transient: true });
        ...
        const scheduleOperationPoseCommit = (commitImmediately = false) => {
            ...
            if (commitImmediately)
                operationPoseCommitScheduler.flush();
        };
        ...
        const settleOperationMovement = (commitImmediately = false) => {
            if (operationMovementFrame !== 0) {
        ...
        const onOperationKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.code === "KeyR" && station) {
                ...
                event.stopImmediatePropagation();
                if (!event.repeat)
                    reloadVehicleWeaponRef.current();
                return;
            }
            if (movementCodes.has(event.code)) {
                ...
                event.preventDefault();
                event.stopImmediatePropagation();
                operationPoseCommitScheduler.cancel();
            ...
            const action = operationViewKeyAction({
                ...
                equipmentRefs: operationEquipmentRefsRef.current,
                inventorySlots: operationInventorySlotsRef.current,
            });
            if (!action || (action.kind === "zoom" && !station))
                return;
            event.preventDefault();
            event.stopImmediatePropagation();
            if (action.kind === "zoom") {
                ...
                selectOperationEquipmentRef.current(action.equipmentRef);
            }
        };
        ...
        const settleOnVisibilityLoss = () => {
            if (document.hidden)
                settleOperationMovement(true);
        };
        const settleOnWindowBlur = () => settleOperationMovement(true);
        const flushOperationPoseCommit = () => settleOperationMovement(true);
        flushOperationPoseCommitRef.current = flushOperationPoseCommit;
        ...
        window.addEventListener("blur", settleOnWindowBlur);
        return () => {
            ...
            window.removeEventListener("blur", settleOnWindowBlur);
            if (operationMovementFrame !== 0) {
                cancelAnimationFrame(operationMovementFrame);
            }
            heldOperationKeys.clear();
            operationPoseCommitScheduler.cancel();
            if (flushOperationPoseCommitRef.current === flushOperationPoseCommit) {
                flushOperationPoseCommitRef.current = null;
            }
        };
    }, [
        activeCrewViewStationId,
    ...
    const [infantryPreviewDistanceM, setInfantryPreviewDistanceM] = useState<number | null>(null);
    ...
    useEffect(() => {
        if (!attackSelectorOpen)
            setUpperOptionsRevealed(false);
    }, [attackSelectorOpen]);
    ...
    const [localExteriorSpacedArmorHighlight, setExteriorSpacedArmorHighlight] = useState(false);
    ...
    const specialArmorVisible = displayOverrides?.specialArmorVisible ?? localSpecialArmorVisible;
    const exteriorSpacedArmorHighlight = displayOverrides?.exteriorSpacedArmorHighlight ??
        localExteriorSpacedArmorHighlight;
    const physicalPoseEnabled = displayOverrides?.physicalPoseEnabled ?? localPhysicalPoseEnabled;
    const relativeArmorScale = displayOverrides?.relativeArmorScale ?? localRelativeArmorScale;
    const [armorThicknessRange, setArmorThicknessRange] = useState<HitSceneArmorThicknessRange | null>(null);
    ...
    const [explosionPlacementCoverage, setExplosionPlacementCoverage] = useState<"covered" | "clear" | "unknown" | null>(null);
    ...
    const [protectionEnabled, setProtectionEnabled] = useState(navigationState?.protection ?? false);
    const [protectionOpacityPercent, setProtectionOpacityPercent] = useState(70);
    const [protectionPrecision, setProtectionPrecision] = useState<RuntimeProtectionMapPrecision>(RUNTIME_PROTECTION_MAP_MIN_PRECISION);
    ...
    const requestGlobalAttackLibrary = useCallback(() => {
        ...
        void request
            ...
            setAttackLibraryError(error instanceof Error ? error.message : String(error));
    ...
    const loadIndexedAttackLibrary = useCallback(async (attackerId: string) => {
        ...
        if (!resolved)
            return null;
        const document = await loadWikiVehicleWeaponRuntimeSource(resolved.entry.cardId) as WikiWeaponRuntimeSourceDocument;
        return createRuntimeAttackSourceLibrary(document, resolved.presentation);
    }, []);
    useEffect(() => {
        let active = true;
    ...
    const graphOperationEquipmentRefs = useMemo(() => driverViewActive
        ? preview.stationGraph?.vehicleEquipmentRefs ?? []
        : activeOperationGraphStation?.equipmentRefs ?? [], [
        activeOperationGraphStation?.equipmentRefs,
        driverViewActive,
    ...
    const sightOperationEquipmentRefs = useMemo(() => activeGunnerSightStation?.weaponModes.map((mode) => mode.equipmentRef) ?? [], [activeGunnerSightStation]);
    const playableOperationEquipmentRefs = useMemo(() => vehicleOperationWeapons.flatMap((weapon) => weapon.stationEquipmentId ? [weapon.stationEquipmentId] : []), [vehicleOperationWeapons]);
    const operationEquipmentRefs = useMemo(() => !equipmentResolver ? [] : operationViewEquipmentRefs({
        stationEquipmentRefs: graphOperationEquipmentRefs,
        sightEquipmentRefs: sightOperationEquipmentRefs,
        playableEquipmentRefs: playableOperationEquipmentRefs,
        inventorySlots: graphOperationEquipmentRefs.flatMap((equipmentRef) => equipmentResolver?.(equipmentRef)?.firingPresentation?.inventorySlotNumbers.map((slotNumber) => ({ equipmentRef, slotNumber })) ?? []),
    }), [
        graphOperationEquipmentRefs,
        playableOperationEquipmentRefs,
        sightOperationEquipmentRefs,
        equipmentResolver,
    ]);
    useEffect(() => {
        setActiveOperationEquipmentRef((current) => operationEquipmentRefs.includes(current)
            ? current
            : operationEquipmentRefs[0] ?? "");
    }, [
        operationEquipmentRefs,
        sightOperationEquipmentRefs,
    ]);
    const activeOperationWeapon = activeOperationEquipmentRef
        ? vehicleOperationWeapons.find((weapon) => weapon.stationEquipmentId === activeOperationEquipmentRef) ?? null
    ...
    const gunnerSightStationPoseBindings = useMemo<GunnerSightStationPoseBinding[]>(() => runtimeTurretStations.flatMap((station) => {
        if (!station.crewSeat.seatPawnClassPath)
            return [];
        return [{
                stationId: station.id,
                ...
                parentStationId: station.parentCatalogSeatIndex === null
                    ? null
                    : runtimeTurretStations.find((candidate) => candidate.seat.index === station.parentCatalogSeatIndex)?.id ?? null,
            }];
    }), [runtimeTurretStations]);
    const selectOperationEquipment = useCallback((equipmentRef: string) => {
        if (!operationEquipmentRefs.includes(equipmentRef) ||
            equipmentRef === activeOperationEquipmentRef)
            return;
        stopVehicleProjectileFireRef.current();
        setActiveOperationEquipmentRef(equipmentRef);
    }, [activeOperationEquipmentRef, operationEquipmentRefs]);
    useEffect(() => {
        operationEquipmentRefsRef.current = operationEquipmentRefs;
        operationInventorySlotsRef.current = operationEquipmentRefs.flatMap((equipmentRef) => equipmentResolver?.(equipmentRef)?.firingPresentation?.inventorySlotNumbers.map((slotNumber) => ({ equipmentRef, slotNumber })) ?? []);
        selectOperationEquipmentRef.current = selectOperationEquipment;
        return () => {
            if (selectOperationEquipmentRef.current === selectOperationEquipment) {
                selectOperationEquipmentRef.current = () => undefined;
            }
        };
    }, [operationEquipmentRefs, selectOperationEquipment, equipmentResolver]);
    useEffect(() => {
        if (activeCrewViewStationId === null || firingPresentation)
            return;
        let active = true;
        void loadWikiDataset("/data/weapons/firing-presentation.json", "sigua-vehicle-firing-presentation/v1")
            .then((value) => { if (active)
            setFiringPresentation(value as VehicleFiringPresentation); })
            .catch((error: unknown) => {
            if (active)
                setVehicleProjectileNotice(error instanceof Error ? error.message : String(error));
        });
        return () => { active = false; };
    }, [activeCrewViewStationId, firingPresentation]);
    useEffect(() => {
        if (!firingPresentation || activeCrewViewStationId === null)
            return;
        const weapon = firingPresentation.weapons[equipmentResolver?.(activeOperationEquipmentRef)?.firingPresentation?.weaponClassPath ?? ""];
        if (!weapon)
            return;
        let active = true;
        const visuals = [weapon.projectileClass, weapon.tracerProjectileClass]
            .flatMap(path => path && firingPresentation.projectiles[path] ? [firingPresentation.projectiles[path]!] : []);
        void Promise.all(visuals.map(visual => prepareVehicleProjectileVisualRef.current?.(visual)))
            .catch((error: unknown) => { if (active)
            setVehicleProjectileNotice(`弹体模型加载失败：${error instanceof Error ? error.message : String(error)}`); });
        return () => { active = false; };
    }, [activeCrewViewStationId, activeOperationEquipmentRef, firingPresentation, equipmentResolver, viewerState.kind]);
    useEffect(() => {
        if (activeCrewViewStationId === null)
            return;
        ...
        void loadWikiWeaponBallistics()
            ...
            const algorithm = await loadWikiNativeProjectileAlgorithm(catalog.algorithms.projectile);
            if (!active)
                return;
    ...
    useEffect(() => {
        ...
        if (activeOperationEquipmentRef &&
            !activeOperationWeapon &&
    ...
    const fireVehicleProjectile = useCallback((): HeldOperationFireAttempt => {
        if (!activeOperationWeapon ||
            !vehicleProjectileResource ||
            vehicleProjectileResolution?.state !== "ready") {
            setVehicleProjectileNotice(vehicleProjectileResolution?.state === "unsupported"
        ...
        try {
                ...
                setVehicleProjectileNotice(operationShot.reason === "weapon-reloading"
                    ...
                    : operationShot.reason === "weapon-cooldown"
                        ...
                        : operationShot.reason === "trigger-cycle-complete"
                            ...
                            : "当前弹匣已空");
                return {
                    nextAttemptAtMs: nextVehicleWeaponFireAtMs(operationShot.state, activeOperationSpec, nowMs),
                };
            }
            ...
            const spread = presentationProjectileSpreadSample(binding.weaponAssignmentId, shotSequence);
            ...
            if (!firingPresentation)
                throw new Error("正在载入游戏弹体显示资源…");
            const sourceWeapon = firingPresentation.weapons[equipmentResolver?.(activeOperationEquipmentRef)?.firingPresentation?.weaponClassPath ?? ""];
            if (!sourceWeapon)
                throw new Error("当前武器缺少源弹体显示配置");
            ...
            if (!sourceShot?.visual)
                throw new Error("当前弹体缺少源显示配置");
            ...
            const shotContacts: Array<{
                hit: SchoolSweepHit;
                direction: THREE.Vector3;
                timeSeconds: number;
            }> = [];
            const simulationInput = { ...buildVehicleProjectileSimulationInput(binding, launch, direction, guidanceAim, (input) => {
                    const hit = school.query.sweepSphere(input, school.offset, traceComplex);
                    if (hit)
                        shotContacts.push({ hit, timeSeconds: input.startTimeSeconds + input.deltaSeconds * hit.timeFraction,
                            direction: new THREE.Vector3(input.endCm.x - input.startCm.x, input.endCm.z - input.startCm.z, input.endCm.y - input.startCm.y).normalize() });
                    return hit;
                }),
            ...
            for (const sample of result.samples) {
                samples.push(sample);
                const distanceM = Math.hypot(sample.positionCm.x - origin.x, sample.positionCm.y - origin.y, sample.positionCm.z - origin.z) / 100;
                if (distanceM >= VEHICLE_PROJECTILE_PLAYBACK_MAX_DISTANCE_M)
                    break;
            }
            ...
            if (contact && contact.timeSeconds <= (samples.at(-1)?.timeSeconds ?? 0)) {
                ...
                const center = result.impact ? new THREE.Vector3(result.impact.positionCm.x / 100, result.impact.positionCm.z / 100, result.impact.positionCm.y / 100) : point;
                const hitRangeM = center.distanceTo(new THREE.Vector3(launch.positionCm.x / 100, launch.positionCm.z / 100, launch.positionCm.y / 100));
            ...
            if (binding.guidanceInputPolicy !== "none") {
                setGuidanceActiveUntilMs(nowMs + Math.max(250, (samples.at(-1)?.timeSeconds ?? 0) * 1000));
            }
            setVehicleProjectileNotice("");
            ...
            if (host) {
                ...
                host.dataset.projectileSourceClass = sourceShot.classPath;
                host.dataset.projectileSourceModelCount = String(sourceShot.visual.bodies.length);
            }
            return {
                nextAttemptAtMs: nextVehicleWeaponFireAtMs(operationShot.state, activeOperationSpec, nowMs),
            };
            ...
            const host = hostRef.current;
            if (host)
                host.dataset.projectilePlaybackState = "error";
            return { nextAttemptAtMs: null };
        }
    }, [
        activeOperationEquipmentRef,
    ...
    const releaseActiveVehicleWeaponTrigger = useCallback(() => {
        if (!activeOperationSpec || !activeOperationEquipmentRef)
            return;
        const storedOperationState = vehicleWeaponOperationStatesRef.current.get(activeOperationEquipmentRef);
        if (!storedOperationState)
            return;
        publishVehicleWeaponOperationState(activeOperationEquipmentRef, releaseVehicleWeaponTrigger(storedOperationState, activeOperationSpec, operationClockMs()));
    }, [
        activeOperationEquipmentRef,
        activeOperationSpec,
        publishVehicleWeaponOperationState,
    ]);
    const refillVehicleAmmunition = useCallback(() => {
        stopVehicleProjectileFireRef.current();
        vehicleWeaponOperationStatesRef.current.clear();
        vehicleWeaponOperationStore.clear();
        vehicleProjectileMagazineStateRef.current = {
    ...
    useEffect(() => {
        ...
        return () => {
            if (fireVehicleProjectileRef.current === fireVehicleProjectile) {
                fireVehicleProjectileRef.current = () => ({ nextAttemptAtMs: null });
            }
        };
    ...
    useEffect(() => {
        ...
        return () => {
            if (releaseVehicleWeaponTriggerRef.current ===
                releaseActiveVehicleWeaponTrigger)
                releaseVehicleWeaponTriggerRef.current = () => undefined;
        };
    }, [releaseActiveVehicleWeaponTrigger]);
    const weaponOptions = useMemo(() => attackSource?.weapons.map((_, optionIndex) => optionIndex) ?? [], [attackSource]);
    const displayedWeaponOptionIndex = attackSource &&
    ...
    const selectedWeaponBallistics = useMemo(() => {
        if (!selectedAttackWeapon)
            return null;
        return resolveEditorNativeBallistics(selectedAttackWeapon.ballisticsModel, selectedAttackWeapon.ballisticsWeaponIndex, targetDistanceM);
    }, [selectedAttackWeapon, targetDistanceM]);
    const selectedWeaponHasExplosion = (selectedWeaponBallistics?.explosiveLayers.length ?? 0) > 0;
    useEffect(() => {
        selectedWeaponHasExplosionRef.current = selectedWeaponHasExplosion;
        if (!selectedWeaponHasExplosion)
    ...
    const distanceControl = selectedAttackWeapon
        ? runtimeAttackDistanceControl(selectedAttackWeapon.ballisticsModel, selectedAttackWeapon.ballisticsWeaponIndex)
        : null;
    const maxDistanceM = distanceControl?.maxDistanceM ?? 0;
    const protectionMapAvailable = hitState.kind === "ready" &&
        hitHeader !== null &&
        attackReady &&
        attackHeader !== null &&
        weaponIndex >= 0;
    ...
    const relativeArmorScaleAvailable = Boolean(armorThicknessRange && armorThicknessRange.distinctThicknessCount > 1);
    ...
    const armorThicknessLegendTicks = useMemo(() => relativeArmorScaleActive
        ? relativeArmorThicknessLegendTicks(armorThicknessRange)
        : ARMOR_THICKNESS_LEGEND_TICKS, [armorThicknessRange, relativeArmorScaleActive]);
    const runtimeWeaponOptions = useMemo<RuntimeWeaponOption[]>(() => (attackLibrary?.runtimeAttackSources ?? [])
        .flatMap((source) => source.weapons.flatMap((weapon, optionIndex) => {
        const selectorVariant = weapon.selectorVariant;
        if (selectorVariant?.selectorVisibility === "debug")
            return [];
        const sourceIdentity = runtimeWeaponSourceIdentity(source);
        const provenanceLabels = [
            ...new Set((selectorVariant?.sourceLabels ?? [])
                .map((label) => label.trim())
                .filter(Boolean)),
        ];
        const sourceSummary = sourceIdentity.label;
        const weaponLabel = weaponNameZh(selectorVariant?.label ?? weapon.displayNameZh);
        const familyLabel = weaponNameZh(selectorVariant?.familyLabel ??
            weapon.explosiveCategoryLabel ??
            source.groupName);
        const qualifier = weaponNameZh(selectorVariant?.qualifier ?? weapon.displayNameZh);
        ...
        return [
            {
                ...
                familyId: selectorVariant?.familyId ??
                ...
                provenanceLabels,
                group: familyLabel,
                effectsAtDistance: (distanceM: number) => selectorWeaponEffects(weapon.directFireRoute, resolveEditorNativeBallistics(weapon.ballisticsModel, weapon.ballisticsWeaponIndex, distanceM)),
                searchText: [
        ...
        left.value.localeCompare(right.value)), [attackLibrary]);
    ...
    const sharedShotToken = useMemo(() => {
        ...
        try {
            return encodeSharedShotPaths(savedShots.map((shot) => ({
    ...
    const updateHostShotState = useCallback((result: EditorNativeShotResult | null) => {
        ...
        const radialVisualization = buildRadialDamageVisualizationPlan(result, parsedHitRef.current?.header.components ?? []);
        ...
        host.dataset.hitTerminalDistanceFromFirstHitM = String(shotTerminalDistanceFromFirstHitM(result));
        ...
        host.dataset.hitExplosionOuterRadiiM = result.radial.layers.length > 0
            ? result.radial.layers.map((radialLayer) => {
                const ballisticsLayer = result.ballistics.explosiveLayers.find((layer) => layer.layerId === radialLayer.layerId);
                return ballisticsLayer
                    ? String(ballisticsLayer.outerRadiusCm / 100)
                    : "unknown";
            }).join(",")
            : "none";
        const explosionDamageTypeKinds = result.radial.layers.map((radialLayer) => vehicleDamageTypeIconKindForPath(radialLayer.damageTypePath)
            ?? "generic");
        ...
        host.dataset.shotExplosionSettledComponents = radialVisualization
            ? [...new Set(radialVisualization.outcomes.flatMap((outcome) => outcome.componentIndices))].join(",") || "none"
            : "none";
        host.dataset.shotExplosionTargetSelection =
            radialVisualization?.targetSelection ?? "none";
    }, []);
    const applySettledShotDamageHighlight = useCallback((shotId: number | null) => {
    ...
    const applyShotResultToVisual = useCallback((record: RuntimeShotRecord, result: EditorNativeShotResult) => {
        ...
        if (result.layers.length === 0) {
            ...
            shotVisual.explosionLayers.forEach((visual, layerIndex) => {
                ...
                const damageTypeIconKind = vehicleDamageTypeIconKindForPath(ballisticsLayer.damageTypePath)
                    ?? "generic";
        ...
        const start = shotVisual.rayOrigin.clone().addScaledVector(shotVisual.rayDirection, shotVisual.firstHitDistanceM - ingressLength);
        const terminalDistanceFromFirstHitM = shotTerminalDistanceFromFirstHitM(result);
        const lastVehicleIntersectionDistanceFromFirstHitM = Math.max(0, ...record.intersections.map((intersection) => intersection.distanceFromRayOriginM - shotVisual.firstHitDistanceM));
        const omittedDistanceM = terminalDistanceFromFirstHitM
            - lastVehicleIntersectionDistanceFromFirstHitM;
        const omitContinuation = result.stoppedAtLayer === null && omittedDistanceM > 0.2;
        const traceEndsInsideVehicle = result.stoppedAtLayer === null
        ...
        const solidEndDistanceFromFirstHitM = omitContinuation
            ? lastVehicleIntersectionDistanceFromFirstHitM
            : terminalDistanceFromFirstHitM;
        const end = shotVisual.rayOrigin.clone().addScaledVector(shotVisual.rayDirection, shotVisual.firstHitDistanceM + solidEndDistanceFromFirstHitM);
        ...
        const traceRotation = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), traceDirection.normalize());
        const continuationStart = end.clone();
        const continuationEnd = shotVisual.rayOrigin.clone().addScaledVector(shotVisual.rayDirection, shotVisual.firstHitDistanceM
            + lastVehicleIntersectionDistanceFromFirstHitM
        ...
        shotVisual.traceOutline.material.color.set(opaqueShotPathMarkerColor(traceStyle.glow));
        ...
        shotVisual.entryMarker.position.copy(shotVisual.rayOrigin.clone().addScaledVector(shotVisual.rayDirection, shotVisual.firstHitDistanceM));
        ...
        shotVisual.layerMarkers.forEach((marker, index) => {
            ...
            const markerKind = editorPathMarkerKind(hitScene?.header ?? null, hitScene?.header.components[layer.componentIndex], hitScene?.header.surfaceProfiles[layer.surfaceProfileIndex], layer.penetrated);
            paintShotPathMarker(marker, index + 1, markerKind);
            marker.sphere.position.copy(shotVisual.rayOrigin!.clone().addScaledVector(shotVisual.rayDirection!, shotVisual.firstHitDistanceM + layer.distanceFromFirstHitM));
            layerMarkerProgress[index] = THREE.MathUtils.clamp((ingressLength
                + Math.max(0, layer.distanceFromFirstHitM)) / traceLength, 0, 1);
        });
        shotVisual.terminalMarker.position.copy(shotVisual.rayOrigin.clone().addScaledVector(shotVisual.rayDirection, shotVisual.firstHitDistanceM + terminalDistanceFromFirstHitM));
        shotVisual.terminalMarker.quaternion.copy(traceRotation);
        shotVisual.terminalMarkerMaterial.color.set(traceStyle.stroke);
        shotVisual.terminalMarker.scale.setScalar(traceEndsInsideVehicle ? 1.45 : 1);
        ...
        shotVisual.animationLayout = {
            ...
            continuationLengthM: omitContinuation
                ...
                : 0,
            firstImpactProgress: THREE.MathUtils.clamp(ingressLength / traceLength, 0, 1),
            layerMarkerProgress,
            traceDurationMs: THREE.MathUtils.clamp(SHOT_TRACE_MIN_DURATION_MS + traceLength * 45, SHOT_TRACE_MIN_DURATION_MS, SHOT_TRACE_MAX_DURATION_MS),
        ...
        const firstImpact = record.intersections.find((intersection) => intersection.componentIndex === firstLayer.componentIndex
            && intersection.surfaceProfileIndex === firstLayer.surfaceProfileIndex
            && intersection.triangleIndex === firstLayer.triangleIndex) ?? record.intersections[0];
        ...
        shotVisual.explosionLayers.forEach((visual, layerIndex) => {
            ...
            const radialLayer = result.radial.layers.find((layer) => layer.layerId === ballisticsLayer.layerId);
            const originOffsetM = (radialLayer?.explosionOriginOffsetCm
                ?? ballisticsLayer.impactNormalOffsetCm) / 100;
            const origin = impactPoint.clone().addScaledVector(impactNormal, originOffsetM);
            const damageTypeIconKind = vehicleDamageTypeIconKindForPath(ballisticsLayer.damageTypePath)
                ?? "generic";
            configureShotExplosionLayerVisual(visual, {
                origin,
                ...
                settledComponentIndices: radialVisualization?.outcomes
                    .filter((outcome) => outcome.radialLayerId === ballisticsLayer.layerId)
                    .flatMap((outcome) => outcome.componentIndices) ?? [],
                originComponentLabel: playerHitComponentLabel(parsedHitRef.current?.header.components[firstLayer.componentIndex]
                    ?? firstLayer),
                originOffsetM,
            });
        });
    ...
    const commitSelectedShot = useCallback((record: RuntimeShotRecord) => {
        ...
        if (hitModelRef.current) {
            setHitSceneThreeModelHoveredProfile(hitModelRef.current, rendersDirectTrace ? firstLayer?.surfaceProfileIndex ?? null : null);
        }
    ...
    const cancelShotAnimation = useCallback((settle: boolean) => {
        ...
        const animatedRecord = animatedShotId === null
            ? null
            : shotRecordsRef.current.find((record) => record.shotId === animatedShotId) ?? null;
        if (animatedRecord) {
            ...
            if (settle) {
                ...
                animatedRecord.visual.explosionLayers.forEach((layer) => {
                    settleShotExplosionLayerVisual(layer, animatedRecord.visual.selected);
                });
            }
        }
        applySettledShotDamageHighlight(settle ? activeShotIdRef.current : null);
        const host = hostRef.current;
        if (host) {
    ...
    const startShotAnimation = useCallback((record: RuntimeShotRecord) => {
        ...
        const configuredExplosionLayers = record.visual.explosionLayers.filter((layer) => layer.configured);
        ...
        const explosionEndMs = configuredExplosionLayers.length > 0
            ...
            : 0;
        const traceEndMs = layout.traceDurationMs + layout.continuationDurationMs;
        ...
        if (host) {
            ...
            host.dataset.shotAnimationShotId = String(record.shotId);
            host.dataset.shotAnimationDurationMs = String(Math.round(totalDurationMs));
        }
        const animateShot = (timestamp: number) => {
            if (animatedShotIdRef.current !== record.shotId)
                return;
            ...
            const continuationProgress = layout.continuationDurationMs > 0
                ...
                : 1;
            setShotTraceAnimationProgress(record.visual, solidProgress, continuationProgress);
            ...
            record.visual.explosionLayers.forEach((layer) => {
                const layerHighlightStrength = setShotExplosionLayerAnimationFrame(layer, explosionElapsedMs, record.visual.selected);
                if (layerHighlightStrength <= 0)
                    return;
    ...
    useEffect(() => {
        if (!attackLibrary)
            return;
        const requested = attackLibrary.runtimeAttackSourceForId(navigationState?.attacker ?? "");
        const preferred = requested ??
            attackLibrary.runtimeAttackSourceForId(preview.cardId) ??
            attackLibrary.runtimeAttackSources[0];
        if (preferred)
            setAttackSourceCardId(preferred.cardId);
        ...
        try {
            for (const indexedWeapon of source.weapons) {
                ...
                if (!modelWeapon ||
                    ...
                    throw new Error(`攻击来源弹道索引不匹配：${source.cardId}/${indexedWeapon.ballisticsId}`);
                }
            }
            const requestedNavigation = navigationStateRef.current;
            const navigationApplies = !requestedNavigation?.attacker ||
            ...
            const pendingByIndex = pendingSelection?.sourceCardId === source.cardId &&
            ...
            if (!preferredModel)
                throw new Error(`攻击来源弹道未加载：${source.cardId}`);
            const preferredMaxDistance = runtimeAttackTargetDistanceLimitM(preferredModel, preferredWeapon.ballisticsWeaponIndex);
            const requestedDistance = distancePreferenceRef.current;
            const initialDistance = preferredMaxDistance > 0
                ? Math.min(Math.max(0, requestedDistance), preferredMaxDistance)
                : 0;
            ...
            if (host)
                host.dataset.attackSourceState = "error";
        }
    }, [attackLibrary, attackLibraryError, attackSource, clearShotVisual]);
    const simulatePublishedRadialShot = useCallback((input: Parameters<typeof simulateEditorNativeShot>[0], radialOriginOverrideM: readonly [
        number,
        number,
        number
    ] | null = null) => {
        ...
        if (hitGroup && (explicitOriginWorld || firstImpact)) {
            ...
            if (explicitOriginWorld) {
                result = simulateEditorNativeShot({
                    ...input,
                    intersections: [],
                    radialOriginOverrideCm: pointLocal.toArray().map((value) => value * 100) as [
                        number,
                        number,
                        number
                    ],
        ...
        const radialLayerHitSets = buildVehicleRadialLayerHitSets({
            source,
            model: input.model,
            impactPointM: pointLocal.toArray() as [
                number,
                number,
                number
            ],
            impactNormal: normalLocal.toArray() as [
                number,
                number,
                number
            ],
        ...
        result = simulateEditorNativeShot({
            ...
            radialOriginOverrideCm: explicitOriginWorld
                ? pointLocal.toArray().map((value) => value * 100) as [
                    number,
                    number,
                    number
                ]
    ...
    const simulateCurrentShot = useCallback((nextWeaponIndex: number, nextDistanceM: number) => {
        ...
        const activeRecord = shotRecordsRef.current.find((record) => record.shotId === activeShotIdRef.current);
        if (!parsed || !weaponModel || !activeRecord || nextWeaponIndex < 0)
            return;
        cancelShotAnimation(true);
        ...
        let result = simulatePublishedRadialShot(simulationInput, activeRecord.radialOriginOverrideM);
        if (activeRecord.radialOriginOverrideM &&
            result.ballistics.explosiveLayers.length === 0) {
    ...
    const setShotExplosionOrigin = useCallback((shotId: number, originM: [
        number,
        number,
        number
    ] | null) => {
        ...
        if (!parsed || !weaponModel || selectedWeaponIndex < 0 || !record)
            return;
        cancelShotAnimation(true);
        ...
        if (originM && result.ballistics.explosiveLayers.length === 0)
            return;
    ...
    const saveRayShot = useCallback(({ intersections, rayOrigin, rayDirection, distanceM, animate = true, radialOriginOverrideM = null, }: {
        intersections: EditorNativeIntersection[];
        rayOrigin: THREE.Vector3;
        rayDirection: THREE.Vector3;
        distanceM: number;
        ...
        if (!parsed ||
            ...
            return null;
        if (radialOriginOverrideM &&
            resolveEditorNativeBallistics(weaponModel, selectedWeaponIndex, distanceM).explosiveLayers.length === 0)
            return null;
        cancelShotAnimation(true);
        const records = shotRecordsRef.current;
        const reusableRecord = records.length >= maxShotTraces ? records.shift() ?? null : null;
        const visual = reusableRecord?.visual ?? shotVisualsRef.current.find((candidate) => !records.some((record) => record.visual === candidate));
        if (!visual)
            return null;
    ...
    const saveExplosionOrigin = useCallback((originM: [
        number,
        number,
        number
    ], rayOrigin: THREE.Vector3, rayDirection: THREE.Vector3) => saveRayShot({
        intersections: [],
        rayOrigin,
        rayDirection,
    ...
    useEffect(() => {
        ...
        if (requestedNavigation.weaponIndex !== null &&
        ...
        if (!requestedWeapon || !requestedModel)
            return;
        const requestedMaxDistance = runtimeAttackTargetDistanceLimitM(requestedModel, requestedWeapon.ballisticsWeaponIndex);
        const requestedDistance = requestedMaxDistance > 0
            ? Math.min(requestedMaxDistance, Math.max(0, distancePreferenceRef.current))
            : 0;
        ...
        if (!weaponChanged && !distanceChanged)
            return;
    ...
    useEffect(() => {
        ...
        const unchanged = current.view === next.view &&
    ...
    useEffect(() => {
        ...
        const hitGroup = hitGroupRef.current;
        if (visualGroup)
            visualGroup.visible = mode === "exterior" || !hitGroup;
        ...
        if (hitModelRef.current) {
            setHitSceneThreeModelMode(hitModelRef.current, mode, exteriorSpacedArmorHighlight);
            setHitSceneThreeModelSpecialArmorVisible(hitModelRef.current, mode === "exterior" ? true : specialArmorVisibleRef.current);
        }
    ...
    useEffect(() => {
        ...
        if (!hitModelRef.current)
            return;
        setHitSceneThreeModelSpecialArmorVisible(hitModelRef.current, modeRef.current === "exterior" ? true : specialArmorVisible);
        renderRef.current?.();
    }, [specialArmorVisible]);
    useEffect(() => {
        ...
        if (!hitModelRef.current)
            return;
        setHitSceneThreeModelArmorThicknessScale(hitModelRef.current, relativeArmorScaleActive ? "relative" : "absolute");
        renderRef.current?.();
    ...
    useEffect(() => {
        ...
        if (protectionActive)
            ...
            cancelProtectionMapRef.current?.();
    }, [protectionActive]);
    useEffect(() => {
    ...
    useEffect(() => {
        const host = hostRef.current;
        if (host)
            host.dataset.hitProbeVerdict = verdict;
    }, [verdict]);
    useEffect(() => {
        const host = hostRef.current;
        if (!host || !selectedAttackWeapon)
            return;
        host.dataset.attackSourceWeaponIndex = String(selectedAttackWeapon.ballisticsWeaponIndex);
        ...
        if (selectedAttackWeapon.explosiveCategory) {
            ...
            host.dataset.attackExplosiveLayerCount = String(selectedAttackWeapon.explosiveLayerCount ?? 0);
        ...
        if (selectedAttackWeapon.weaponIndex >= 0) {
            host.dataset.attackSourceRuntimeWeaponIndex = String(selectedAttackWeapon.weaponIndex);
        }
        else {
    ...
    useEffect(() => {
        ...
        if (appliedTurretNavigationKeyRef.current === navigationKey)
            return;
        const decoded = token ? decodeViewerTurretState(token) : null;
        const defaultStationIndex = defaultTurretStation
            ? runtimeTurretStations.findIndex((station) => station.id === defaultTurretStation.id)
            : 0;
        const requestedActiveIndex = decoded?.activeStationIndex ??
            Math.max(0, defaultStationIndex);
        const requestedActiveStation = runtimeTurretStations[requestedActiveIndex] ??
        ...
        for (const encodedPose of decoded?.poses ?? []) {
            ...
            if (!station)
                continue;
            const yawDegrees = clampTurretYaw(station.turret, encodedPose.yawDegrees);
            nextPoseStates[station.id] = {
                yawDegrees,
                pitchDegrees: clampTurretPitch(station.turret, yawDegrees, encodedPose.pitchDegrees),
            };
        }
        turretPoseStatesRef.current = nextPoseStates;
        liveTurretPoseStore.publish(nextPoseStates);
    ...
    useEffect(() => {
        ...
        if (host) {
            if (activeTurretStation) {
                ...
                host.dataset.turretYawPlacementCount = String(activeTurretStation.assembly?.yawPlacementIds.length ?? 0);
                host.dataset.turretPitchPlacementCount = String(activeTurretStation.assembly?.pitchPlacementIds.length ?? 0);
    ...
    useEffect(() => {
        ...
        host.dataset.suppressedExactVisualDuplicates = String(dedupedVisual.suppressed.length);
        host.dataset.analysisIdentityStableSurfacePlacementCount = String(renderPlacements.filter(isStableAnalysisVisualSurfacePlacement).length);
        let cancelled = false;
        ...
        const compactPortableDrone = preview.cardId.includes("--portable-recon-drone--");
        ...
        const updateSkeletalPoseDataset = (enabled: boolean) => {
            ...
            for (const { controller, generatedClass, stableOccurrenceId, observedRunningGearRecord, } of skeletalPoseBindings) {
                controller.selectedBoneNames.forEach((name) => selectedBoneNames.add(name));
                controller.changedBoneNames.forEach((name) => changedBoneNames.add(name));
                declaredReferenceEquivalentMismatch ||=
                    controller.declaredReferenceEquivalentMismatch;
                if (observedRunningGearRecord) {
                    observedRunningGearIdentities.add(`${generatedClass ?? "unknown"}\u0000${stableOccurrenceId}`);
                    observedRunningGearOccurrenceIds.add(stableOccurrenceId);
                    observedRunningGearRecord.wheels.forEach((wheel) => observedRunningGearBoneNames.add(wheel.boneName));
                }
            }
            ...
            host.dataset.skeletalPoseControllerCount = String(skeletalPoseBindings.size);
            host.dataset.skeletalPoseSelectedBoneCount = String(selectedBoneNames.size);
            ...
            host.dataset.suspensionPoseRecordCount = String(observedRunningGearIdentities.size);
            host.dataset.suspensionPoseAppliedWheelOffsetCount = String(enabled ? observedRunningGearBoneNames.size : 0);
        ...
        const applySkeletalPose = (enabled: boolean) => {
            for (const { controller, skinnedMeshes, } of skeletalPoseBindings) {
                if (!enabled) {
        ...
        const registerSkeletalPose = (model: THREE.Object3D, placement: RuntimeVisualPlacement) => {
            if (placement.name.trim().toLowerCase() !== "vehicle mesh" ||
                placement.runtimeBonePoseStatus !== "observed") {
                return;
            }
            const observedRunningGearRecord = runtimePlanarSuspensionPoseForVisualOccurrence(preview.suspension.records, preview.generatedClass, placement.stableOccurrenceId);
            const skinnedMeshesBySkeleton = new Map<THREE.Skeleton, THREE.SkinnedMesh[]>();
            model.traverse((object) => {
                if (object instanceof THREE.SkinnedMesh) {
                    const skinnedMeshes = skinnedMeshesBySkeleton.get(object.skeleton) ?? [];
            ...
            for (const [skeleton, skinnedMeshes] of skinnedMeshesBySkeleton) {
                const controller = createRuntimeSkeletalPoseController(skeleton, {
                    observedSampleCount: placement.runtimeBonePoseNormalTimeSampleCount ?? 1,
                    referenceEquivalent: placement.runtimeBonePoseReferenceEquivalent === true,
                });
                if (!controller)
                    continue;
        ...
        const applyChassisPoseMatrix = (enabled: boolean) => {
            ...
            chassisPoseGroup.matrix.copy(active ? settledChassisPoseMatrix : staticChassisPoseMatrix);
            ...
            if (chassisPose) {
                ...
                host.dataset.chassisPoseActorOriginHeightCm = String(chassisPose.heightAbovePlaneCm);
            }
        };
        ...
        analysisVisualGroup.add(createAnalysisVisualDepthReset(), analysisVisualDepthGroup);
        ...
        let driverMaskLoadRequest: Promise<RuntimeDriverMaskLayer | null> | null = null;
        ...
        chassisPoseGroup.add(crewViewpointMarker.root, driverViewpointMarker.root, driverMaskHolder, crewOccupantHolder);
        ...
        const syncAnalysisVisualPresentation = () => {
            const exteriorPlaceholderActive = modeRef.current === "exterior" && analysisVisualReady && !exteriorReady;
            analysisVisualGroup.visible = exteriorPlaceholderActive || (modeRef.current !== "exterior" && Boolean(hitGroupRef.current));
            if (analysisVisualReady &&
                (exteriorPlaceholderActive || modeRef.current !== "exterior")) {
                setAnalysisVisualPresentation(analysisVisualGroup, exteriorPlaceholderActive ? "exterior-placeholder" : "analysis");
            }
            let visiblePlaceholderOccurrences = 0;
            analysisOccurrences.forEach((occurrences, stableOccurrenceId) => {
                const occurrenceVisible = !exteriorPlaceholderActive || !exteriorOccurrences.has(stableOccurrenceId);
            ...
            host.dataset.exteriorPlaceholderState = !analysisVisualReady
                ...
                : exteriorPlaceholderActive
                    ? "visible"
                    : "hidden";
            host.dataset.exteriorPlaceholderOccurrenceCount = String(visiblePlaceholderOccurrences);
        };
        ...
        scene.add(fillLight);
        const shotVisuals = Array.from({ length: maxShotTraces }, (_, traceIndex) => createShotVisual(traceIndex));
        ...
        const vehicleProjectileThreeRuntime = createVehicleProjectileThreeRuntime({
            ...
            onImpactTrace: (trace) => {
                if (worldImpactStatus)
                    worldImpactStatus.textContent = trace.summary;
        ...
        const stationMotionMatrices = (station: RuntimeTurretPreviewStation) => {
            const pose = stationPose(station.id);
            const frame = station.assembly ??
                resolveRuntimeTurretMotionFrame(station.visualAttachment);
            if (!frame)
                return null;
            const matrices = turretArticulationMatrices(frame, pose.yawDegrees, pose.pitchDegrees);
        ...
        const parentStationForView = (station: RuntimeTurretPreviewStation, stations: RuntimeTurretPreviewStation[]) => {
            const parentCatalogSeatIndex = station.crewSeat.positionSemantics?.seatPawnAttachment
                ?.parentCatalogSeatIndex;
            if (Number.isSafeInteger(parentCatalogSeatIndex)) {
                return stations.find((candidate) => candidate.seat.index === parentCatalogSeatIndex) ?? null;
            }
        ...
        const crewPoseForStation = (station: RuntimeTurretPreviewStation): CrewViewPose | null => {
            ...
            try {
                ...
                if (matrices.length === 0)
                    return null;
                return transformCrewViewPose(crewViewBasePose(station.view), matrices);
            }
            catch {
                return null;
            }
        ...
        const updateCrewViewpointMarker = () => {
            const station = runtimeTurretStationsRef.current.find(({ id }) => id === activeTurretStationIdRef.current) ?? preferredCrewViewStation(runtimeTurretStationsRef.current);
            const pose = station ? crewPoseForStation(station) : null;
            const crewViewActive = station?.id === activeCrewViewStationIdRef.current;
            const operatorViewActive = activeCrewViewStationIdRef.current !== null;
            crewViewpointMarker.root.visible = Boolean(crewViewpointMarkerEnabledRef.current && pose && !operatorViewActive);
            host.dataset.crewViewpointMarkerEnabled = String(crewViewpointMarkerEnabledRef.current);
            if (!station || !pose) {
            ...
            host.dataset.crewViewpointHorizontalFovDeg = String(pose.horizontalFovDegrees);
            if (crewViewActive) {
                activeCrewViewPose = {
                    ...pose,
                    horizontalFovDegrees: activeCrewViewPose?.horizontalFovDegrees ??
        ...
        const setDriverSceneVisibility = (active: boolean) => {
            if (active) {
                ...
                if (hitGroupRef.current)
                    hitGroupRef.current.visible = false;
                return;
            }
            const hitGroup = hitGroupRef.current;
        ...
        const ensureDriverMaskLayer = () => {
            ...
            if (driverMaskLayer)
                return Promise.resolve(driverMaskLayer);
            if (driverMaskLoadRequest)
            ...
            driverMaskLoadRequest = loadRuntimeDriverMask(driverView)
                ...
                if (layer)
                    driverMaskHolder.add(layer.root);
        ...
        const updateDriverViewpointMarker = () => {
            const operatorViewActive = activeCrewViewStationIdRef.current !== null;
            driverViewpointMarker.root.visible = Boolean(driverViewpointMarkerEnabledRef.current && !operatorViewActive);
            driverViewpointMarker.root.position.fromArray(driverPose.position);
            driverViewpointMarker.root.updateMatrixWorld(true);
            host.dataset.driverViewpointMarkerEnabled = String(driverViewpointMarkerEnabledRef.current);
            host.dataset.driverViewpointPosition = driverPose.position.join(",");
            host.dataset.driverViewpointHorizontalFovDeg = String(driverPose.horizontalFovDegrees);
            return driverPose;
        };
        ...
        const publishCrewOccupantLayer = (layer: RuntimeCrewOccupantLayer) => {
            ...
            host.dataset.crewOccupantUniqueModelVertices = String(layer.stats.uniqueModelVertices);
            host.dataset.crewOccupantEstimatedModelTriangles = String(layer.stats.estimatedModelTriangles);
            host.dataset.crewOccupantHitProxyDrawCalls = String(layer.stats.hitProxyDrawCalls);
            host.dataset.crewOccupantProtectedPoseDrawCalls = String(layer.stats.protectedPoseDrawCalls);
            host.dataset.crewOccupantExactAnimationPoses = String(layer.stats.exactAnimationPoses);
            host.dataset.crewOccupantBuildDurationMs = String(layer.stats.buildDurationMs);
        ...
        const applyCrewOccupantVisibility = (visible: boolean) => {
            const externalVisible = visible &&
                activeCrewViewStationIdRef.current === null;
            if (referenceSoldier)
                referenceSoldier.visible = !visible;
            ...
            if (crewOccupantLoadRequest)
                return;
            host.dataset.crewOccupantState = "loading";
            crewOccupantLoadRequest = import("./runtime-crew-occupants")
                .then(({ createRuntimeCrewOccupantLayer }) => createRuntimeCrewOccupantLayer({
        ...
        const render = () => {
            ...
            const activeExplosionRecord = shotRecordsRef.current.find((record) => record.shotId === activeShotIdRef.current);
            const groundPlaneY = groundReferenceY + 0.02;
            activeExplosionRecord?.visual.explosionLayers.forEach((layer) => {
                if (!layer.configured || !layer.root.visible)
                    return;
                ...
                const groundRadiusM = radialDamageGroundIntersectionRadiusM(layer.outerRadiusM, heightFromGroundM);
                const groundLocal = layer.root.worldToLocal(new THREE.Vector3(layerWorldOrigin.x, groundPlaneY, layerWorldOrigin.z));
            ...
            const activeExplosionLayer = activeExplosionRecord?.visual.explosionLayers.find((layer) => layer.configured && layer.root.visible && layer.showOriginLabel);
            const explosionHud = explosionOriginHudRef.current;
            const placementPreviewVisible = explosionPlacementPreview.root.visible;
            const hudAnchor = activeExplosionLayer?.root ?? (placementPreviewVisible ? explosionPlacementPreview.root : null);
            if (hudAnchor && explosionHud) {
                const worldOrigin = hudAnchor.getWorldPosition(new THREE.Vector3());
                ...
                if (activeExplosionLayer && detached) {
                    const heightAboveGroundM = Math.max(0, worldOrigin.y - groundPlaneY);
                    const groundLocal = activeExplosionLayer.root.worldToLocal(new THREE.Vector3(worldOrigin.x, groundPlaneY, worldOrigin.z));
                    ...
                    paintShotExplosionGroundHeight(activeExplosionLayer, heightAboveGroundM);
            ...
            if (crewViewpointHud &&
                ...
                const worldPosition = crewViewpointMarker.root.getWorldPosition(new THREE.Vector3());
                ...
                if (onScreen) {
                    ...
                    crewViewpointHud.style.setProperty("--crew-viewpoint-y", `${y - 30}px`);
                }
            }
            ...
            if (driverViewpointHud &&
                driverViewpointMarker.root.visible) {
                modelGroup.updateMatrixWorld(true);
                const worldPosition = driverViewpointMarker.root.getWorldPosition(new THREE.Vector3());
                const projected = worldPosition.clone().project(camera);
                const onScreen = projected.z >= -1 && projected.z <= 1;
                ...
                if (onScreen) {
                    ...
                    driverViewpointHud.style.setProperty("--crew-viewpoint-y", `${y - 30}px`);
                }
            }
        ...
        const applyCrewViewCameraPose = (station: RuntimeTurretPreviewStation, pose: CrewViewPose, zoomIndex = activeCrewViewZoomIndexRef.current) => {
            const zoomHorizontalFovDegrees = station.view
                ? operationViewHorizontalFovForMagnification(station.view.magnificationLevels[zoomIndex])
                : null;
            ...
            camera.fov = verticalFovForHorizontalFov(horizontalFovDegrees, camera.aspect);
            camera.near = 0.01;
            camera.far = Math.max(camera.far, VEHICLE_PROJECTILE_PLAYBACK_MAX_DISTANCE_M + 250);
            ...
            host.dataset.cameraHorizontalFovDeg = String(horizontalFovDegrees);
            host.dataset.cameraVerticalFovDeg = String(camera.fov);
            host.dataset.cameraZoomIndex = String(zoomIndex);
            host.dataset.cameraZoomMagnification = String(station.view?.magnificationLevels[zoomIndex] ?? 1);
            host.dataset.cameraZoomHorizontalFovDeg = String(horizontalFovDegrees);
            host.dataset.operationViewAspectRatio = "16:9";
            host.dataset.operationViewReferenceHorizontalFovDeg = String(OPERATION_VIEW_STANDARD_HORIZONTAL_FOV_DEGREES);
        ...
        const applyDriverViewCameraPose = () => {
            ...
            controls.target.copy(worldPosition).addScaledVector(worldForward, 25);
            camera.aspect = OPERATION_VIEW_STANDARD_ASPECT_RATIO;
            camera.fov = verticalFovForHorizontalFov(driverPose.horizontalFovDegrees, camera.aspect);
            camera.near = 0.005;
            ...
            host.dataset.cameraHorizontalFovDeg = String(driverPose.horizontalFovDegrees);
            host.dataset.cameraVerticalFovDeg = String(camera.fov);
        ...
        const applyTurretPose = (options: {
            ...
            const poses = turretPosesRef.current.filter((pose): pose is RuntimeTurretPose & {
                assembly: RuntimeTurretAssembly;
            } => Boolean(pose.assembly));
            const poseTransforms = poses.map((pose) => {
                const matrices = turretArticulationMatrices(pose.assembly, pose.yawDegrees, pose.pitchDegrees);
            ...
            for (const { pose, yawTransform, yawPitchTransform, pitchPlacementIds, } of poseTransforms) {
                for (const occurrenceId of pose.assembly.yawPlacementIds) {
                    const combined = occurrenceTransforms.get(occurrenceId) ??
                        new THREE.Matrix4();
                    combined.multiply(pitchPlacementIds.has(occurrenceId)
                        ? yawPitchTransform
                        : yawTransform);
            ...
            for (const [occurrenceId, articulationTransform] of occurrenceTransforms) {
                ...
                for (const analysisOccurrence of analysisOccurrences.get(occurrenceId) ?? []) {
            ...
            const turretHitPoseKey = poses.length > 0
                ...
                : "none";
            const runningGearBonePoseByIdentity = new Map<string, {
                stableOccurrenceId: string;
                boneName: string;
                matrix: readonly number[];
            }>();
            if (physicalPoseEnabledRef.current) {
                for (const { controller, stableOccurrenceId, observedRunningGearRecord, model, placementMatrix, } of skeletalPoseBindings) {
                    if (!observedRunningGearRecord)
                        continue;
                    ...
                    for (const wheel of observedRunningGearRecord.wheels) {
                        ...
                        if (runningGearBonePoseByIdentity.has(identity))
                            continue;
                        const componentPose = controller.componentPoseMatrixForBone(wheel.boneName, model);
                        if (!componentPose)
                            continue;
            ...
            const runningGearHitPoses = parsedHit && physicalPoseEnabledRef.current
                ? resolveRuntimeRunningGearHitComponentPoses(parsedHit.header.components, runningGearBonePoses)
                : {
                    componentPoses: [],
            ...
            if (hitModel &&
                ...
                const componentTransforms = new Map<number, THREE.Matrix4>(runningGearHitPoses.componentPoses.map(({ componentIndex, matrix }) => [
                    componentIndex,
                    new THREE.Matrix4().fromArray(matrix),
                ] as const));
                for (const { pose, yawTransform, yawPitchTransform, } of poseTransforms) {
                    const componentAssembly = resolveRuntimeTurretHitComponentAssembly({
                    ...
                    for (const componentIndex of componentAssembly.yawComponentIndices) {
                        const combined = componentTransforms.get(componentIndex) ??
                            new THREE.Matrix4();
                        combined.multiply(pitchComponents.has(componentIndex)
                            ? yawPitchTransform
                            : yawTransform);
                ...
                const hitComponentPoses = [...componentTransforms].map(([componentIndex, visualMatrix]) => ({
                    componentIndex,
                    matrix: visualToHit
                        .clone()
                ...
                radialQueryHitPoseRef.current = new Map(hitComponentPoses.map(({ componentIndex, matrix }) => [componentIndex, matrix]));
                const result = setHitSceneThreeModelComponentPoses(hitModel, parsedHit, {
                    componentPoses: hitComponentPoses,
                });
                host.dataset.turretAppliedHitComponentCount = String(result.appliedComponentCount);
                host.dataset.turretHitVertexConflictCount = String(result.conflictedVertexCount);
                host.dataset.turretAppliedHitPose = turretHitPoseKey;
                host.dataset.runningGearAppliedHitPose = runningGearHitPoseKey;
                host.dataset.runningGearAppliedHitComponentCount = String(runningGearHitPoses.componentPoses.length);
                host.dataset.runningGearUnmatchedHitComponentCount = String(runningGearHitPoses.unmatchedComponentIndices.length);
                host.dataset.runningGearAmbiguousHitComponentCount = String(runningGearHitPoses.ambiguousComponentIndices.length);
                const wheelHitComponentCount = parsedHit.header.components.filter(({ semanticKind }) => semanticKind === "wheel").length;
                const trackHitComponentCount = parsedHit.header.components.filter(({ semanticKind }) => semanticKind === "track").length;
        ...
        const collectRayIntersections = () => {
            ...
            if (!parsed || !analysisMesh)
                return [];
            modelGroup.updateMatrixWorld(true);
            normalMatrix.getNormalMatrix(analysisMesh.matrixWorld);
            return raycaster.intersectObject(analysisMesh, false).flatMap<EditorNativeIntersection>((intersection) => {
                const triangleIndex = intersection.faceIndex;
                if (triangleIndex === undefined || triangleIndex === null || !intersection.face)
                    return [];
                ...
                if (componentIndex === undefined || surfaceProfileIndex === undefined)
                    return [];
        ...
        const runProtectionMap = () => {
            ...
            if (!protectionCache ||
                ...
                protectionCache = {
                    standard: createProtectionMapGridCache(standardGridSize.width, standardGridSize.height),
            ...
            const standardTarget = Math.min(targetPrecision, RUNTIME_PROTECTION_MAP_STANDARD_MAX_PRECISION) as RuntimeProtectionMapStandardPrecision;
            const superGridSize = runtimeProtectionMapSuperGridSize(viewportWidth, viewportHeight);
            const totalSamples = targetPrecision === RUNTIME_PROTECTION_MAP_SUPER_PRECISION
                ...
                : runtimeProtectionMapCumulativeSampleCount(standardGrid.width, standardGrid.height, standardTarget);
            let workingPrecision = Math.min(standardTarget, Math.max(RUNTIME_PROTECTION_MAP_MIN_PRECISION, cache.completedStandardPrecision + 1)) as RuntimeProtectionMapStandardPrecision;
            let levelSamples: Array<[
                number,
                number
            ]> = [];
            let levelSampleIndex = 0;
            let superSampleOrder: Uint32Array | null = null;
            let phase: "standard" | "super" | "done" = cache.completedStandardPrecision >= standardTarget ? "done" : "standard";
            let completedSamples = protectionMapSampleCount(standardGrid.sampledMask) +
            ...
            const buildLevelSamples = (level: RuntimeProtectionMapStandardPrecision) => {
                ...
                for (const [columnOffset, rowOffset] of runtimeProtectionMapLevelOffsets(level)) {
                    for (let row = rowOffset; row < standardGrid.height; row += RUNTIME_PROTECTION_MAP_BLOCK_SIZE) {
                        for (let column = columnOffset; column < standardGrid.width; column += RUNTIME_PROTECTION_MAP_BLOCK_SIZE) {
                            const cellIndex = row * standardGrid.width + column;
                            if (standardGrid.sampledMask[cellIndex] === 0)
                                samples.push([column, row]);
                        }
                    }
                }
                return samples;
            };
            const sampleCell = (grid: Pick<ProtectionMapGridCache, "width" | "height">, column: number, row: number) => {
                pointer.set(((column + 0.5) / grid.width) * 2 - 1, 1 - ((row + 0.5) / grid.height) * 2);
                const intersections = collectIntersections(pointer);
                if (intersections.length === 0)
                    return 0 as RuntimeProtectionMapCell;
            ...
            const ensureSuperGrid = () => {
                if (!cache.superGrid ||
                    ...
                    cache.superGrid = createProtectionMapGridCache(superGridSize.width, superGridSize.height);
                    seedSuperProtectionMap(standardGrid, cache.superGrid);
                }
                superSampleOrder = runtimeProtectionMapSuperSampleOrder(cache.superGrid.width, cache.superGrid.height);
                phase = "super";
                host.dataset.protectionMapGrid =
                    `${cache.superGrid.width}x${cache.superGrid.height}`;
                paintProtectionMap(canvas, cache.superGrid.reconstructed, cache.superGrid.width, cache.superGrid.height);
            ...
            if (phase === "standard") {
                ...
                paintProtectionMap(canvas, standardGrid.reconstructed, standardGrid.width, standardGrid.height);
            }
            canvas.hidden = false;
            ...
            setProtectionRenderedPrecision(phase === "done"
                ? targetPrecision
                : Math.min(cache.completedStandardPrecision, RUNTIME_PROTECTION_MAP_STANDARD_MAX_PRECISION));
            ...
            const processBatch = () => {
                if (cancelled || token !== protectionToken || !protectionEnabledRef.current)
                    return;
                ...
                if (phase === "standard") {
                    ...
                    while (levelSampleIndex < levelSamples.length &&
                        frameHasBudget()) {
                    ...
                    dirtyBlocks.forEach((blockIndex) => {
                        ...
                        reconstructRuntimeProtectionMapBlock(standardGrid.sampleValues, standardGrid.sampledMask, standardGrid.width, standardGrid.height, blockColumn, blockRow, standardGrid.reconstructed);
                    });
                    if (levelSampleIndex >= levelSamples.length) {
                        ...
                        if (workingPrecision < standardTarget) {
                            workingPrecision = (workingPrecision + 1) as RuntimeProtectionMapStandardPrecision;
                            levelSamples = buildLevelSamples(workingPrecision);
                            levelSampleIndex = 0;
                        }
                        else if (targetPrecision === RUNTIME_PROTECTION_MAP_SUPER_PRECISION) {
                    ...
                    while (superGrid.nextProgressiveIndex < superSampleOrder.length &&
                        frameHasBudget()) {
                        const cellIndex = superSampleOrder[superGrid.nextProgressiveIndex];
                        superGrid.nextProgressiveIndex += 1;
                        visitedCount += 1;
                        if (superGrid.sampledMask[cellIndex] !== 0)
                            continue;
                ...
                if (phase === "done" ||
                    ...
                    const displayGrid = targetPrecision === RUNTIME_PROTECTION_MAP_SUPER_PRECISION &&
                        cache.superGrid
                        ? cache.superGrid
                        : standardGrid;
                    paintProtectionMap(canvas, displayGrid.reconstructed, displayGrid.width, displayGrid.height);
        ...
        const scheduleInitialFitStabilization = () => {
            ...
            initialFitStabilizationTimer = window.setTimeout(() => {
                ...
                if (!initialFitStabilizationPending || cameraFitUserLocked)
                    return;
                initialFitStabilizationPending = false;
                if (Math.abs(camera.aspect - initialFitAspect) < 0.08)
                    return;
        ...
        const applyCameraNavigation = (state: ViewerNavigationState | undefined, restoreDefault = true) => {
            if (fittedSource === null)
                return;
            const key = cameraNavigationKey(state);
            if (lastAppliedCameraNavigationKey === key)
                return;
            ...
            if (sharedCamera) {
                ...
                const spherical = new THREE.Spherical(sharedCamera.distance, THREE.MathUtils.degToRad(90 - sharedCamera.pitch), THREE.MathUtils.degToRad(sharedCamera.yaw));
                camera.position.copy(controls.target).add(new THREE.Vector3().setFromSpherical(spherical));
                controls.update();
            }
            else if ((state?.yaw !== null && state?.yaw !== undefined) ||
                (state?.pitch !== null && state?.pitch !== undefined)) {
                const distance = camera.position.distanceTo(controls.target);
                const spherical = new THREE.Spherical(distance, THREE.MathUtils.degToRad(90 - (state?.pitch ?? 0)), THREE.MathUtils.degToRad(state?.yaw ?? 0));
                camera.position.copy(controls.target).add(new THREE.Vector3().setFromSpherical(spherical));
                controls.update();
            }
            ...
            if (host) {
                if (state?.camera)
                    host.dataset.cameraShareToken = state.camera;
                else
                    delete host.dataset.cameraShareToken;
            }
            render();
        };
        applyCameraNavigationRef.current = (state) => applyCameraNavigation(state);
        const publishCameraNavigation = () => {
            if (fittedSource === null || !onNavigationStateChangeRef.current)
                return;
        ...
        const onControlsStart = () => {
            ...
            protectionCache = null;
            if (protectionEnabledRef.current)
                cancelProtectionMap(true, false);
        };
        const onControlsEnd = () => {
        ...
        resize();
        const fitViewToGroup = (targetGroup: THREE.Object3D, source: "hit" | "analysis" | "exterior", { force = false, preserveCamera = false, }: {
            force?: boolean;
            preserveCamera?: boolean;
        } = {}) => {
            if (fittedSource !== null && !force)
                return;
            ...
            if (bounds.isEmpty())
                throw new Error(`Loaded ${source} package produced an empty scene`);
            const modelLengthM = bounds.max.x - bounds.min.x;
            const modelWidthM = bounds.max.z - bounds.min.z;
            const groundReferenceClearanceM = runtimeGroundReferenceClearanceM(modelLengthM, modelWidthM);
            const runtimePoseGroundActive = chassisPose !== null && physicalPoseEnabledRef.current;
            ...
            if (referenceSoldier) {
                ...
                const initialSoldierBounds = new THREE.Box3().setFromObject(referenceSoldier);
                const soldierSize = initialSoldierBounds.getSize(new THREE.Vector3());
                const soldierCenter = initialSoldierBounds.getCenter(new THREE.Vector3());
                referenceSoldier.position.set(bounds.min.x
                    - groundReferenceClearanceM
                    - soldierSize.x / 2
                    - soldierCenter.x, (runtimePoseGroundActive ? 0 : bounds.min.y) -
                    initialSoldierBounds.min.y, bounds.min.z
                    - groundReferenceClearanceM
                    - soldierSize.z / 2
                    - soldierCenter.z);
                referenceSoldier.updateMatrixWorld(true);
                referenceSoldierBounds = new THREE.Box3().setFromObject(referenceSoldier);
                host.dataset.referenceSoldierRearX = String(bounds.min.x - groundReferenceClearanceM);
                host.dataset.referenceSoldierRightZ = String(bounds.min.z - groundReferenceClearanceM);
                ...
                host.dataset.referenceSoldierClearanceM = String(groundReferenceClearanceM);
            }
            const fitBounds = bounds.clone();
            if (referenceSoldierBounds)
                fitBounds.union(referenceSoldierBounds);
            ...
            modelGroup.updateMatrixWorld(true);
            const radius = Math.max(sphere.radius, compactPortableDrone ? 0.3 : 2.5);
            const groundY = runtimePoseGroundActive
                ? modelGroup.position.y
                : Math.min(bounds.min.y, referenceSoldierBounds?.min.y ?? bounds.min.y) -
                    center.y -
                    0.03;
            ...
            const groundScaleLengthSpanM = Math.max(modelLengthM, bounds.max.x - groundScaleOriginWorldX);
            const groundScaleWidthSpanM = Math.max(modelWidthM, bounds.max.z - groundScaleOriginWorldZ);
            const groundScaleLengthM = runtimeGroundScaleLengthM(groundScaleLengthSpanM);
            const groundScaleWidthM = runtimeGroundScaleLengthM(groundScaleWidthSpanM);
            ...
            const groundScaleLengthSegments = groundScaleLengthM / RUNTIME_GROUND_SCALE_TICK_INTERVAL_M;
            const groundScaleWidthSegments = groundScaleWidthM / RUNTIME_GROUND_SCALE_TICK_INTERVAL_M;
            host.dataset.groundScaleSegments = String(groundScaleLengthSegments + groundScaleWidthSegments);
            host.dataset.groundScaleLengthSegments = String(groundScaleLengthSegments);
            host.dataset.groundScaleWidthSegments = String(groundScaleWidthSegments);
            host.dataset.groundScaleTickIntervalM = String(RUNTIME_GROUND_SCALE_TICK_INTERVAL_M);
            host.dataset.groundScaleLabelIntervalM = String(RUNTIME_GROUND_SCALE_LABEL_INTERVAL_M);
            ...
            const updateCameraRangeAndFitEvidence = () => {
                ...
                const horizontalFovRadians = 2 * Math.atan(Math.tan(verticalFovRadians / 2) * Math.max(camera.aspect, 0.0001));
            ...
            const resetView = ({ preserveShotVisual = false }: {
                preserveShotVisual?: boolean;
            } = {}) => {
                protectionCache = null;
                applyInspectionProjection();
                ...
                camera.position.copy(new THREE.Vector3(1.7, 1.25, -2.7).normalize().multiplyScalar(fitDistance));
                controls.update();
                if (!preserveShotVisual)
                    clearShotVisual();
                render();
                if (protectionEnabledRef.current) {
                    scheduleProtectionMap({ invalidate: true });
            ...
            const applySquadPerspective = (distanceM: number, viewId: RuntimeViewerCameraViewId | null) => {
                ...
                if (preset) {
                    ...
                    controls.target.set(vehicleCameraTarget.x, basePosition[1], vehicleCameraTarget.z);
                    camera.position.set(vehicleCameraTarget.x + basePosition[0], basePosition[1], vehicleCameraTarget.z + basePosition[2]);
                    delete host.dataset.cameraViewPreset;
                    host.dataset.cameraViewKind = "soldier-ground";
                }
                camera.fov = verticalFovForHorizontalFov(SQUAD_INFANTRY_DEFAULT_HORIZONTAL_FOV_DEG, camera.aspect);
                camera.near = Math.max(radius / 300, 0.02);
                ...
                host.dataset.cameraHorizontalFovDeg = String(SQUAD_INFANTRY_DEFAULT_HORIZONTAL_FOV_DEG);
                host.dataset.cameraVerticalFovDeg = String(camera.fov);
                host.dataset.infantryPreviewDistanceM = String(safeDistanceM);
                host.dataset.infantryPreviewHorizontalFovDeg = String(SQUAD_INFANTRY_DEFAULT_HORIZONTAL_FOV_DEG);
                host.dataset.infantryPreviewVerticalFovDeg = String(camera.fov);
                if (preset?.kind === "overhead") {
                    delete host.dataset.infantryPreviewEyeHeightM;
                }
                else {
                    host.dataset.infantryPreviewEyeHeightM = String(camera.position.y - groundY);
                }
                render();
                publishCameraNavigation();
                if (protectionEnabledRef.current) {
            ...
            const applyInspectionView = (viewId: RuntimeViewerCameraViewId) => {
                applySquadPerspective(infantryPreviewDistanceRef.current ??
                    RUNTIME_VIEWER_INFANTRY_DISTANCES_M[0], viewId);
            };
            ...
            const enterCrewViewpoint = (stationId: string) => {
                if (activeCrewViewStationIdRef.current !== null &&
                    activeCrewViewStationIdRef.current !== stationId)
                    flushOperationPoseCommitRef.current?.();
                setDriverSceneVisibility(false);
                applyDriverMaskVisibility(false);
                const station = runtimeTurretStationsRef.current.find(({ id }) => id === stationId);
                const pose = station ? crewPoseForStation(station) : null;
                if (!station || !pose)
                    return false;
            ...
            const enterDriverViewpoint = () => {
                ...
                updateDriverViewpointMarker();
                if (driverMaskAvailable) {
                    void ensureDriverMaskLayer().then(() => applyDriverMaskVisibility(driverMaskEnabledRef.current));
                }
                else {
                    applyDriverMaskVisibility(false);
                }
            ...
            const applyCrewViewZoom = (stationId: string, zoomIndex: number) => {
                if (activeCrewViewStationIdRef.current !== stationId)
                    return false;
                const station = runtimeTurretStationsRef.current.find(({ id }) => id === stationId);
                const pose = station ? crewPoseForStation(station) : null;
                const horizontalFovDegrees = station?.view
                    ? operationViewHorizontalFovForMagnification(station.view.magnificationLevels[zoomIndex])
                    : null;
                if (!station || !pose || horizontalFovDegrees === null)
                    return false;
            ...
            const exitCrewViewpoint = () => {
                const wasDriver = activeCrewViewStationIdRef.current === DRIVER_VIEWPOINT_ID;
                if (!wasDriver)
                    flushOperationPoseCommitRef.current?.();
                activeCrewViewStationIdRef.current = null;
                setActiveCrewViewStationId(null);
        ...
        const loadReferenceSoldierAsset = () => {
            ...
            void import("./runtime-reference-soldier")
                ...
                if (proxy) {
                    ...
                    soldierScene.position.set(proxyCenter.x - soldierCenter.x, proxyBounds.min.y - soldierBounds.min.y, proxyCenter.z - soldierCenter.z);
                ...
                host.dataset.referenceSoldierGlassMeshes = String(glassRebind.reboundMeshCount);
                host.dataset.referenceSoldierGlassVertices = String(glassRebind.reboundVertexCount);
                render();
            })
                .catch((error: unknown) => {
                if (cancelled)
        ...
        startReferenceSoldierAsset = () => {
            ...
            if (referenceSoldierLoadScheduled)
                return;
            referenceSoldierLoadScheduled = true;
            host.dataset.referenceSoldierState = "scheduled";
            if (typeof window.requestIdleCallback === "function") {
                referenceSoldierIdleCallback = window.requestIdleCallback(loadReferenceSoldierAsset, { timeout: 1500 });
                return;
            }
            referenceSoldierLoadTimer = window.setTimeout(loadReferenceSoldierAsset, 0);
        };
        const lowerReferencePlaneToGroup = (targetGroup: THREE.Object3D, datasetPrefix: "exterior" | "analysis") => {
            targetGroup.updateMatrixWorld(true);
            const bounds = new THREE.Box3().setFromObject(targetGroup);
            if (bounds.isEmpty())
                return;
            host.dataset[`${datasetPrefix}BoundsMinY`] = String(bounds.min.y);
            if (chassisPose !== null && physicalPoseEnabledRef.current)
                return;
        ...
        const explosionGroundPoint = (normalizedPointer: THREE.Vector2) => {
            raycaster.setFromCamera(normalizedPointer, camera);
            return raycaster.ray.intersectPlane(new THREE.Plane(new THREE.Vector3(0, 1, 0), -explosionGroundFloorY()), new THREE.Vector3());
        };
        ...
        const updateExplosionPlacementPreview = (normalizedPointer: THREE.Vector2 | null = lastPlacementPointer) => {
            if (normalizedPointer)
                lastPlacementPointer = normalizedPointer.clone();
            const weaponModel = attackModelRef.current;
            const selectedWeaponIndex = weaponIndexRef.current;
            const activeRecord = shotRecordsRef.current.find((record) => record.shotId === activeShotIdRef.current);
            if (!lastPlacementPointer ||
            ...
            const ballistics = resolveEditorNativeBallistics(weaponModel, selectedWeaponIndex, targetDistanceRef.current);
            ...
            explosionPlacementPreview.areaDiscs.forEach((disc, layerIndex) => {
                ...
                if (layer)
                    disc.scale.setScalar(Math.max(0.001, layer.outerRadiusCm / 100));
            });
            explosionPlacementPreview.exactRadiusRings.forEach((ring, layerIndex) => {
                const layer = ballistics.explosiveLayers[layerIndex];
                ring.visible = Boolean(layer);
                if (layer)
                    ring.scale.setScalar(Math.max(0.001, layer.outerRadiusCm / 100));
            });
            explosionPlacementPreview.root.visible = false;
            const parsed = parsedHitRef.current;
            ...
            if (parsed && radialQueryRef.current && hitGroupRef.current) {
                ...
                const radialDamageEvents = previewResult.damage.filter((event) => event.damageKind === "radial");
                coverageState = radialDamageCoverageState(previewResult);
                const coveragePlan = buildRadialDamageVisualizationPlan(previewResult, parsed.header.components);
                highlightedComponentIndices = [...new Set([
                        ...(coveragePlan?.outcomes.flatMap((outcome) => outcome.componentIndices) ?? []),
                        ...radialDamageEvents.map((event) => event.sourceComponentIndex),
                    ])].filter((componentIndex) => componentIndex >= 0 && componentIndex < parsed.header.components.length);
            }
            const coverageColor = coverageState === "covered"
                ...
                : coverageState === "clear"
                    ? 0xff8d78
                    : 0xffd67f;
            explosionPlacementPreview.originCore.material.color.setHex(coverageState === "covered" ? 0xe5fffb : coverageColor);
        ...
        const scheduleExplosionPlacementPreview = (normalizedPointer: THREE.Vector2) => {
            pendingPlacementPointer = normalizedPointer.clone();
            if (placementPreviewFrame !== 0)
                return;
            placementPreviewFrame = requestAnimationFrame(() => {
                ...
                pendingPlacementPointer = null;
                if (nextPointer)
                    updateExplosionPlacementPreview(nextPointer);
            });
        };
        refreshExplosionPlacementPreviewRef.current = () => {
        ...
        const pickExplosionDragHandle = (event: PointerEvent) => {
            const activeRecord = shotRecordsRef.current.find((record) => record.shotId === activeShotIdRef.current);
            const layer = activeRecord?.visual.explosionLayers.find((candidate) => candidate.configured && candidate.root.visible);
            if (!activeRecord || !layer)
                return null;
        ...
        const flushExplosionDrag = () => {
            ...
            if (!explosionDrag?.pendingOrigin)
                return;
        ...
        let heldOperationFirePointerId: number | null = null;
        const heldOperationFireController = createHeldOperationFireController({
            attempt: () => activeCrewViewStationIdRef.current === null
                ? { nextAttemptAtMs: null }
                : fireVehicleProjectileRef.current(),
            nowMs: operationClockMs,
        ...
        const stopHeldOperationFire = () => {
            ...
            if (heldOperationFirePointerId !== null &&
                renderer.domElement.hasPointerCapture(heldOperationFirePointerId)) {
                renderer.domElement.releasePointerCapture(heldOperationFirePointerId);
            }
            heldOperationFirePointerId = null;
            host.dataset.operationFireHeld = "false";
        };
        const onOperationVisibilityChange = () => {
            if (document.visibilityState !== "visible")
                stopHeldOperationFire();
        };
        stopVehicleProjectileFireRef.current = stopHeldOperationFire;
        const onPointerDown = (event: PointerEvent) => {
            if (activeCrewViewStationIdRef.current !== null) {
                ...
                if (event.isPrimary &&
                    ...
                    event.button === 0) {
                    heldOperationFirePointerId = event.pointerId;
                    renderer.domElement.setPointerCapture(event.pointerId);
                    host.dataset.operationFireHeld = "true";
                    heldOperationFireController.start();
                }
                event.preventDefault();
                event.stopImmediatePropagation();
                return;
            ...
            if (event.button === 0) {
                ...
                if (pickedExplosion) {
                    const origin = pickedExplosion.layer.root.getWorldPosition(new THREE.Vector3());
                    explosionDrag = {
        ...
        const onPointerMove = (event: PointerEvent) => {
            ...
            if (explosionDrag?.pointerId === event.pointerId) {
                normalizedPointerForEvent(event);
                raycaster.setFromCamera(pointer, camera);
                const origin = raycaster.ray.intersectPlane(explosionDrag.plane, new THREE.Vector3());
                if (origin) {
                    origin.y = Math.max(origin.y, explosionGroundFloorY());
                    explosionDrag.pendingOrigin = origin.toArray() as [
                        number,
                        number,
                        number
                    ];
            ...
            renderer.domElement.style.setProperty("cursor", selectedWeaponHasExplosionRef.current ? "crosshair" : "");
            if (hoverFrame !== 0)
                return;
        ...
        const onPointerLeave = (event: PointerEvent) => {
            if (heldOperationFirePointerId === event.pointerId) {
                stopHeldOperationFire();
            }
            if (explosionDrag)
                return;
        ...
        const onPointerUp = (event: PointerEvent) => {
            ...
            pointerStart = null;
            if (movement > SHOT_GESTURE_THRESHOLD_PX)
                return;
            ...
            if (intersections.length === 0) {
                ...
                clearExplosionPlacementPreview();
                raycaster.setFromCamera(shotPointer, camera);
                saveExplosionOriginRef.current(origin.toArray() as [
                    number,
                    number,
                    number
                ], raycaster.ray.origin, raycaster.ray.direction);
                return;
            }
            clearExplosionPlacementPreview();
            saveRayShot({
        ...
        const onExplosionWheel = (event: WheelEvent) => {
            ...
            if (!event.shiftKey)
                return;
            const record = shotRecordsRef.current.find((candidate) => candidate.shotId === activeShotIdRef.current);
            if (!record?.radialOriginOverrideM)
                return;
            event.preventDefault();
            ...
            setShotExplosionOriginRef.current(record.shotId, [
                ...
                Math.max(explosionGroundFloorY(), record.radialOriginOverrideM[1] + direction * 0.25),
        ...
        window.addEventListener("blur", stopHeldOperationFire);
        document.addEventListener("visibilitychange", onOperationVisibilityChange);
        const urls = [...new Set(renderPlacements.map(({ assetUrl }) => assetUrl))];
        const sourceAlphaAssetUrls = new Set(renderPlacements
            .filter((placement) => runtimeAnalysisVisualTexturePolicy(placement) === "source-alpha")
        ...
        sourceAlphaLoadingManager.setURLModifier((url) => runtimeAnalysisVisualUrl(url, "source-alpha"));
        ...
        const loadAnalysisVisualAssets = async () => {
            ...
            await mapWithConcurrency(urls, renderQuality.assetLoadConcurrency, async (url) => {
                ...
                if (exteriorSource) {
                    host.dataset.analysisReusedExteriorSourceCount = String(Number(host.dataset.analysisReusedExteriorSourceCount ?? "0") + 1);
                }
                analysisLoaded += 1;
            ...
            renderPlacements.forEach((placement) => {
                const source = sources.get(placement.assetUrl);
                if (!source)
                    throw new Error(`Missing loaded source for ${placement.assetUrl}`);
                ...
                const articulatedOccurrences = analysisOccurrences.get(placement.stableOccurrenceId) ?? [];
                articulatedOccurrences.push({
                    object: depthOccurrence,
                    baseMatrix: depthBaseMatrix,
                });
                analysisOccurrences.set(placement.stableOccurrenceId, articulatedOccurrences);
                ...
                const analysisBaseMatrix = new THREE.Matrix4().fromArray(placement.matrix);
                analysisOccurrence.matrix.copy(analysisBaseMatrix);
                const stableSurfacePlacement = isStableAnalysisVisualSurfacePlacement(placement);
                ...
                analysisModel.traverse((object) => {
                    ...
                    if (sourceMaterials.some(isSiguaDProjectedMark)) {
                        ...
                        object.material = Array.isArray(object.material)
                            ? sourceMaterials.map((material) => isSiguaDProjectedMark(material)
                    ...
                    const materialStableSurface = sourceMeshRequiresStableAnalysisSurface(object);
                    const stableSurface = stableSurfacePlacement ||
                    ...
                    replaceAnalysisVisualMaterial(object, stableSurface, false);
                    occurrenceMeshes.push(object);
                });
                analysisMeshesByOccurrence.set(placement.stableOccurrenceId, occurrenceMeshes);
                analysisSurfaceEvidence.push({
                    stableOccurrenceId: placement.stableOccurrenceId,
                    actor: placement.actor,
                    name: placement.name,
                    sourceMeshPath: placement.sourceMeshPath,
                    geometryScore: (sourceGeometryScores.get(placement.assetUrl) ?? 0) *
            ...
            host.dataset.analysisVisualDepthOccluderMeshCount = String(analysisVisualDepthOccluderMeshCount);
        ...
        const loadExteriorAssets = () => {
            ...
            const exteriorPlacementsByUrl = new Map(urls.map((url) => [
                url,
                renderPlacements.filter((placement) => placement.assetUrl === url),
            ]));
            const exteriorAssetUrlBySourceUrl = new Map(urls.map((url) => {
                const selectedUrls = new Set((exteriorPlacementsByUrl.get(url) ?? []).map((placement) => runtimeExteriorVisualAssetUrl(placement, renderQuality.tier)));
                    ...
                    model.traverse((object) => {
                        ...
                        materials.forEach((material) => {
                            ...
                            [candidate.map, candidate.normalMap, candidate.roughnessMap, candidate.metalnessMap]
                                ...
                                .forEach((texture) => {
                                texture.anisotropy = Math.min(renderQuality.textureAnisotropy, renderer.capabilities.getMaxAnisotropy());
            ...
            exteriorPromise = mapWithConcurrency(urls, renderQuality.assetLoadConcurrency, async (url) => {
                const exteriorAssetUrl = exteriorAssetUrlBySourceUrl.get(url);
                if (!exteriorAssetUrl)
                    throw new Error(`Missing exterior source for ${url}`);
                const gltf = await exteriorLoader.loadAsync(exteriorAssetUrl);
                if (cancelled) {
                    disposeScene(gltf.scene);
                    return;
                }
                const textureReuse = dedupeRuntimeSceneTextures(gltf.scene, exteriorTextureCache);
                reusedExteriorTextures += textureReuse.reused;
                host.dataset.exteriorUniqueTextureCount = String(textureReuse.unique);
                host.dataset.exteriorReusedTextureCount = String(reusedExteriorTextures);
        ...
        const radialQueryPromise = radialQuery
            ? loadWikiVehicleRadialQuery(radialQuery.recordUrl).then((value) => {
                ...
                if (source.rawName !== preview.variantRawName ||
        ...
        const hitPromise = hit
            ? Promise.all([
                ...
                setHitSceneThreeModelMode(hitModel, modeRef.current, exteriorSpacedArmorHighlightRef.current);
                setHitSceneThreeModelSpecialArmorVisible(hitModel, modeRef.current === "exterior"
                    ? true
                    : specialArmorVisibleRef.current);
                setHitSceneThreeModelArmorThicknessScale(hitModel, relativeArmorScaleRef.current ? "relative" : "absolute");
                setArmorThicknessRange(hitModel.armorThicknessRange);
                hitGroup.add(hitModel.armor, hitModel.armorOverlay, hitModel.blockerOverlay, hitModel.interior);
                hitGroup.visible =
                    modeRef.current !== "exterior" ||
                        exteriorSpacedArmorHighlightRef.current;
                ...
                visualGroup.visible = modeRef.current === "exterior";
                analysisVisualGroup.visible = modeRef.current !== "exterior";
                host.dataset.analysisVisualOccurrenceCount = String(renderPlacements.length);
                setHitState({
                    kind: "ready",
                ...
                if (pendingExplosion && selectedWeaponHasExplosionRef.current) {
                    ...
                    saveExplosionOriginRef.current(pendingExplosion.originM, pendingExplosion.rayOrigin, pendingExplosion.rayDirection);
                }
                host.dataset.hitVehicleId = hit.vehicleId;
                host.dataset.staticHitRuntime = String(parsed.record.header.formatVersion === "hit-scene-record/v1");
        ...
        activateAssetModeRef.current = (nextMode) => {
            ...
            if (hitLoadSucceeded && hitGroupRef.current) {
                if (fittedSource === null)
                    fitViewToGroup(hitGroupRef.current, "hit");
                setViewerState({
                    kind: "ready",
                    loaded: analysisLoaded,
                    total: urls.length,
        ...
        startAnalysisVisualAssets = () => {
            if (analysisVisualPromise || analysisVisualReady)
                return;
            analysisVisualPromise = hitPromise.then(async (parsed) => {
                ...
                if (cancelled)
                    return;
                const stableReasons = analysisVisualStableSurfaceReasons(analysisSurfaceEvidence, parsed?.header.components.map(({ componentPath }) => componentPath) ?? []);
                ...
                stableReasons.forEach((reasons, stableOccurrenceId) => {
                    ...
                    for (const mesh of analysisMeshesByOccurrence.get(stableOccurrenceId) ?? []) {
                        if (mesh.userData.analysisVisualStableSurface === true)
                            continue;
                ...
                host.dataset.analysisStableSurfacePlacementCount = String(stableReasons.size);
                host.dataset.analysisStableSurfaceOccurrenceCount = String(stableAnalysisSurfaceOccurrences.size);
                host.dataset.analysisStableSurfaceMeshCount = String(stableAnalysisSurfaceMeshCount);
                host.dataset.analysisVisualDepthBiasMeshCount = String(analysisVisualDepthOccluderMeshCount);
                host.dataset.analysisVisualDepthBiasFactor = String(ANALYSIS_VISUAL_DEPTH_BIAS_FACTOR);
                host.dataset.analysisVisualDepthBiasUnits = String(ANALYSIS_VISUAL_DEPTH_BIAS_UNITS);
                host.dataset.analysisStableSurfaceIdentityCount = String(reasonCounts.get("identity") ?? 0);
                host.dataset.analysisStableSurfaceMaterialCount = String(reasonCounts.get("source-material") ?? 0);
                host.dataset.analysisStableSurfaceActorAbsentCount = String(reasonCounts.get("actor-absent-from-hit") ?? 0);
                host.dataset.analysisStableSurfaceSubordinateCount = String(reasonCounts.get("subordinate-geometry") ?? 0);
                ...
                if (pendingSharedShots.paths.length > 0 &&
                    ...
                    const restoredActiveIndex = Math.min(Math.max(0, pendingSharedShots.activeIndex), restoredRecords.length - 1);
                    const restoredActive = restoredRecords[restoredActiveIndex];
                    if (restoredActive)
                        selectSavedShot(restoredActive.shotId);
        ...
        return () => {
            ...
            if (applyCrewOccupantVisibilityRef.current ===
                applyCrewOccupantVisibility) {
                applyCrewOccupantVisibilityRef.current = null;
            }
            if (applyCrewHitProxyVisibilityRef.current ===
            ...
            window.removeEventListener("blur", stopHeldOperationFire);
            document.removeEventListener("visibilitychange", onOperationVisibilityChange);
            stopHeldOperationFire();
            if (stopVehicleProjectileFireRef.current === stopHeldOperationFire) {
                stopVehicleProjectileFireRef.current = () => undefined;
            }
    ...
    const targetBurning = useMemo(() => vehicleTargetBurningProfile(referenceData), [referenceData]);
    const targetBurningProfile = targetBurning.state === "ready"
        ? targetBurning.profile
        : null;
    const hitDpsFactsState = weaponDpsFactsState === "ready" && targetBurning.state === "unavailable"
        ? "unavailable"
        : weaponDpsFactsState;
    const weaponHitDpsEstimates = useMemo(() => weaponDpsFacts && shotResult && targetBurning.state === "ready"
        ? estimateWeaponHitDps(weaponDpsFacts, shotResult, {
            targetHealth: 1,
            horizonSeconds: 120,
    ...
    const weaponHitDpsTargets = useMemo(() => shotResult ? targetPoolsForShot(shotResult, targetBurningProfile) : [], [shotResult, targetBurningProfile]);
    useEffect(() => {
        if (!selectedAttackWeapon ||
            !weaponDpsFacts ||
        ...
        onDuelHitChangeRef.current?.({
            ...
            weaponLabel: weaponNameZh(selectedAttackWeapon.selectorVariant?.label ??
    ...
    const activeSavedShot = savedShots.find((savedShot) => savedShot.shotId === activeShotId) ?? null;
    const explosionOriginDraggable = (ballistics?.explosiveLayers.length ?? 0) > 0 &&
        activeShotId !== null;
    const explosionOriginPlacement = selectedWeaponHasExplosion && !explosionOriginDraggable;
    const explosionOriginPlacementTargetsVehicle = explosionOriginPlacement && realtimePointer !== null;
    const explosionOriginDetached = Boolean(activeSavedShot?.radialOriginOverrideM);
    ...
    const explosionOriginContactOffsetM = explosionOriginDetached
        ...
        : (shotResult?.radial.layers[0]?.explosionOriginOffsetCm
            ...
            ?? 0) / 100;
    const ballisticsPenetrationKind = weaponPenetrationKindForDamageTypePath(ballistics?.damageTypePath ?? null);
    ...
    const explosionDamageEvents = shotResult
        ...
        : [];
    const hasVehicleRadialDamage = shotResult?.damage.some((damage) => damage.damageKind === "radial"
        && editorNativeEffectiveDamageAmount(damage) > 0
        && isEditorNativeVehicleDamageEvent(damage)) ?? false;
    ...
    const damageEventsByLayer = groupDamageEventsByVisibleLayer(visibleShotLayers, effectiveDamageEvents);
    const damageOutcomeSummaries = summarizeDamageOutcomes(effectiveDamageEvents);
    const hullDamageOutcome = damageOutcomeSummaries.find((outcome) => outcome.poolKind === "hull") ?? null;
    const componentDamageOutcomes = damageOutcomeSummaries.filter((outcome) => outcome.poolKind !== "hull");
    ...
    const totalEffectiveDamage = effectiveDamageEvents
        ...
        .reduce((total, event) => total + editorNativeEffectiveDamageAmount(event), 0);
    const damageAnimationKey = `${activeShotId}:${damageAnimationRevision}`;
    const activeShotWeaponName = selectedAttackWeapon
        ? weaponNameZh(selectedAttackWeapon.selectorVariant?.label
            ?? selectedAttackWeapon.displayNameZh)
        : null;
    return (<div className="viewer-stage runtime-vehicle-viewer" data-viewer-state={viewerState.kind} data-viewer-presentation={viewerPresentation} data-viewer-variant-raw-name={preview.variantRawName} data-hit-state={hitState.kind} data-hit-access={hit?.status ?? "absent"} data-hit-probe-verdict={verdict} data-static-hit-runtime={hitState.kind === "ready" ? "true" : undefined} data-hit-solver={hitState.kind === "ready" ? "editor-native-direct-hit" : undefined} data-hit-weapon-label-source={attackReady
      ...
      <div className="viewer-canvas" aria-label={`${displayName} 交互式 3D 视图`}>
        ...
        <div className="viewer-explosion-origin-hud" data-placement={explosionOriginPlacement ? "true" : "false"} data-placement-target={explosionOriginPlacementTargetsVehicle ? "vehicle" : "scene"} data-detached={explosionOriginDetached ? "true" : "false"} data-coverage={explosionOriginCoverage ?? undefined} ref={explosionOriginHudRef} hidden={!explosionOriginDraggable && !explosionOriginPlacement} aria-label={explosionOriginPlacement
          ...
          <b>{explosionOriginPlacement
            ...
            : explosionOriginDetached
                ? "自由"
                : "拖动"}</b>
          {explosionOriginCoverage ? (<span className="viewer-explosion-origin-hud__coverage" data-state={explosionOriginCoverage}>
          ...
          {!explosionOriginPlacement &&
            ...
            Math.abs(explosionOriginContactOffsetM) > 0.025 ? (<span className="viewer-explosion-origin-hud__offset">
              ↗ {explosionOriginContactOffsetM.toFixed(1)} m
            </span>) : null}
          {explosionOriginDetached ? <kbd>Shift ⇅</kbd> : null}
          {explosionOriginDetached && activeShotId !== null ? (<button type="button" onClick={() => setShotExplosionOrigin(activeShotId, null)} aria-label="将爆心贴回原命中点" title="贴回命中点">
              <RotateCcw size={13} aria-hidden="true"/>
            </button>) : null}
        </div>
        <div className="viewer-crew-viewpoint-hud" ref={crewViewpointHudRef} hidden aria-label={activeTurretStation
        ...
        <div className="viewer-crew-viewpoint-hud viewer-driver-viewpoint-hud" ref={driverViewpointHudRef} hidden aria-label="驾驶员 F1 观察点">
          <span className="viewer-crew-viewpoint-hud__optic" aria-hidden="true">
            <Crosshair size={13}/>
          </span>
          <span className="viewer-crew-viewpoint-hud__copy">
      ...
      {gunnerSightOverlayVisible && activeGunnerSightStation && activeTurretStation && gunnerSight ? (<GunnerSightOverlay station={activeGunnerSightStation} projections={gunnerSight.projections} stationLabel={activeTurretStation.label} magnificationLevels={activeTurretStation.view?.magnificationLevels ?? []} zoomHorizontalFovDegrees={activeTurretStation.view?.magnificationLevels.map((_, zoomIndex) => operationViewHorizontalFovForMagnification(activeTurretStation.view!.magnificationLevels[zoomIndex])) ?? []} activeZoomIndex={activeCrewViewZoomIndex} activeEquipmentRef={activeOperationEquipmentRef} activeStationId={activeTurretStation.id} poseStore={liveTurretPoseStore} stationPoseBindings={gunnerSightStationPoseBindings} operationState={gunnerSightOperationState} weaponOperation={{
                store: vehicleWeaponOperationStore,
                equipmentRef: activeOperationEquipmentRef,
      ...
      {activeCrewViewStationId !== null ? (<div className="crew-view-operation-dock">
          <div className="crew-view-immersive-controls" aria-label={driverViewActive
            ...
            {driverViewActive && driverMaskAvailable ? (<button type="button" role="switch" aria-label="显示驾驶遮罩" aria-checked={driverMaskEnabled} data-active={driverMaskEnabled || undefined} onClick={() => setDriverMaskEnabled((enabled) => !enabled)}>
          ...
          {!driverViewActive && activeTurretStation ? (<div className="crew-view-operation-panel" data-expanded={operationPanelExpanded} aria-label={`${activeTurretStation.label}方位俯仰控制`}>
      ...
      {showSceneLoadingOverlay ? (<VehicleViewerLoading vehicleName={displayName} onClose={onClose} embedded/>) : null}
      {viewerState.kind === "error" ? (<div className="runtime-vehicle-viewer__error" role="alert">
      ...
      {realtimePointer ? (<span className="viewer-realtime-crosshair" data-outline={realtimePointer.outline} data-fill={realtimePointer.fill ?? "none"} style={{ left: realtimePointer.x, top: realtimePointer.y }} aria-hidden="true"/>) : null}
      {realtimePointer ? (<aside className="viewer-realtime-readout" data-outline={realtimePointer.outline} data-placement={realtimePointer.placement} style={{ left: realtimePointer.x, top: realtimePointer.y }} aria-label="实时命中解算">
          <dl>
            ...
            <div className="viewer-realtime-readout__components">
              ...
              <strong>{realtimePointer.componentLabels.length > 0
                ? realtimePointer.componentLabels.map((label, index) => (<span className="viewer-realtime-readout__component-label" key={`${label}-${index}`}>
                      {index > 0 ? "/ " : ""}{label}
                    </span>))
                : "—"}</strong>
            </div>
      ...
      {mode === "armor" && hitState.kind === "ready" ? (<div className="viewer-top-guide" data-with-armor-legend="true">
          <div className="viewer-armor-thickness-legend" data-scale={relativeArmorScaleActive ? "relative" : "absolute"} aria-label={relativeArmorScaleActive && armorThicknessRange
            ...
            <div className="viewer-armor-thickness-legend__bar" style={{
            ...
            <div className="viewer-armor-thickness-legend__ticks">
              {armorThicknessLegendTicks.map((tick) => (<span key={`${tick.thicknessMm}-${tick.normalizedPosition}`} style={{ left: `${tick.normalizedPosition * 100}%` }}>
                  {tick.label}
                </span>))}
            </div>
            <small>
      ...
      <div className="viewer-engagement-controls" aria-label="命中分析参数">
        {attackLibrary ? (<>
            <RuntimeWeaponSelector value={attackSource && displayedWeaponOptionIndex >= 0
                ? weaponSelectionValue(attackSource.cardId, displayedWeaponOptionIndex)
                : ""} options={runtimeWeaponOptions} targetDistanceM={targetDistanceM} onRequestGlobalLibrary={requestGlobalAttackLibrary} globalLibraryState={globalAttackLibraryState} onOpenChange={setWeaponSelectorOpen} onSourceOpenChange={setSourceSelectorOpen} onChange={(nextValue) => {
                const selection = parseWeaponSelectionValue(nextValue);
                if (!selection)
                    return;
                const nextSource = attackLibrary?.runtimeAttackSourceForId(selection.sourceCardId) ?? null;
                const nextWeapon = nextSource?.weapons[selection.optionIndex];
                if (!nextSource || !nextWeapon)
                    return;
                if (nextSource.cardId !== attackSource?.cardId) {
                    ...
                    if (current) {
                        const next = {
                            ...
                            weapon: "",
                            weaponIndex: selection.optionIndex === defaultAttackWeaponOptionIndex(nextSource)
                                ? null
                ...
                if (!nextWeapon || !nextModel)
                    return;
                const nextMaxDistance = runtimeAttackTargetDistanceLimitM(nextModel, nextWeapon.ballisticsWeaponIndex);
          ...
          </>) : attackLibraryError ? (<button className="viewer-global-weapon-fallback" type="button" disabled={globalAttackLibraryState === "loading"} onClick={requestGlobalAttackLibrary}>
            {globalAttackLibraryState === "loading"
                ? "正在载入武器选择器…"
                : "当前载具没有独立武器分片 · 载入全站武器选择器"}
          </button>) : (<div className="viewer-attack-library-status" data-state="loading">
            正在加载当前载具武器…
          </div>)}
        <div className="viewer-distance-control" data-disabled={maxDistanceM === 0}>
          <span>
            <span className="viewer-distance-control__label">
              <span>攻击距离</span>
              {distanceDecayNotice ? (<small className="viewer-distance-control__notice">{distanceDecayNotice}</small>) : null}
            </span>
            <strong>{distanceLabel}</strong>
          </span>
          <div className="viewer-distance-slider" data-has-ticks={quickDistanceTicks.length > 0}>
            <input type="range" aria-label={maxDistanceM === 0 ? "当前弹药不可调整距离" : `攻击距离 ${targetDistanceM} 米`} min={0} max={Math.max(maxDistanceM, 1)} step={50} value={Math.min(targetDistanceM, Math.max(maxDistanceM, 1))} disabled={maxDistanceM === 0} style={{
            "--range-progress": `${maxDistanceM > 0 ? (targetDistanceM / maxDistanceM) * 100 : 0}%`,
        } as CSSProperties} onPointerDown={() => setDistanceInteractionActive(true)} onPointerUp={() => setDistanceInteractionActive(false)} onPointerCancel={() => setDistanceInteractionActive(false)} onKeyDown={() => setDistanceInteractionActive(true)} onKeyUp={() => setDistanceInteractionActive(false)} onBlur={() => setDistanceInteractionActive(false)} onChange={(event) => {
            ...
            {quickDistanceTicks.length > 0 ? (<div className="viewer-distance-ticks" aria-label="快速距离刻度">
                {quickDistanceTicks.map((tick, index) => (<button type="button" key={tick} data-edge={index === 0 ? "start" : index === quickDistanceTicks.length - 1 ? "end" : undefined} style={{ left: `${(tick / maxDistanceM) * 100}%` }} aria-label={`设置距离为 ${tick} 米`} onClick={() => {
      ...
      <div className="viewer-toolbar" aria-label="3D 查看模式">
        <div className="viewer-toolbar__tertiary">
          <div className="viewer-protection-controls viewer-control-deck" data-enabled={protectionMapAvailable} data-selector-open={attackSelectorOpen} data-revealed={upperOptionsRevealed}>
            {attackSelectorOpen ? (<button className="viewer-protection-controls__collapse-cue" type="button" data-viewer-control-cue="weapon-selector" data-revealed={upperOptionsRevealed} aria-label={upperOptionsRevealed
                ? "收起上方选项栏"
                : "展开上方选项栏"} title={upperOptionsRevealed
                ? "收起上方选项栏"
                : "展开上方选项栏"} onPointerDown={(event) => event.stopPropagation()} onClick={() => setUpperOptionsRevealed((revealed) => !revealed)}>
                <span aria-hidden="true">{upperOptionsRevealed ? "‹" : "›"}</span>
              </button>) : null}
            <div className="viewer-render-row">
              <div className="viewer-mode-tabs" role="group" aria-label="渲染模式" data-mode-count={exteriorUnavailableMessage ? 2 : 3} style={{
            "--viewer-mode-count": exteriorUnavailableMessage ? 2 : 3,
            "--viewer-mode-index": Math.max(0, VIEWER_MODES
                ...
                {VIEWER_MODES
            .filter(([value]) => !exteriorUnavailableMessage || value !== "exterior")
            .map(([value, label]) => (<button type="button" key={value} data-active={mode === value} aria-pressed={mode === value} disabled={!onModeChange} onClick={() => onModeChange?.(value)}>
                      {label}
                    </button>))}
              </div>
            </div>
            ...
            <div className="viewer-protection-primary" data-enabled={protectionMapAvailable}>
              <button className="viewer-protection-switch viewer-state-switch" type="button" role="switch" aria-label="防护分析，仅在当前浏览器本机计算" aria-checked={protectionActive} data-active={protectionActive} disabled={!protectionMapAvailable} title="射线与伤害求解仅在当前浏览器分帧执行，不占用服务器算力" onClick={() => {
            ...
            if (current) {
              ...
              <label className="viewer-protection-opacity" data-disabled={!protectionActive}>
                <span>透明度</span>
                <input type="range" min={10} max={100} step={5} value={protectionOpacityPercent} disabled={!protectionActive} style={{
            "--range-progress": `${((protectionOpacityPercent - 10) / 90) * 100}%`,
        } as CSSProperties} aria-label={`防护分析透明度 ${protectionOpacityPercent}%`} onChange={(event) => setProtectionOpacityPercent(Number(event.currentTarget.value))}/>
            ...
            "--precision-progress": `${((protectionPrecision - RUNTIME_PROTECTION_MAP_MIN_PRECISION) / (RUNTIME_PROTECTION_MAP_MAX_PRECISION - RUNTIME_PROTECTION_MAP_MIN_PRECISION)) * 100}%`,
                ...
                <input type="range" min={RUNTIME_PROTECTION_MAP_MIN_PRECISION} max={RUNTIME_PROTECTION_MAP_MAX_PRECISION} step={1} value={protectionPrecision} disabled={!protectionActive} aria-label={protectionPrecision === RUNTIME_PROTECTION_MAP_SUPER_PRECISION
            ? "防护分析计算精度 超级档，可能导致严重卡顿"
            : `防护分析计算精度 ${protectionPrecision} 档`} onInput={(event) => setProtectionPrecision(clampRuntimeProtectionMapPrecision(Number(event.currentTarget.value)))} onChange={(event) => setProtectionPrecision(clampRuntimeProtectionMapPrecision(Number(event.currentTarget.value)))}/>
                <span className="viewer-protection-precision__capture-speed" aria-hidden="true">
                  {[1, 2, 3, 4, 5, 6].map((level) => (<b key={level} data-ready={level <= protectionRenderedPrecision} data-target={level <= protectionPrecision} data-super={level === RUNTIME_PROTECTION_MAP_SUPER_PRECISION}/>))}
                </span>
                <span className="viewer-protection-precision__ticks" aria-hidden="true">
                  {[1, 2, 3, 4, 5, 6].map((level) => (<b key={level} data-active={level <= protectionRenderedPrecision} data-target={level === protectionPrecision && protectionPrecision !== protectionRenderedPrecision} data-super={level === RUNTIME_PROTECTION_MAP_SUPER_PRECISION}>
            ...
            : protectionSampleProgress.completed >= protectionSampleProgress.total
                ...
                <span className="viewer-protection-precision__warning" data-visible={protectionPrecision === RUNTIME_PROTECTION_MAP_SUPER_PRECISION} aria-hidden={protectionPrecision !== RUNTIME_PROTECTION_MAP_SUPER_PRECISION} role="note">
                  高负载 · 可能严重卡顿
                </span>
              </span>
            </label>
            {hitState.kind === "ready" ? (<>
                <div className="viewer-spaced-armor-row">
                  <button className="viewer-protection-switch viewer-state-switch viewer-spaced-armor-switch" type="button" role="switch" aria-label="显示附加装甲/无敌区域" aria-checked={specialArmorDisplayActive} data-active={specialArmorDisplayActive} title={mode === "exterior"
                ? "在外观模式叠加附加装甲与命中层高亮"
                : "在装甲或内构模式显示附加装甲与无敌区域"} onClick={toggleSpecialArmorDisplay}>
                    <span className="viewer-protection-switch__track viewer-state-switch__track" aria-hidden="true"><span /></span>
                    <span>附加装甲/无敌区域</span>
                ...
                <div className="viewer-relative-armor-row">
                  <button className="viewer-protection-switch viewer-state-switch viewer-relative-armor-switch" type="button" role="switch" aria-label="按当前载具相对厚度着色" aria-checked={relativeArmorScaleActive} data-active={relativeArmorScaleActive} disabled={!relativeArmorScaleAvailable} title={relativeArmorScaleAvailable && armorThicknessRange
                ...
                : "当前载具没有两个以上可比较的装甲厚度"} onClick={() => setRelativeArmorScale((enabled) => !enabled)}>
                    <span className="viewer-protection-switch__track viewer-state-switch__track" aria-hidden="true"><span /></span>
                    <span>相对厚度色阶</span>
            ...
            {protectionActive ? (<div className="viewer-protection-legend" aria-label="防护分析图例">
            ...
            : "该载具没有可定位的乘员 construction-reference frame。"} onClick={() => setCrewOccupantDisplayEnabled((enabled) => !enabled)}>
                <span className="viewer-protection-switch__track viewer-state-switch__track" aria-hidden="true">
                  <span />
                </span>
                <span>乘员位置</span>
            ...
            : "无数据"}</strong>
              ...
              {crewOccupantDisplayEnabled ? (<div className="viewer-crew-occupant-legend" aria-label="乘员位置图例">
                  <span data-kind="hittable">
                    <i />可受击真实人物
                    <b>{crewOccupantCounts.hittable}</b>
                  </span>
                  {crewOccupantCounts.hittable > 0 ? (<button type="button" className="viewer-state-switch viewer-crew-hit-proxy-toggle" role="switch" aria-label="显示乘员受击判定范围" aria-checked={crewHitProxyDisplayEnabled} data-active={crewHitProxyDisplayEnabled || undefined} onClick={() => setCrewHitProxyDisplayEnabled((enabled) => !enabled)}>
      ...
      {!controlPanelOpen ? (<button className="viewer-weapon-panel-launcher" type="button" aria-controls="viewer-weapon-panel" aria-expanded="false" aria-label="打开视角与武器站控制" onClick={() => setControlPanelOpen(true)}>
          <span>
            <i aria-hidden="true"/>
            <b>视角与武器站</b>
          </span>
      ...
      {controlPanelOpen ? (<aside className="viewer-weapon-panel" id="viewer-weapon-panel" aria-label="视角与武器站控制">
          ...
          <div className="viewer-weapon-panel__body" data-control-target={controlTargetId}>
            {controlTargetId === CAMERA_CONTROL_TARGET_ID ? (<div className="viewer-unified-camera-controls">
                <RuntimeViewerCameraControls activeView={activeCameraView} infantryDistanceM={infantryPreviewDistanceM} disabled={!initialCameraFitReady} onView={(viewId) => applyCameraViewPresetRef.current?.(viewId)} onInfantryDistance={(distanceM) => applyInfantryDistancePreviewRef.current?.(distanceM)} onFree={() => enterFreeCameraViewRef.current?.()}/>
                <div className="viewer-physical-pose-row">
                  <button className="viewer-state-switch viewer-physical-pose-switch" type="button" role="switch" aria-label="真实物理状态" aria-checked={physicalPoseActive} data-active={physicalPoseActive} disabled={!chassisPose} title={chassisPose
                    ? `稳定物理姿态：俯仰 ${chassisPose.pitchDeg.toFixed(2)}°，横滚 ${chassisPose.rollDeg.toFixed(2)}°`
                    : "当前载具没有稳定收敛的运行时底盘姿态"} onClick={() => setPhysicalPoseEnabled((enabled) => !enabled)}>
                    <span className="viewer-state-switch__track" aria-hidden="true"><span /></span>
                    <span>真实物理状态</span>
                    <strong>{chassisPose
              ...
              </div>) : controlTargetId === DRIVER_CONTROL_TARGET_ID ? (<div className="viewer-driver-view-controls" data-mask-available={driverMaskAvailable || undefined}>
                <span className="viewer-control-inline-status">
                  {driverMaskAvailable ? "专属遮罩" : "无专属遮罩"}
                </span>
                <button className="viewer-state-switch" type="button" role="switch" aria-label="显示驾驶员观察点" aria-checked={driverViewpointMarkerEnabled} data-active={driverViewpointMarkerEnabled || undefined} onClick={() => {
                ...
                <button className="viewer-driver-view-controls__enter" type="button" data-active={driverViewActive || undefined} onClick={() => {
                    if (driverViewActive)
                        exitCrewViewpointRef.current?.();
                    else {
                        setControlPanelOpen(false);
                  ...
                  {driverViewActive ? "退出驾驶员视角" : "进入真实驾驶视角"}
                    ...
                    const nextPoseStates = updateTurretStationPose(controlTargetStation, 0, 0);
                    commitTurretNavigation(controlTargetStation.id, nextPoseStates);
                }} viewpointActive={activeCrewViewStationId === controlTargetStation.id} viewpointMarkerEnabled={crewViewpointMarkerEnabled} onViewpointMarkerToggle={() => {
                ...
                }} onInteractionEnd={() => commitTurretNavigation(controlTargetStation.id)}/>) : null}
          </div>
        </aside>) : null}

      {viewerState.kind !== "loading" ? (<div className="viewer-load-status" data-with-close={Boolean(onClose)} aria-live="polite">
      ...
      {onClose ? (<button className="viewer-close" type="button" onClick={onClose} aria-label="关闭载具详情">
          <span aria-hidden="true">×</span>
        </button>) : null}

      {shotResult ? (<div className="viewer-shot-result" aria-live="polite">
          <div className="viewer-shot-result__scroll">
            <div className="viewer-shot-history" aria-label="已保存的射线">
            <div className="viewer-mode-tabs viewer-shot-history__tabs" role="group" aria-label={`${maxShotTraces} 条命中记录`} style={{
                "--viewer-mode-count": maxShotTraces,
                "--viewer-mode-index": Math.max(0, savedShots.findIndex((savedShot) => savedShot.shotId === activeShotId)),
            } as CSSProperties}>
              <span className="viewer-mode-tabs__thumb" aria-hidden="true"/>
              {Array.from({ length: maxShotTraces }, (_, index) => {
                const savedShot = savedShots[index];
                const active = savedShot?.shotId === activeShotId;
                return (<button key={`shot-slot-${index + 1}`} type="button" data-active={active} data-filled={savedShot ? "true" : "false"} aria-pressed={active} aria-label={savedShot
                        ? `查看命中记录 ${index + 1}，结算距离 ${savedShot.distanceM} 米`
                        : `命中记录 ${index + 1} 尚未创建`} title={savedShot ? `命中记录 ${index + 1} · ${savedShot.distanceM} m` : undefined} disabled={!savedShot} onClick={() => savedShot && selectSavedShot(savedShot.shotId)}>
                    <span>记录</span><b>{index + 1}</b>
                  </button>);
            })}
            </div>
            <button className="viewer-shot-history__clear" type="button" aria-label={`清除已保留的 ${savedShots.length} 条命中射线`} onClick={clearShotVisual}>
              清空{savedShots.length}
            </button>
          </div>
          <div className="viewer-shot-heading">
            {activeShotWeaponName ? (<strong className="viewer-shot-weapon-name" title={selectedAttackWeapon?.displayNameEnglish}>
                {activeShotWeaponName}
              </strong>) : null}
            <div className="viewer-shot-metrics" aria-label="弹道摘要">
              <span data-penetration-kind={ballisticsPenetrationKind} title={ballisticsPenetrationLabel} aria-label={`${ballisticsPenetrationLabel} ${metricText(ballistics?.penetrationAtRangeMm ?? null)} 毫米`}>
              ...
              <span data-metric="post-penetration-distance" title="从首个有效命中点起算的最大后效距离" aria-label={`最大后效距离 ${metricText(ballistics?.traceDistanceAfterPenetrationM ?? null)} 米`}>
                <b>后效</b>
                {metricText(ballistics?.traceDistanceAfterPenetrationM ?? null)} m
              </span>
            </div>
          </div>
          {effectiveDamageEvents.length > 0 ? (<section className="viewer-shot-outcome-summary" data-has-components={componentDamageOutcomes.length > 0 ? "true" : "false"} aria-label="本次命中结果">
              <div className="viewer-shot-outcome-summary__total">
                ...
                {hullDamageOutcome?.maxHealth === null
                    || hullDamageOutcome?.maxHealth === undefined
                    || hullRemainingHealth === null ? null : (<span className="viewer-shot-outcome-summary__hull-health" title="车体剩余血量 / 总血量" aria-label={`车体剩余血量 ${metricText(hullRemainingHealth)}，总血量 ${metricText(hullDamageOutcome.maxHealth)}`}>
                    <i aria-hidden="true">
              ...
              <div className="viewer-shot-outcome-summary__details">
                ...
                {componentDamageOutcomes.slice(0, 4).map((outcome) => {
                    ...
                    return (<li key={`${damageAnimationKey}:outcome:${outcome.key}:${outcome.effect?.id ?? "damage"}`} data-damage-pool={outcome.poolKind} data-damage-kind={outcome.damageKinds.size > 1
                            ? "mixed"
                            : [...outcome.damageKinds][0]} data-damage-effect={outcome.effect?.id}>
                      <span className="viewer-shot-outcome-summary__target-heading">
                        <strong>{outcome.label}</strong>
                        {outcome.damageKinds.size > 1 ? (<small>复合</small>) : outcome.damageKinds.has("radial") ? (<small>爆炸</small>) : null}
                        {outcome.effect ? <em>{outcome.effect.label}</em> : null}
                        <b title="本次实际生效伤害">−{metricText(outcome.effectiveDamage)}</b>
                      </span>
                      {remainingHealth === null || outcome.maxHealth === null ? null : (<span className="viewer-shot-outcome-summary__health-rail" title="组件剩余血量 / 总血量" aria-label={`组件剩余血量 ${metricText(remainingHealth)}，总血量 ${metricText(outcome.maxHealth)}`}>
                          <i style={{ width: `${remainingRatio * 100}%` }} aria-hidden="true"/>
                          <b>{metricText(remainingHealth)} / {metricText(outcome.maxHealth)}</b>
                        </span>)}
                      {outcome.effect ? (<span className="viewer-damage-effect" aria-hidden="true">
                          <i />
                          <i />
                          <i />
                        </span>) : null}
                ...
                {componentDamageOutcomes.length > 4 ? (<li className="viewer-shot-outcome-summary__more">
                    另有 {componentDamageOutcomes.length - 4} 个组件
                  </li>) : null}
                </ul>
              </div>
            </section>) : null}
          {visibleShotLayers.length > 0 ? (<div className="viewer-causal-spine__columns" aria-label="路径数值列">
            <span>厚度 · mm</span>
            <span>剩余 · mm</span>
            <span>结果</span>
          </div>) : null}
          <ol className="viewer-causal-spine">
            {visibleShotLayers.map((layer, index) => {
                ...
                const markerKind = editorPathMarkerKind(hitHeader, component, profile, layer.penetrated);
                ...
                return (<li key={`${layer.triangleIndex}:${index}`} className="viewer-causal-spine__step" data-has-next={index < visibleShotLayers.length - 1}>
                  <div className="viewer-causal-spine__layer" data-penetrated={layer.penetrated === true} data-spaced-armor={isSpacedArmor} data-no-penetration={isNoPenetration} data-path-marker={markerKind} data-component-index={layer.componentIndex} data-zero-thickness-behavior={layer.armorThicknessMm === 0
                        ? isNoPenetration
                            ? "explicitly-blocked"
                    ...
                    <span className="viewer-causal-spine__label" title={physicalMaterial ? assetLabel(physicalMaterial) : undefined}>
                    ...
                    <span className="viewer-causal-spine__metrics">
                      <span data-metric="thickness" title={layer.armorThicknessMm === 0
                        ? isNoPenetration
                            ? "该表面明确禁用穿透；0 mm 不会覆盖 NoPen 规则"
                            : layer.penetrated === true
                                ? "原生严格比较为可用穿深 > 0 mm，因此该层已穿透"
                                : layer.stopReason ?? "0 mm 表面的穿透状态无法确认"
                        : "装甲厚度"} aria-label={`装甲厚度 ${metricText(layer.armorThicknessMm)} 毫米`}>
                        {layer.armorThicknessMm === null ? "不可穿透" : layer.armorThicknessMm.toFixed(1)}
                      </span>
                      <span data-metric="remaining" title={pathPresentation.remainingPenetrationMm === null
                        ? "射线已在该层终止，没有后续剩余穿深"
                        : `剩余穿深；距首层 ${layer.distanceFromFirstHitM.toFixed(2)} m，后效距离系数 ${(layer.postPenetrationTraceFactor * 100).toFixed(1)}%`} aria-label={pathPresentation.remainingPenetrationMm === null
                        ? "没有后续剩余穿深"
                      ...
                      <span data-metric="result" data-terminal={pathPresentation.terminalLabel ? "true" : "false"} title={pathPresentation.terminalLabel ?? "本层伤害吸收"} aria-label={pathPresentation.terminalLabel
                  ...
                  {layerDamageEvents.length > 0 ? (<DamageSettlementListItems events={layerDamageEvents} animationKey={`${damageAnimationKey}:layer:${index}`} penetrationKind={ballisticsPenetrationKind}/>) : null}
                  {index < visibleShotLayers.length - 1 ? (<span className="viewer-causal-spine__connector" aria-hidden="true">
                      <i />
                    </span>) : null}
                </li>);
            })}
            {damageEventsByLayer.unassigned.length > 0 ? (<li className="viewer-causal-spine__step viewer-causal-spine__step--unassigned">
                <DamageSettlementListItems events={damageEventsByLayer.unassigned} animationKey={`${damageAnimationKey}:unassigned`} penetrationKind={ballisticsPenetrationKind}/>
              </li>) : null}
          </ol>
          {hasVehicleRadialDamage &&
                shotResult.radial.layers.length > 0 &&
                shotResult.radial.componentFanout !== "drivetrain-resolved" ? (<p className="viewer-radial-coverage-note" data-state={shotResult.radial.componentFanout}>
          ...
          {effectiveDamageEvents.length > 0 ? (<HitDpsTimingCard estimates={weaponHitDpsEstimates} targets={weaponHitDpsTargets} weapon={weaponDpsFacts} factsState={hitDpsFactsState} factsUnavailableReason={targetBurning.state === "unavailable" ? targetBurning.reason : null} clickedSemanticKind={clickedComponent?.semanticKind ?? null}/>) : null}
        </div>) : null}
    </div>);
}
