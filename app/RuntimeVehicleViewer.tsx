...
import {
  ...
  type HeldOperationFireAttempt,
} from "../lib/held-operation-fire-control";
import {
...
function runtimeTurretWorldYaw(
  station: RuntimeTurretPreviewStation,
  stations: RuntimeTurretPreviewStation[],
  poseStates: Readonly<Record<string, RuntimeTurretPoseState>>,
  visiting = new Set<string>(),
): number {
...
function runtimeTurretPosesForStates(
  stations: RuntimeTurretPreviewStation[],
  poseStates: Readonly<Record<string, RuntimeTurretPoseState>>,
): RuntimeTurretPose[] {
...
function LiveOperationTurretControls({
  poseStore,
  stations,
  topDownProjection,
  activeStationId,
  ...
  onReset,
  onInteractionEnd,
}: LiveOperationTurretControlsProps) {
  const poseStates = useSyncExternalStore(
    poseStore.subscribe,
  ...
  const orientationIndicators = useMemo<TurretOrientationIndicator[]>(
    () =>
      stations.map((station) => {
        const relativeYawDegrees = clampTurretYaw(
          station.turret,
          poseStates[station.id]?.yawDegrees ?? 0,
        );
        return {
          id: station.id,
          label: station.label,
  ...
  const yawDegrees = clampTurretYaw(
    activeStation.turret,
    activePose.yawDegrees,
  );
  const pitchDegrees = clampTurretPitch(
    activeStation.turret,
    yawDegrees,
    activePose.pitchDegrees,
  );

  return (
    <>
      <button
        className="crew-view-operation-panel__toggle"
        type="button"
        aria-expanded={expanded}
        aria-label={expanded ? "收起方位俯仰控制" : "展开方位俯仰控制"}
...
function setRuntimeGroundScaleVehicleOcclusion(
  root: THREE.Object3D | null,
  vehicleOccluded: boolean,
) {
  root?.traverse((object) => {
...
function HitDpsTimingCard({
  ...
  const primaryEstimate = selectPrimaryWeaponHitDpsEstimate(
    estimates,
    clickedSemanticKind,
  );
  ...
  const primaryIsAmmoRackVehicleKill =
...
export function RuntimeVehicleViewer({
  ...
  const fireVehicleProjectileRef = useRef<() => HeldOperationFireAttempt>(
  ...
  const selectOperationEquipmentRef = useRef<(equipmentRef: string) => void>(
    () => undefined,
  );
  ...
  const [liveTurretPoseStore] = useState<RuntimeTurretPoseStore>(
    createRuntimeTurretPoseStore,
  );
  ...
  const updateTurretStationPose = useCallback((
    ...
    if (options.transient) {
      turretPosesRef.current = runtimeTurretPosesForStates(
  ...
  const [vehicleWeaponOperationStore] = useState(
    createRuntimeVehicleWeaponOperationStore,
  );
  ...
  const publishVehicleWeaponOperationState = useCallback((
    equipmentRef: string,
    operationState: VehicleWeaponOperationState,
  ) => {
  ...
  useEffect(() => {
    ...
    if (activeCrewViewStationId === null) stopVehicleProjectileFireRef.current();
  ...
  useEffect(() => {
    ...
    const operationPoseCommitScheduler =
      createOperationViewPoseCommitScheduler();
    ...
    const startOperationMovement = () => {
      if (operationMovementFrame !== 0) return;
    ...
    const onOperationKeyDown = (event: KeyboardEvent) => {
      ...
      if (movementCodes.has(event.code)) {
        ...
        if (movementWasIdle) startOperationMovement();
        return;
      }
      ...
      if (!action || (action.kind === "zoom" && !station)) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      if (action.kind === "zoom") {
  ...
  const graphOperationEquipmentRefs = useMemo(
    () => driverViewActive
      ? preview.stationGraph?.vehicleEquipmentRefs ?? []
      : activeOperationGraphStation?.equipmentRefs ?? [],
    [
  ...
  const sightOperationEquipmentRefs = useMemo(
    () => activeGunnerSightStation?.weaponModes.map(
      (mode) => mode.equipmentRef,
    ) ?? [],
    [activeGunnerSightStation],
  );
  const playableOperationEquipmentRefs = useMemo(
    () => vehicleOperationWeapons.flatMap(
      (weapon) => weapon.stationEquipmentId ? [weapon.stationEquipmentId] : [],
    ),
    [vehicleOperationWeapons],
  );
  const operationEquipmentRefs = useMemo(
    () => !equipmentResolver ? [] : operationViewEquipmentRefs({
      ...
      inventorySlots: graphOperationEquipmentRefs.flatMap((equipmentRef) =>
        equipmentResolver?.(equipmentRef)?.firingPresentation?.inventorySlotNumbers.map(
          (slotNumber) => ({ equipmentRef, slotNumber }),
        ) ?? []),
    }),
    [
  ...
  useEffect(() => {
    setActiveOperationEquipmentRef((current) =>
      operationEquipmentRefs.includes(current)
        ? current
  ...
  const selectOperationEquipment = useCallback((equipmentRef: string) => {
    if (
      !operationEquipmentRefs.includes(equipmentRef) ||
  ...
  useEffect(() => {
    ...
    operationInventorySlotsRef.current = operationEquipmentRefs.flatMap((equipmentRef) =>
      equipmentResolver?.(equipmentRef)?.firingPresentation?.inventorySlotNumbers.map(
        (slotNumber) => ({ equipmentRef, slotNumber }),
      ) ?? []);
  ...
  useEffect(() => {
    if (activeCrewViewStationId === null || firingPresentation) return;
    let active = true;
    void loadWikiDataset("/data/weapons/firing-presentation.json", "sigua-vehicle-firing-presentation/v1")
      .then((value) => { if (active) setFiringPresentation(value as VehicleFiringPresentation); })
      .catch((error: unknown) => {
        if (active) setVehicleProjectileNotice(error instanceof Error ? error.message : String(error));
      });
    return () => { active = false; };
  }, [activeCrewViewStationId, firingPresentation]);
  useEffect(() => {
    if (!firingPresentation || activeCrewViewStationId === null) return;
    ...
    void Promise.all(visuals.map(visual => prepareVehicleProjectileVisualRef.current?.(visual)))
      .catch((error: unknown) => { if (active) setVehicleProjectileNotice(`弹体模型加载失败：${error instanceof Error ? error.message : String(error)}`); });
    return () => { active = false; };
  ...
  const fireVehicleProjectile = useCallback((): HeldOperationFireAttempt => {
    ...
    try {
      ...
      if (!firingPresentation) throw new Error("正在载入游戏弹体显示资源…");
      ...
      if (!sourceWeapon) throw new Error("当前武器缺少源弹体显示配置");
      ...
      if (!pawnOrigin) throw new Error("当前武器的所属位置尚未就绪");
      ...
      if (!armorCache) { armorCache = new NativeWeaponArmorCache(); schoolWeaponCachesRef.current.set(cacheKey,armorCache); }
      ...
      if (contact && contact.timeSeconds <= (samples.at(-1)?.timeSeconds ?? 0)) {
        ...
        impactTrace = buildSchoolImpactTrace({ query:school.query, offset:school.offset, hit:contact.hit.sceneHit,
          ...
          evaluateHit:hit=>resolveEditorWeaponHitRanges(activeOperationWeapon.ballisticsModel,
            activeOperationWeapon.ballisticsWeaponIndex,{
              ...
              // New shots begin with the published default damage multiplier.
  ...
  const refillVehicleAmmunition = useCallback(() => {
    // Stop held fire before clearing: releasing the trigger can publish a state.
    stopVehicleProjectileFireRef.current();
    vehicleWeaponOperationStatesRef.current.clear();
    vehicleWeaponOperationStore.clear();
    vehicleProjectileMagazineStateRef.current = {
    ...
    // Unused/reset weapons initialize from their source magazine capacities and
  ...
  useEffect(() => {
    ...
    setRuntimeGroundScaleVehicleOcclusion(
      groundScaleRef.current,
      mode === "exterior",
    );
  ...
  useEffect(() => {
    ...
    const ensureSchoolQuery = () => {
      if (schoolQueryRequest || cancelled) return;
      ...
      schoolQueryRequest = import("../lib/runtime-school-native-loader")
        .then(module => module.loadNarvaSchoolQuery()).then(query => {
          if (cancelled) return;
          // Keep the live environment translation as the vehicle fit settles.
    ...
    const applyTurretPose = (options: { interactive?: boolean } = {}) => {
      ...
      for (const [occurrenceId, articulationTransform] of occurrenceTransforms) {
        ...
        for (const analysisOccurrence of
          ...
          if (!interactive) appliedAnalysisOccurrenceCount += 1;
        }
      }
      if (interactive) {
    ...
    resolveWeaponPawnOriginRef.current = (stationId) => {
      chassisPoseGroup.updateMatrixWorld(true);
      if (stationId === null) return new THREE.Vector3().setFromMatrixPosition(chassisPoseGroup.matrixWorld);
      ...
      if (!definition?.mount.referenceFrame.value || !station) return null;
      const matrix=vehicleProjectileAnchorMatrixFromUnrealFrame(definition.mount.referenceFrame.value);
      // The owning Pawn is the mounted seat Actor. Its own turret components
      // rotate beneath it; only inherited parent motion moves its Actor origin.
      for (const parent of stationArticulationMatrixChainForChannels(station,[])) matrix.premultiply(new THREE.Matrix4().fromArray(parent));
    ...
    const fitViewToGroup = (
      ...
      const exitCrewViewpoint = () => {
        const wasDriver =
          activeCrewViewStationIdRef.current === DRIVER_VIEWPOINT_ID;
        if (!wasDriver) flushOperationPoseCommitRef.current?.();
        activeCrewViewStationIdRef.current = null;
        setActiveCrewViewStationId(null);
        activeCrewViewZoomIndexRef.current = 0;
        setActiveCrewViewZoomIndex(0);
    ...
    const onOperationVisibilityChange = () => {
      if (document.visibilityState !== "visible") stopHeldOperationFire();
    };
  ...
  return (
    ...
    >
      ...
      {activeCrewViewStationId !== null && firingPresentation && equipmentResolver ? (
        <VehicleWeaponHud
          document={firingPresentation}
          store={vehicleWeaponOperationStore}
          activeEquipmentRef={activeOperationEquipmentRef}
      ...
      {activeCrewViewStationId !== null ? (
        <div className="crew-view-operation-dock">
          <div
          ...
          >
            ...
            {activeOperationWeapon ? (
              <button
                type="button"
                role="switch"
                aria-label="弹道与弹体可见度增强"
                aria-checked={projectileVisibilityEnhanced}
                data-active={projectileVisibilityEnhanced || undefined}
                title="网页辅助标记：加亮弹体位置与飞行轨迹，不改变真实弹道、弹速或模型尺寸"
                onPointerDown={(event) => event.stopPropagation()}
                onClick={(event) => {
            ...
            {activeOperationWeapon || !driverViewActive ? (
              <span className="crew-view-operation-keys" aria-label="键盘操作提示">
                {activeOperationWeapon ? (
                  <><kbd>左键</kbd><span>按住开火</span></>
                ) : null}
                {!driverViewActive ? (
                  <>
          ...
          {!driverViewActive && activeTurretStation ? (
            <div
              className="crew-view-operation-panel"
              data-expanded={operationPanelExpanded}
              aria-label={`${activeTurretStation.label}方位俯仰控制`}
            >
              <LiveOperationTurretControls
                poseStore={liveTurretPoseStore}
                stations={runtimeTurretStations}
                topDownProjection={turretTopDownProjection}
                activeStationId={activeTurretStation.id}
                expanded={operationPanelExpanded}
                onExpandedChange={setOperationPanelExpanded}
      ...
      <div ref={worldImpactStatusRef} className="runtime-world-impact-status" role="status"
        aria-live="off" title="最近一次实际命中的参考线；绿点：穿透，红点：停止，黄点：未确认。只保留一条，不代表弹体继续飞行；适用于当前学校范围，不包含建筑破坏与爆炸范围伤害。" />
            ...
            {preview.targetKind !== "command-aircraft" ? <div className="viewer-crew-occupant-row">
              ...
              {crewOccupantDisplayEnabled ? (
                ...
                >
                  ...
                  {crewOccupantCounts.nonSpatial > 0 ? (
                    <span
                      data-kind="nonspatial"
                      title={`无人物 socket 的 Hidden：${nonSpatialCrewOccupants.map(
                        (occupant) =>
                          `F${occupant.catalogSeatIndex} ${occupant.requestedSocketName ?? "未命名人物 socket"}${
