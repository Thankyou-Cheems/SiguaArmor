...
import { crewViewBasePose, crewViewHorizontalFovForZoom, preferredCrewViewStation, transformCrewViewPose, type CrewViewPose, } from "../lib/vehicle-crew-viewpoint";
import { driverViewPose } from "../lib/vehicle-driver-view";
import { operationViewKeyAction, operationViewScenePresentation, } from "../lib/operation-view-control";
...
import { type RuntimeAttackSource, } from "./runtime-probe-weapon-labels";
...
export function RuntimeVehicleViewer({ preview, showChrome = true, mode: requestedMode = "exterior", displayName = preview.variantRawName, attackSourcePresentation, referenceData, onModeChange, onClose, navigationState, onNavigationStateChange, onExteriorStreamingChange, attackLibraryOverride, duelTarget = false, allowGlobalAttackSources = true, onDuelHitChange, displayOverrides, shotTraceLimit = MAX_SHOT_TRACES, }: {
    ...
    return (<div className="viewer-stage runtime-vehicle-viewer" data-viewer-state={viewerState.kind} data-viewer-presentation={viewerPresentation} data-viewer-variant-raw-name={preview.variantRawName} data-hit-state={hitState.kind} data-hit-access={hit?.status ?? "absent"} data-hit-probe-verdict={verdict} data-static-hit-runtime={hitState.kind === "ready" ? "true" : undefined} data-hit-solver={hitState.kind === "ready" ? "editor-native-direct-hit" : undefined} data-hit-weapon-label-source={attackReady
      ...
      {gunnerSightOverlayVisible && activeGunnerSightStation && activeTurretStation && gunnerSight ? (<GunnerSightOverlay station={activeGunnerSightStation} projections={gunnerSight.projections} stationLabel={activeTurretStation.label} magnificationLevels={activeTurretStation.view?.magnificationLevels ?? []} zoomHorizontalFovDegrees={activeTurretStation.view?.magnificationLevels.map((_, zoomIndex) => crewViewHorizontalFovForZoom(activeTurretStation.view!, zoomIndex)) ?? []} activeZoomIndex={activeCrewViewZoomIndex} onZoomStageChange={(zoomIndex) => {
      ...
      <div className="viewer-toolbar" aria-label="3D 查看模式">
        <div className="viewer-toolbar__tertiary">
          <div className="viewer-protection-controls viewer-control-deck" data-enabled={protectionMapAvailable} data-selector-open={attackSelectorOpen} data-revealed={upperOptionsRevealed}>
        ...
        } as CSSProperties} aria-label={`防护图透明度 ${protectionOpacityPercent}%`} onChange={(event) => setProtectionOpacityPercent(Number(event.currentTarget.value))}/>
            ...
            : `防护图计算精度 ${protectionPrecision} 档`} onInput={(event) => setProtectionPrecision(clampRuntimeProtectionMapPrecision(Number(event.currentTarget.value)))} onChange={(event) => setProtectionPrecision(clampRuntimeProtectionMapPrecision(Number(event.currentTarget.value)))}/>
            ...
            {protectionActive ? (<div className="viewer-protection-legend" aria-label="防护图图例">
      ...
      {controlPanelOpen ? (<aside className="viewer-weapon-panel" id="viewer-weapon-panel" aria-label="视角与武器站控制">
          <div className="viewer-control-target-row">
            <div className="viewer-control-target-slider" role="tablist" aria-label="选择视角或操控位置">
              <button type="button" role="tab" aria-selected={controlTargetId === CAMERA_CONTROL_TARGET_ID} data-active={controlTargetId === CAMERA_CONTROL_TARGET_ID || undefined} onClick={() => setControlTargetId(CAMERA_CONTROL_TARGET_ID)}>
                相机
              </button>
              <button type="button" role="tab" aria-selected={controlTargetId === DRIVER_CONTROL_TARGET_ID} data-active={controlTargetId === DRIVER_CONTROL_TARGET_ID || undefined} onClick={() => setControlTargetId(DRIVER_CONTROL_TARGET_ID)}>
                驾驶 · F1
              </button>
              {runtimeTurretStations.map((station) => (<button type="button" role="tab" aria-selected={controlTargetId === station.id} data-active={controlTargetId === station.id || undefined} title={`${station.label} · ${station.equipmentLabel}`} onClick={() => {
