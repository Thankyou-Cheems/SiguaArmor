...
export function RuntimeVehicleViewer({ preview, showChrome = true, mode: requestedMode = "exterior", displayName = preview.variantRawName, attackSourcePresentation, referenceData, onModeChange, onClose, navigationState, onNavigationStateChange, onExteriorStreamingChange, attackLibraryOverride, duelTarget = false, allowGlobalAttackSources = true, onDuelHitChange, displayOverrides, shotTraceLimit = MAX_SHOT_TRACES, }: {
    ...
    return (<div className="viewer-stage runtime-vehicle-viewer" data-viewer-state={viewerState.kind} data-viewer-presentation={viewerPresentation} data-viewer-variant-raw-name={preview.variantRawName} data-hit-state={hitState.kind} data-hit-access={hit?.status ?? "absent"} data-hit-probe-verdict={verdict} data-static-hit-runtime={hitState.kind === "ready" ? "true" : undefined} data-hit-solver={hitState.kind === "ready" ? "editor-native-direct-hit" : undefined} data-hit-weapon-label-source={attackReady
      ...
      <div className="viewer-toolbar" aria-label="3D 查看模式">
        <div className="viewer-toolbar__tertiary">
          <div className="viewer-protection-controls viewer-control-deck" data-enabled={protectionMapAvailable} data-selector-open={attackSelectorOpen} data-revealed={upperOptionsRevealed}>
            ...
            <details className="viewer-control-section" data-control-section="camera">
              <summary>
                <span><i aria-hidden="true"/>相机与姿态</span>
                <strong className="viewer-control-section__status">
                  {RUNTIME_VIEWER_CAMERA_VIEWS.find(({ id }) => id === activeCameraView)?.label ?? "自由"}
                </strong>
              </summary>
              <div className="viewer-control-section__body">
            ...
            <div className="viewer-driver-view-controls" data-mask-available={driverMaskAvailable || undefined}>
            ...
            <div className="viewer-physical-pose-row">
              <button className="viewer-protection-switch viewer-state-switch viewer-physical-pose-switch" type="button" role="switch" aria-label="真实物理状态" aria-checked={physicalPoseActive} data-active={physicalPoseActive} disabled={!chassisPose} title={chassisPose
            ? [
            ...
            : "该 exact generated class 没有稳定收敛的运行时底盘姿态；不会猜测或套用相近载具。"} onClick={() => setPhysicalPoseEnabled((enabled) => !enabled)}>
      ...
      {!weaponPanelOpen && activeTurretStation ? (<button className="viewer-weapon-panel-launcher" type="button" aria-controls="viewer-weapon-panel" aria-expanded="false" onClick={() => setWeaponPanelOpen(true)}>
          <span>
            <i aria-hidden="true"/>
            <b>火控与武器站</b>
            <small>右侧面板</small>
      ...
      {weaponPanelOpen && activeTurretStation ? (<aside className="viewer-weapon-panel" id="viewer-weapon-panel" aria-label="武器站与真实操作视角">
          <button className="viewer-weapon-panel__collapse" type="button" onClick={() => setWeaponPanelOpen(false)} aria-label="收起武器站与炮镜面板">
            收起 ›
          </button>
          <div className="viewer-weapon-panel__body">
            <TurretPreviewControls embedded stations={runtimeTurretStations} orientationIndicators={turretOrientationIndicators} activeStationId={activeTurretStation.id} yawDegrees={clampedTurretYaw} pitchDegrees={clampedTurretPitch} onStationChange={(stationId) => {
                setActiveTurretStationId(stationId);
                if (activeCrewViewStationId !== null) {
                    enterCrewViewpointRef.current?.(stationId);
                }
                ...
                const nextPoseStates = updateTurretStationPose(activeTurretStation, 0, 0);
                commitTurretNavigation(activeTurretStation.id, nextPoseStates);
            }} viewpointActive={activeCrewViewStationId === activeTurretStation.id} viewpointMarkerEnabled={crewViewpointMarkerEnabled} onViewpointMarkerToggle={() => {
            ...
            }} onInteractionEnd={() => commitTurretNavigation(activeTurretStation.id)}/>
          </div>
        </aside>) : null}
